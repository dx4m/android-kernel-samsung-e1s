...
#if defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E8845)
			set_bits(ctrl_dmicif, VTS_DMIC_LPFEXT_EN_SIZE, VTS_DMIC_LPFEXT_EN_OFFSET);
			set_bits(ctrl_dmicif, VTS_DMIC_CPSEXT_EN_SIZE, VTS_DMIC_CPSEXT_EN_OFFSET);
#endif
...
static irqreturn_t vts_internal_rec_elapsed_handler(int irq, void *dev_id)
{
	...
	if (pointer)
		pointer = (pointer - (data->dma_area_vts + DMA_BUF_INTERNAL_OFFSET));
