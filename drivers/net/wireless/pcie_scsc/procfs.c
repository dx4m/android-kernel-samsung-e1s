...
static ssize_t slsi_procfs_mlo_sec_link_mac_write(struct file *file, const char __user *user_buf, size_t len,
						  loff_t *ppos)
{
	struct slsi_dev *sdev = (struct slsi_dev *)file->private_data;
	struct net_device *dev = slsi_get_netdev(sdev, SLSI_NET_INDEX_WLAN);
	...
	u8 read_len;
	int val, ret;
	char *sec_link_addr = NULL;
	char *sec_link_saveptr = NULL;
	char *mac_addr = NULL;
	char *mac_addr_saveptr = NULL;
	u8 i = 0, j = 0;

	read_len = simple_write_to_buffer(read_string, sizeof(read_string), ppos, user_buf, (len > sizeof(read_string) ?
					  sizeof(read_string) - 1 : len));
	read_string[read_len] = '\0';

	sec_link_addr = slsi_strtok_r(read_string, " ", &sec_link_saveptr);
	while (sec_link_addr != NULL && i < MAX_SUPP_MLO_LINKS) {
		j = 0;
		mac_addr = slsi_strtok_r(sec_link_addr, ":", &mac_addr_saveptr);
		while (mac_addr != NULL && j < ETH_ALEN) {
			if (kstrtoint(mac_addr, 16, &val)) {
				SLSI_ERR(sdev, "invalid input %s\n", read_string);
				ret = -EINVAL;
			}
			ndev_vif->sta.sec_ap_link_addr[i][j] = val;
			mac_addr = slsi_strtok_r(NULL, ":", &mac_addr_saveptr);
			j++;
		}

		if (j < ETH_ALEN) {
			SLSI_ERR(sdev, "invalid mac addr\n");
			ret = -EINVAL;
		}

		sec_link_addr = slsi_strtok_r(NULL, " ", &sec_link_saveptr);
		i++;
	}
...
static ssize_t slsi_procfs_mlo_forced_bandcontrol_write(struct file *file, const char __user *user_buf, size_t len,
							loff_t *ppos)
{
	struct slsi_dev *sdev = (struct slsi_dev *)file->private_data;
	struct net_device *dev = slsi_get_netdev(sdev, SLSI_NET_INDEX_WLAN);
	...
	if (kstrtoint(read_string, 10, &val)) {
		...
		ndev_vif->sta.mlo_forced_bandcontrol = (bool)val;
		ret = sizeof(read_string) - 1;
	}
...
SLSI_PROCFS_WRITE_FILE_OPS(mlo_sec_link_mac);
SLSI_PROCFS_WRITE_FILE_OPS(mlo_forced_bandcontrol);
#endif
...
int slsi_create_proc_dir(struct slsi_dev *sdev)
{
	...
	if (parent) {
		...
		SLSI_PROCFS_ADD_FILE(sdev, mlo_sec_link_mac, parent, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
		SLSI_PROCFS_ADD_FILE(sdev, mlo_forced_bandcontrol, parent, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
#endif
...
void slsi_remove_proc_dir(struct slsi_dev *sdev)
{
	if (sdev->procfs_dir) {
		...
		SLSI_PROCFS_REMOVE_FILE(mlo_sec_link_mac, sdev->procfs_dir);
		SLSI_PROCFS_REMOVE_FILE(mlo_forced_bandcontrol, sdev->procfs_dir);
#endif
