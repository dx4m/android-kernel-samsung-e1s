...
#include "tx_api.h"
#endif
#if defined(CONFIG_SLSI_WLAN_CP_COEX)
...
static struct sk_buff *slsi_mlme_wait_for_ind(struct slsi_dev *sdev, struct net_device *dev, struct slsi_sig_send *sig_wait, u16 ind_id)
{
	...
	if (ind_id == MLME_SCAN_DONE_IND)
		/* To handle the coex scenario where BTscan has high priority increasing the wait time to 20 secs */
...
int slsi_mlme_set_roaming_parameters(struct slsi_dev *sdev, struct net_device *dev, u16 psid, int mib_value, int mib_length)
{
	...
	SLSI_NET_DBG1(dev, SLSI_MLME, "mlme_set_roaming_parameters_req(vif:%d value:%d)\n",
				  ndev_vif->vifnum, mib_value);

	req = fapi_alloc(mlme_set_roaming_parameters_req, MLME_SET_ROAMING_PARAMETERS_REQ, ndev_vif->vifnum, 0);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
	fapi_set_u16(req, u.mlme_set_roaming_parameters_req.vif, ndev_vif->vifnum);
...
{
	if (sdev->device_config.supported_band == SLSI_FREQ_BAND_2_4GHZ_5GHZ_6GHZ)
		return true;

	if (band == NL80211_BAND_2GHZ &&
	    (sdev->device_config.supported_band != SLSI_FREQ_BAND_AUTO &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_2GHZ &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_2_4GHZ_6GHZ))
		return false;
	if (band == NL80211_BAND_5GHZ &&
	    (sdev->device_config.supported_band != SLSI_FREQ_BAND_AUTO &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_5GHZ &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_5GHZ_6GHZ))
		return false;
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
	if (band == NL80211_BAND_6GHZ &&
	    (sdev->device_config.supported_band != SLSI_FREQ_BAND_6GHZ &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_5GHZ_6GHZ &&
	     sdev->device_config.supported_band != SLSI_FREQ_BAND_2_4GHZ_6GHZ))
		return false;
#endif
...
int slsi_mlme_start(struct slsi_dev *sdev, struct net_device *dev, const unsigned char *bssid, struct cfg80211_ap_settings *settings, const u8 *wpa_ie_pos, const u8 *wmm_ie_pos, bool append_vht_ies)
{
	...
	SLSI_NET_INFO(dev, "mlme_start_req(ifnum:%u, bssid:" MACSTR ", ssid:%.*s, hidden:%d)\n",
	...
	SLSI_NET_INFO(dev, "channel_encode (fixed) = %d\n", channel_encode);
...
#ifdef CONFIG_SCSC_WLAN_EHT
static bool slsi_rsno_check_akm_type(const struct cfg80211_bss_ies *beacon_ies, int variant)
{
	const u8 *rsn, *rsne_o1, *rsne_o2;

	if (variant == 1) {
		rsne_o1 = cfg80211_find_vendor_ie(WLAN_OUI_WFA, SLSI_RSNE_OVERRIDE_IE_VENDOR_TYPE, beacon_ies->data,
						  beacon_ies->len);
		return slsi_is_rsno_wpa3_owe_support(rsne_o1, beacon_ies->len - (rsne_o1 - beacon_ies->data));
	} else if (variant == 2) {
		rsne_o2 = cfg80211_find_vendor_ie(WLAN_OUI_WFA, SLSI_RSNE_OVERRIDE_2_IE_VENDOR_TYPE, beacon_ies->data,
						  beacon_ies->len);
		return slsi_is_rsno_wpa3_owe_support(rsne_o2, beacon_ies->len - (rsne_o2 - beacon_ies->data));
	} else if (variant == 0) {
		rsn = cfg80211_find_ie(WLAN_EID_RSN, beacon_ies->data, beacon_ies->len);
		if (rsn)
			return (slsi_is_wpa3_support(rsn, beacon_ies->len - (rsn - beacon_ies->data)) ||
...
int slsi_mlme_connect(struct slsi_dev *sdev, struct net_device *dev, struct cfg80211_connect_params *sme, struct ieee80211_channel *channel, const u8 *bssid)
{
	...
	if (ndev_vif->sta.sta_bss) {
		...
		if (ml_ie) {
			/* RSN Selection Element for RSN Override Parsing */
			if (sme->ie_len)
				rsnse = cfg80211_find_vendor_ie(WLAN_OUI_WFA, SLSI_RSN_SELECTION_IE_VENDOR_TYPE,
								sme->ie, sme->ie_len);
			/* Length is fixed in RSN Selection element */
			if (rsnse && (rsnse[1] == SLSI_RSN_SELECTION_IE_LEN)) {
				variant = rsnse[6];
				allow_ml = slsi_rsno_check_akm_type(beacon_ies, variant);
				SLSI_NET_DBG1(dev, SLSI_MLME, "RSNO: RSNSE Variant: %d allow_ml: %d\n", variant, allow_ml);
			}
			rsn = cfg80211_find_ie(WLAN_EID_RSN, beacon_ies->data, beacon_ies->len);
			if (allow_ml || (rsn &&
	...
	if (!ndev_vif->sta.mlo_forced_bandcontrol)
			    ...
			    struct slsi_sta_dump_data *dump_data)
{
	...
	/* The meaning of -32768 is the error value transmitted by Firmware
	 * If the antenna is disabled or measure incorrectly and invalid,
	 * it is included in the value and transmitted.
	 */
	if (values[mib_index].type != SLSI_MIB_TYPE_NONE) {
		...
		if (values[mib_index].u.intValue != -32768)
	...
	if (dump_data->param_field & (BIT(SLSI_STA_DUMP_ANT1RSSI) | BIT(SLSI_STA_DUMP_ANT2RSSI))) {
		...
		if (dump_data->data[SLSI_STA_DUMP_ANT1RSSI] == -32768)
			dump_data->data[SLSI_STA_DUMP_RSSI] = dump_data->data[SLSI_STA_DUMP_ANT2RSSI];
		else if (dump_data->data[SLSI_STA_DUMP_ANT2RSSI] == -32768)
			      ...
			      u16 data_desc, u16 msg_type, u16 host_tag, u16 freq, u32 dwell_time, u32 period)
{
	...
	* Work around to modify the auth packet and use BSSID instead */
	...
	if (ndev_vif->iftype == NL80211_IFTYPE_STATION && ndev_vif->sta.sta_bss) {
		...
		} else if (ndev_vif->sta.valid_links) {
			SLSI_ETHER_COPY(mgmt->da, ndev_vif->sta.links[ndev_vif->sta.assoc_resp_link_id].bssid);
			SLSI_ETHER_COPY(mgmt->bssid, ndev_vif->sta.links[ndev_vif->sta.assoc_resp_link_id].bssid);
		}
	}
#endif
...
int slsi_mlme_sar_get_max_tx_power(struct slsi_dev *sdev, struct net_device *dev,
				   struct slsi_sar_max_tx_power *max_tx_pwr)
{
	struct sk_buff *req;
	struct sk_buff *cfm;
	int ret = 0;

	req = fapi_alloc(mlme_sar_get_max_tx_power_req, MLME_SAR_GET_MAX_TX_POWER_REQ, 0, 0);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}

	cfm = slsi_mlme_req_cfm(sdev, NULL, req, MLME_SAR_GET_MAX_TX_POWER_CFM);
	if (!cfm)
		return -EIO;

	ret = fapi_get_u16(cfm, u.mlme_sar_get_max_tx_power_cfm.result_code);
	if (ret == FAPI_RESULTCODE_SUCCESS) {
		max_tx_pwr->antenna1non_dbs_power_limit =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna1non_dbs_power_limit);
		max_tx_pwr->antenna2non_dbs_power_limit =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna2non_dbs_power_limit);
		max_tx_pwr->antenna1dbs_power_limit =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna1dbs_power_limit);
		max_tx_pwr->antenna2dbs_power_limit =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna2dbs_power_limit);
		max_tx_pwr->antenna1last_tx_power =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna1last_tx_power);
		max_tx_pwr->antenna2last_tx_power =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna2last_tx_power);
		max_tx_pwr->antenna1average_tx_power =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna1average_tx_power);
		max_tx_pwr->antenna2average_tx_power =
			fapi_get_buff(cfm, u.mlme_sar_get_max_tx_power_cfm.antenna2average_tx_power);
	} else {
		SLSI_NET_ERR(dev, "mlme_sar_get_max_tx_power_cfm(result:0x%04x) ERROR\n", ret);
		ret = -EINVAL;
	}
...
int slsi_mlme_set_uwbcx(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(ndev_vif->vif_mutex));
	req = fapi_alloc(mlme_set_uwbcx_req, MLME_SET_UWBCX_REQ, 0, 0);
	if (!req)
		return -ENOMEM;
...
void slsi_mlme_tas_init(struct slsi_dev *sdev)
{
	...
	slsi_mlme_tas_clear_param(sdev);
	tas_info->cat2_dsi_id = 20; /* DSI index Max mode */
...
#if defined(CONFIG_SLSI_WLAN_DCTAS)
/**
 * slsi_mlme_sar_set_dctas_req - Send a MLME-SAR-SET-DCTAS.request to fw.
 * @sdev: Pointer to the slsi_dev structure.
 *
 * This function sends a MLME-SAR-SET-DCTAS.request to the firmware with the current CP state information.
 * It allocates a socket buffer for the request, sets the appropriate fields, and sends the request.
 * It then waits for the confirmation and processes the result.
 * Returns 0 on success, otherwise returns an appropriate error code.
 */
int slsi_mlme_sar_set_dctas_req(struct slsi_dev *sdev)
{
	struct sk_buff *req = NULL;
	struct sk_buff *cfm = NULL;
	int ret = 0;

	req = fapi_alloc(mlme_sar_set_dctas_req, MLME_SAR_SET_DCTAS_REQ, 0, 0);
	if (!req) {
		SLSI_ERR_NODEV("failed to alloc DCTAS request\n");
		return -ENOMEM;
	}

	SLSI_INFO_NODEV("state : %d, duration : %d, policy : %d, average : %d, s_upper : %d\n",
			sdev->dctas.cp_cp_state.state, sdev->dctas.cp_state_duration, sdev->dctas.cp_cp_state.policy,
			sdev->dctas.cp_cp_state.average, sdev->dctas.cp_cp_state.s_upper);

	fapi_set_u8(req, u.mlme_sar_set_dctas_req.cellular_state, sdev->dctas.cp_cp_state.state);
	fapi_set_u8(req, u.mlme_sar_set_dctas_req.cellular_off_state_duration, sdev->dctas.cp_state_duration);
	fapi_set_u8(req, u.mlme_sar_set_dctas_req.wlan_policy, sdev->dctas.cp_cp_state.policy);
	fapi_set_u8(req, u.mlme_sar_set_dctas_req.cellular_sar_average, sdev->dctas.cp_cp_state.average);
	fapi_set_u8(req, u.mlme_sar_set_dctas_req.cellular_sar_upper, sdev->dctas.cp_cp_state.s_upper);

	cfm = slsi_mlme_req_cfm(sdev, NULL, req, MLME_SAR_SET_DCTAS_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_sar_set_dctas_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_ERR_NODEV("mlme_sar_set_dctas_cfm(result:0x%04x) ERROR\n",
			       fapi_get_u16(cfm, u.mlme_sar_set_dctas_cfm.result_code));
		ret = -EINVAL;
		if (fapi_get_u16(cfm, u.mlme_sar_set_dctas_cfm.result_code) == FAPI_RESULTCODE_NOT_SUPPORTED)
			ret = -EOPNOTSUPP;
	}
...
int slsi_mlme_set_max_bw(struct slsi_dev *sdev, struct net_device *dev, u8 link_id,
			 u16 announce_validate, u16 bandwidth)
{
	...
	SLSI_NET_DBG1(dev, SLSI_MLME, "announce_validate:%d bandwidth:%d\n", announce_validate, bandwidth);
...
#if defined(CONFIG_SLSI_WLAN_CP_COEX)
int slsi_mlme_set_cellular_info(struct slsi_dev *sdev)
{
	struct sk_buff *req;
	struct sk_buff *rx;
	int            r = 0, i = 0;
	size_t         alloc_data_size = 0;
	struct slsi_mlme_parameters req_hdr;

	WLBT_WARN_ON(!SLSI_MUTEX_IS_LOCKED(sdev->cp_coex_mutex));

	SLSI_INFO(sdev,  "cp_cell_infos.num_cell: %d\n", sdev->cp_cell_infos.num_cell);
	if (sdev->cp_cell_infos.num_cell > CP_MAX_BAND)
		return -EINVAL;

	memset(&req_hdr, 0, sizeof(req_hdr));
	req_hdr.element_id = SLSI_WLAN_EID_VENDOR_SPECIFIC;
	req_hdr.length = SLSI_MLME_PARAM_HEADER_SIZE + sizeof(struct cp_cell_info);
	slsi_mlme_put_oui(req_hdr.oui, SLSI_MLME_SAMSUNG_OUI);
	slsi_mlme_put_oui_type(&req_hdr, SLSI_MLME_TYPE_COEX, SLSI_MLME_SUBTYPE_COEX_CP_INFO);

	alloc_data_size = (sdev->cp_cell_infos.num_cell ? sdev->cp_cell_infos.num_cell : 1) * (req_hdr.length + 2);
	req = fapi_alloc(mlme_set_cellular_info_req, MLME_SET_CELLULAR_INFO_REQ, 0, alloc_data_size);
	if (!req) {
		SLSI_ERR(sdev, "failed to alloc %zd\n", alloc_data_size);
		return -ENOMEM;
	}
	SLSI_DBG2(sdev, SLSI_MLME, "count:%d allocated data size: %d\n", sdev->cp_cell_infos.num_cell, alloc_data_size);

	fapi_set_u16(req, u.mlme_set_cellular_info_req.vif, 0);
	do {
		fapi_append_data(req, (u8 *)&req_hdr, sizeof(req_hdr));
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].rat, 1);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].band, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].channel, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].connection_status, 1);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].bandwidth, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].sinr, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].rsrp, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].rsrq, 4);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].cqi, 1);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i].dl_mcs, 1);
		fapi_append_data(req, (u8 *)&sdev->cp_cell_infos.cell_list[i++].pusch_power, 4);
	} while (i < sdev->cp_cell_infos.num_cell);

	rx = slsi_mlme_req_cfm(sdev, NULL, req, MLME_SET_CELLULAR_INFO_CFM);
	if (!rx)
		return -EIO;
	if (fapi_get_u16(rx, u.mlme_set_cellular_info_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_ERR(sdev, "mlme_set_cellular_info_cfm(ERROR:0x%04x)",
			 fapi_get_u16(rx, u.mlme_set_cellular_info_cfm.result_code));
		r = -EINVAL;
	}
