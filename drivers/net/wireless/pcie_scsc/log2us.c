			       ...
			       short rssi)
{
	...
	if (rssi == 0) {
		slsi_log2us_get_rssi(sdev, dev, (int *) &rssi, SLSI_PSID_UNIFI_RSSI);
		SLSI_INFO(sdev, "Rssi fetched from mib = %d\n", rssi);
	}
...
void slsi_conn_log2us_assoc_req(struct slsi_dev *sdev, struct net_device *dev,
				...
				int mgmt_frame_subtype, u8 mlo_band, short rssi)
{
	int pos = 0;
	char *log_buffer = NULL;
	int buf_size = BUFF_SIZE;
...
void slsi_conn_log2us_deauth(struct slsi_dev *sdev, struct net_device *dev, char *str_type,
			     const unsigned char *bssid,
			     int sn, int status, char *vs_ie, short rssi)
{
	int pos = 0;
	char *log_buffer = NULL;
	int buf_size = BUFF_SIZE;
...
void slsi_conn_log2us_btm_wtc(struct slsi_dev *sdev, struct net_device *dev,
			      u8 reason, u8 subreason, u32 duration)
{
	int pos = 0;
	char *log_buffer = NULL;
	int buf_size = BUFF_SIZE;
	...
	get_kernel_timestamp(time);
	pos += scnprintf(log_buffer + pos, buf_size - pos, "[%d.%d][BTM] WTC reason=%d sub_code=%d duration=%d",
			 time[0], time[1], reason, subreason, duration);
...
void slsi_conn_log2us_btm_wtc_sent(struct slsi_dev *sdev, struct net_device *dev,
				   u8 reason_code)
{
	int pos = 0;
	char *log_buffer = NULL;
	...
	get_kernel_timestamp(time);
	pos += scnprintf(log_buffer + pos, buf_size - pos, "[%d.%d][BTM] WTC reason_code=%d",
			 time[0], time[1], reason_code);
...
void slsi_conn_log2us_deauth(struct slsi_dev *sdev, struct net_device *dev, char *str_type,
			     const unsigned char *bssid, int sn, int status, char *vs_ie, short rssi)
{
}
