...
static bool tx_bot_mvif_rules_enable = true;
module_param(tx_bot_mvif_rules_enable, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_mvif_rules_enable, "enable new BoT rules for Multi VIF modes");

static bool tx_bot_ac_cod_manual = false;
module_param(tx_bot_ac_cod_manual, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_ac_cod_manual, "set AC COD values to magic values");

static int tx_bot_rule1_num_vifs = 2;
module_param(tx_bot_rule1_num_vifs, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule1_num_vifs, "set number of VIFs for BoT Rule 1");

static int tx_bot_rule1_gmod = 3200;
module_param(tx_bot_rule1_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule1_gmod, "set global MOD for BoT Rule 1");

static int tx_bot_rule2_gmod = 2816;
module_param(tx_bot_rule2_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule2_gmod, "set global MOD for BoT Rule 2");

static int tx_bot_rule2_mhs_mod = (128 * 2);
module_param(tx_bot_rule2_mhs_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule2_mhs_mod, "set MHS MOD for BoT Rule 2");

static int tx_bot_rule3_gmod = 2816;
module_param(tx_bot_rule3_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule3_gmod, "set global MOD for BoT Rule 3");

static int tx_bot_rule3_sta_mod = 128;
module_param(tx_bot_rule3_sta_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule3_sta_mod, "set STA MOD for BoT Rule 3");

static int tx_bot_rule3_mhs_mod = 128;
module_param(tx_bot_rule3_mhs_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule3_mhs_mod, "set MHS MOD for BoT Rule 3");

static int tx_bot_rule5_gmod = 2816;
module_param(tx_bot_rule5_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule5_gmod, "set global MOD for BoT Rule 5");

static int tx_bot_rule5_sta_mod = 256;
module_param(tx_bot_rule5_sta_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule5_sta_mod, "set STA MOD for BoT Rule 5");

static int tx_bot_rule5_p2p_mod = 1024;
module_param(tx_bot_rule5_p2p_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule5_p2p_mod, "set P2P MOD for BoT Rule 5");

static int tx_bot_rule6_gmod = 3200;
module_param(tx_bot_rule6_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule6_gmod, "set global MOD for BoT Rule 6");

static int tx_bot_rule6_sta_mod = 1280;
module_param(tx_bot_rule6_sta_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule6_sta_mod, "set STA MOD for BoT Rule 6");

static int tx_bot_rule6_p2p_mod = 1024;
module_param(tx_bot_rule6_p2p_mod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule6_p2p_mod, "set P2P MOD for BoT Rule 6");

static int tx_bot_rule7_gmod = 3200;
module_param(tx_bot_rule7_gmod, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_rule7_gmod, "set global MOD for BoT Rule 7");

static uint tx_bot_vif_idle_timeout_in_ms = 1000; /* 1 sec */
module_param(tx_bot_vif_idle_timeout_in_ms, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tx_bot_vif_idle_timeout_in_ms, "Idle Timeout (in ms) for redistributions of MOD");
...
static __always_inline int slsi_txbp_has_resource(struct tx_netdev_data *tx_priv)
{
	...
	/* Global domain */
	mod = txbp_priv.bp_mod - SLSI_TX_BP_MBULK_GUARD;
	cod = txbp_priv.cod;
...
static __always_inline u32 slsi_txbp_get_dynamic_netdev_mod(struct tx_netdev_data *tx_priv)
{
	struct tx_netdev_data *tx_data = NULL;
	u32 ndev_mod = tx_priv->netdev_mod;
	u32 free_slots = 0;

	/* When an interface1 is active AND another interface2 used 0 BDs (out of its Y budget)
	 * in the last N (default 1) seconds, allow X to grow to (X + Y * 3/4).
	 * When the interface2 becomes non-idle, i.e. starts using its own budget,
	 * (X + Y * 3/4) for interface shrinks (naturally as BDs get returned) back to X.
	 */
	if (tx_bot_mvif_rules_enable && txbp_priv.vif_cnt > tx_bot_rule1_num_vifs) {
		list_for_each_entry(tx_data, &txbp_priv.vif_list, list) {
			/* is this VIF idle */
			if (jiffies_to_msecs(jiffies - tx_data->last_presence_check_time) > (tx_bot_vif_idle_timeout_in_ms)) {
				free_slots += ((tx_data->netdev_mod * 3) / 4);
			}
		}
		ndev_mod += free_slots;
	}
...
static __always_inline u32 slsi_txbp_get_max_qlen(struct tx_netdev_data *tx_priv, u8 ac)
{
	...
	ndev_mod = slsi_txbp_get_dynamic_netdev_mod(tx_priv);
...
static __always_inline int slsi_txbp_budget_distribution(struct tx_struct *tx, int available_budget, int *budget)
{
	...
	for (ac = SLSI_TRAFFIC_Q_BE; ac < AC_CATEGORIES; ac++) {
		if (tx_bot_mvif_rules_enable && tx_bot_ac_cod_manual) {
			switch (ac) {
				...
				case SLSI_TRAFFIC_Q_VI:
					...
					break;
				default:
					SLSI_NET_WARN(tx->ndev, "BP: unknown ac %d\n", ac);
					break;
			}
		} else {
...
static int slsi_txbp_napi(struct napi_struct *napi, int budget)
{
	...
	atomic_set(&tx_priv->pkt_remained, 0);
	...
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
		...
		while (ac_budget_limit[ac]) {
			...
			if (!skb) {
				SLSI_NET_ERR(tx->ndev, "skb is NULL ac%d queue is empty\n", ac);
				break;
			}
			...
			if (txbp_priv.bp_mod - cod < SLSI_TX_BP_MBULK_GUARD)
				goto tx_done_check_q;
		}
	}
tx_done_check_q:
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
		...
		if (work_done == 0 && __netif_subqueue_stopped(tx->ndev, ac) && !ndev_vif->softap_suspend_mode) {
#else
		slsi_txbp_check_q_status(tx->ndev, tx_priv, ac);
		if (work_done == 0 && __netif_subqueue_stopped(tx->ndev, ac)) {
#endif
			if (slsi_txbp_has_resource(tx_priv)) {
				/*
				* we repoll Tx napi if there is single queue in paused state.
				*/
				work_done = SLSI_NAPI_FORCE_REPOLL;
			} else {
...
tx_done:
	if (work_done)
		...
		if (tx_priv && atomic_read(&tx_priv->pkt_remained)) {
			/* packet arrived from network stack while napi polling.
			 * make 'work done' value to non-zero to make it repoll.
			 * Then this packet could be processed without waiting for the next packet to arrive.
			 */
			work_done = SLSI_NAPI_FORCE_REPOLL;
		} else {
#ifdef CONFIG_SCSC_WLAN_LOAD_BALANCE_MANAGER
...
__always_inline int count_setbits_u16(u16 n)
{
...
__always_inline void slsi_txbp_vif_budget_redistribute(struct slsi_dev *sdev, struct net_device *dev, bool is_activated)
{
	struct tx_netdev_data *tx_priv = NULL;
	int mod;
	...
	if (tx_bot_mvif_rules_enable && (txbp_priv.vif_cnt == 1)) {
		/* Rule 7:
		 * When more than 4 NDP peers are connected,
		 * limit total outstanding BDs to 5*(512+128) = 3200.
		 */
#ifdef CONFIG_SCSC_WIFI_NAN_ENABLE
		struct net_device *nmi_dev;
		struct netdev_vif *nmi_ndev_vif;
		u16 ndp_count = 0;

		nmi_dev = sdev->netdev[SLSI_NET_INDEX_NAN];
		if (nmi_dev) {
			nmi_ndev_vif = netdev_priv(nmi_dev);
			if (nmi_ndev_vif->activated) {
				ndp_count = count_setbits_u16(nmi_ndev_vif->nan.ndp_active_id_map);
				if (ndp_count > 4) {
					SLSI_INFO(sdev, "BoT: Rule 7: NDP peers(%d) > 4: limit GMOD to %d\n", ndp_count, tx_bot_rule7_gmod);
					txbp_priv.bp_mod = tx_bot_rule7_gmod;
					list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
						tx_priv->netdev_mod = (tx_bot_rule7_gmod / txbp_priv.vif_cnt);
					}
					return;
				}
			}
	...
	if (tx_bot_mvif_rules_enable && txbp_priv.vif_cnt > tx_bot_rule1_num_vifs) {
		bool is_p2p_active = false;
		bool is_mhs_active = false;
		bool is_nan_active = false;
		bool is_sta_24g = false;
		bool is_sta_5g = false;

		/* Rule 1:
		 * Limit number of outstanding BD to (tx_bot_rule1_gmod)
		 * when multiple VIFs are active
		 */
		SLSI_INFO(sdev, "BoT: Rule 1: num_vifs > %d: limit GMOD to %d\n", tx_bot_rule1_num_vifs, tx_bot_rule1_gmod);
		txbp_priv.bp_mod = tx_bot_rule1_gmod;
		list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
			tx_priv->netdev_mod = (tx_bot_rule1_gmod / txbp_priv.vif_cnt);
		}

		/* Rule 2:
		 * In MHS + NAN, limit number of outstanding BDs for MHS to 2*128
		 * (not 5 MSDUs per A-MSDU), as the MHS operates in 2.4GHz
		 *
		 * Rule 3:
		 * In STA + MHS + NAN, when STA operates in 2.4GHz,
		 * limit number of outstanding BDs for STA to 2*128
		 * (not 5 MSDUs per A-MSDU, as the STA operates in 2.4GHz)
		 *
		 * Rule 5:
		 * In STA + P2P + NAN, if the STA channel is in 2.4G,
		 * limit:STA = 512, P2P = 480, NDP = 1568 (all BDs)
		 *
		 * Rule 6:
		 * In STA + P2P + NAN, if the STA channel is in 5/6G,
		 * limit:STA = 1280, P2P = 480, NDP = 800 (all BDs)
		 */
		list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
			struct net_device *dev = tx_priv->tx.ndev;
			struct netdev_vif *ndev_vif = netdev_priv(dev);

			if (SLSI_IS_VIF_INDEX_WLAN(ndev_vif)) {
				if(!SLSI_IS_VIF_CHANNEL_5G(ndev_vif)) {
					SLSI_INFO(sdev, "BoT: detected STA (2.4G)\n");
					is_sta_24g = true;
				} else {
					SLSI_INFO(sdev, "BoT: detected STA 5G/6G\n");
					is_sta_5g = true;
				}
			} else if (SLSI_IS_VIF_INDEX_MHS(sdev, ndev_vif)) {
				SLSI_INFO(sdev, "BoT: detected MHS\n");
				is_mhs_active = true;
			} else if ((ndev_vif->iftype == NL80211_IFTYPE_P2P_CLIENT) || (ndev_vif->iftype == NL80211_IFTYPE_P2P_GO)) {
				SLSI_INFO(sdev, "BoT: detected P2P CLI/GO\n");
				is_p2p_active = true;
			} else if (ndev_vif->vif_type == SLSI_NAN_VIF_TYPE_NDP) {
				SLSI_INFO(sdev, "BoT: detected NDP\n");
				is_nan_active = true;
			}
		}
		if (is_sta_24g && is_mhs_active && is_nan_active) {
			SLSI_INFO(sdev, "BoT: Rule 3: STA (2.4G) + MHS + NAN: limit GMOD to %d\n", tx_bot_rule3_gmod);
			txbp_priv.bp_mod = tx_bot_rule3_gmod;
			mod = ((tx_bot_rule3_gmod) - (tx_bot_rule3_sta_mod + tx_bot_rule3_mhs_mod));
			list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
				struct net_device *dev = tx_priv->tx.ndev;
				struct netdev_vif *ndev_vif = netdev_priv(dev);

				if (SLSI_IS_VIF_INDEX_WLAN(ndev_vif)) {
					tx_priv->netdev_mod = tx_bot_rule3_sta_mod;
					SLSI_NET_INFO(dev, "BoT: STA MOD %d\n", tx_priv->netdev_mod);
				} else if (SLSI_IS_VIF_INDEX_MHS(sdev, ndev_vif)) {
					tx_priv->netdev_mod = tx_bot_rule3_mhs_mod;
					SLSI_NET_INFO(dev, "BoT: MHS MOD %d\n", tx_priv->netdev_mod);
				} else {
					tx_priv->netdev_mod = (mod / (txbp_priv.vif_cnt - 2));
					SLSI_NET_INFO(dev, "BoT: MOD %d\n", tx_priv->netdev_mod);
				}
			}
		} else if (is_sta_24g && is_p2p_active && is_nan_active) {
			SLSI_INFO(sdev, "BoT: Rule 5: STA (2.4G) + P2P + NAN: limit GMOD to %d\n", tx_bot_rule5_gmod);
			txbp_priv.bp_mod = tx_bot_rule5_gmod;
			mod = (tx_bot_rule5_gmod) - (tx_bot_rule5_sta_mod + tx_bot_rule5_p2p_mod);
			list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
				struct net_device *dev = tx_priv->tx.ndev;
				struct netdev_vif *ndev_vif = netdev_priv(dev);

				if (SLSI_IS_VIF_INDEX_WLAN(ndev_vif)) {
					tx_priv->netdev_mod = tx_bot_rule5_sta_mod;
					SLSI_NET_INFO(dev, "BoT: STA MOD %d\n", tx_priv->netdev_mod);
				} else if ((ndev_vif->iftype == NL80211_IFTYPE_P2P_CLIENT) || (ndev_vif->iftype == NL80211_IFTYPE_P2P_GO)) {
					tx_priv->netdev_mod = tx_bot_rule5_p2p_mod;
					SLSI_NET_INFO(dev, "BoT: P2P MOD %d\n", tx_priv->netdev_mod);
				} else {
					tx_priv->netdev_mod = (mod / (txbp_priv.vif_cnt - 2));
					SLSI_NET_INFO(dev, "BoT: MOD %d\n", tx_priv->netdev_mod);
				}
			}
		} else if (is_sta_5g && is_p2p_active && is_nan_active) {
			SLSI_INFO(sdev, "BoT: Rule 6: STA (5G) + P2P + NAN: limit GMOD to %d\n", tx_bot_rule6_gmod);
			txbp_priv.bp_mod = tx_bot_rule6_gmod;
			mod = (tx_bot_rule6_gmod) - (tx_bot_rule6_sta_mod + tx_bot_rule6_p2p_mod);
			list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
				struct net_device *dev = tx_priv->tx.ndev;
				struct netdev_vif *ndev_vif = netdev_priv(dev);

				if (SLSI_IS_VIF_INDEX_WLAN(ndev_vif)) {
					tx_priv->netdev_mod = tx_bot_rule6_sta_mod;
					SLSI_NET_INFO(dev, "BoT: STA MOD %d\n", tx_priv->netdev_mod);
				} else if ((ndev_vif->iftype == NL80211_IFTYPE_P2P_CLIENT) || (ndev_vif->iftype == NL80211_IFTYPE_P2P_GO)) {
					tx_priv->netdev_mod = tx_bot_rule6_p2p_mod;
					SLSI_NET_INFO(dev, "BoT: P2P MOD %d\n", tx_priv->netdev_mod);
				} else {
					tx_priv->netdev_mod = mod / (txbp_priv.vif_cnt - 2);
					SLSI_NET_INFO(dev, "BoT: MOD %d\n", tx_priv->netdev_mod);
		...
		} else if (is_mhs_active && is_nan_active) {
			SLSI_INFO(sdev, "BoT: Rule 2: MHS + NAN: limit GMOD to %d\n", tx_bot_rule2_gmod);
			txbp_priv.bp_mod = tx_bot_rule2_gmod;
			mod = (tx_bot_rule2_gmod) - (tx_bot_rule2_mhs_mod);
			list_for_each_entry(tx_priv, &txbp_priv.vif_list, list) {
				struct net_device *dev = tx_priv->tx.ndev;
				struct netdev_vif *ndev_vif = netdev_priv(dev);

				if (SLSI_IS_VIF_INDEX_MHS(sdev, ndev_vif)) {
					tx_priv->netdev_mod = tx_bot_rule2_mhs_mod;
					SLSI_NET_INFO(dev, "BoT: MHS MOD %d\n", tx_priv->netdev_mod);
				} else {
					tx_priv->netdev_mod = (mod / (txbp_priv.vif_cnt - 1));
					SLSI_NET_INFO(dev, "BoT: MOD %d\n", tx_priv->netdev_mod);
				}
			}
		}
	} else {
		SLSI_INFO(sdev, "BoT: default: MOD %d, vif_cnt:%d\n", bp_mod, txbp_priv.vif_cnt);
		txbp_priv.bp_mod = bp_mod;
					...
					struct slsi_peer *peer)
{
	...
	/* redistribute budget on peer addition as well */
	slsi_txbp_vif_budget_redistribute(sdev, dev, true);
	return true;
}
...
__always_inline bool slsi_vif_activated_post(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	if (tx_priv->bh_tx) {
		...
		slsi_lbm_register_io_saturation_control(tx_priv->bh_tx);
	...
	if (tx_bot_mvif_rules_enable && ndev_vif->vif_type == FAPI_VIFTYPE_STATION) {
		/* Rule 4:
		 * BA-disabled TIDs adjust per AC COD
		 *
		 * read the unifiBaTxEnableTid MIB.
		 * If the policy for AC_VI or/and AC_VO has been
		 * set to disabled (0x00b), then adjust the
		 * per AC COD to limi outstanding data on non-BA streams
		 */
		u8 allowed = 1;
		u32 ba_allowed = slsi_mib_get_tx_ba_allowed(sdev, dev);
		SLSI_NET_INFO(dev, "MIB: unifiBaTxEnableTid: 0x%X\n", ba_allowed);

		/* read VO AC */
		allowed = (ba_allowed >> 8) & 1;
		allowed = (ba_allowed >> 10) & 1;
		allowed = (ba_allowed >> 12) & 1;
		allowed = (ba_allowed >> 14) & 1;

		if (!allowed) {
			SLSI_NET_INFO(dev, "BOT: Rule 4: BA not allowed on VI and VO -> limit MOD\n");
			tx_bot_ac_cod_manual = true;
		}
	}
	return true;
}
					       ...
					       struct netdev_vif *ndev_vif)
{
	...
	slsi_txbp_vif_budget_redistribute(sdev, dev, false);
	write_unlock_bh(&txbp_priv.vif_lock);
...
static int slsi_txbp_immediate_fwd(struct net_device *dev, struct tx_netdev_data *tx_priv)
{
	struct netdev_vif     *ndev_vif = netdev_priv(dev);
	struct slsi_dev       *sdev = ndev_vif->sdev;
	struct netdev_vif     *ndev_vif_peer;
	struct tx_struct      *tx = NULL;
	int ac;
	int ac_budget_limit[AC_CATEGORIES] = {0};
	int ac_service[AC_CATEGORIES] = {0};
	int work_done = 0;
	...
	if (!slsi_txbp_budget_distribution(tx, slsi_txbp_has_resource(tx_priv), (int *)ac_budget_limit))
		goto tx_done_check_q;
	...
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
		...
		while (ac_budget_limit[ac]) {
			...
			if (!skb) {
				SLSI_NET_ERR(dev, "skb is NULL ac%d queue is empty\n", ac);
				break;
			}
			...
			if (ret != NETDEV_TX_OK) {
				...
				show_cod(tx_priv);
				if (bp_log & LOG_FILTER_TX_FAILURE)
					SLSI_NET_ERR(dev, "BP: Tx fail remain budget %u\n", ac_budget_limit[ac]);
...
tx_done_check_q:
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
#ifdef CONFIG_SCSC_WLAN_SAP_POWER_SAVE
		slsi_txbp_check_q_status(dev, tx_priv, ac, ndev_vif->softap_suspend_mode);
		if (work_done == 0 && __netif_subqueue_stopped(dev, ac) && !ndev_vif->softap_suspend_mode) {
#else
		slsi_txbp_check_q_status(dev, tx_priv, ac);
		if (work_done == 0 && __netif_subqueue_stopped(dev, ac)) {
#endif
			if (tx_priv) {
				SLSI_NET_ERR(dev, "BP: one of the netif subqueues is stopped but no resource!\n");
				show_cod(tx_priv);
			}
		}
	}
...
#ifdef CONFIG_SCSC_WLAN_LOAD_BALANCE_MANAGER
	if (tx_priv->bh_tx && tx_priv->bh_tx->cpu_affinity->state > TRAFFIC_MON_CLIENT_STATE_LOW) {
		if (!test_bit(SLSI_TX_LBM_RUNNING, &tx_info->lbm_bh_state)) {
			set_bit(SLSI_TX_LBM_RUNNING, &tx_info->lbm_bh_state);
			local_bh_disable();
		...
		if (test_bit(SLSI_TX_LBM_RUNNING, &tx_info->lbm_bh_state))
			SLSI_DBG3_NODEV(SLSI_TX, "TX napi is running.\n");
		slsi_txbp_immediate_fwd(dev, tx_priv);
	}
#else
	sdev = slsi_get_sdev();
...
static __always_inline void slsi_txbp_enqueue(struct net_device *dev, struct netdev_vif *ndev_vif, struct sk_buff *skb)
{
	...
	slsi_skb_cb_get(skb)->enqueued_time_ms = jiffies_to_msecs(jiffies);
	...
	if (!skb->xmit_more) {
		...
		curr_state = bh->cpu_affinity->state;
		if (curr_state >= state) {
			if (cpu == bh->cpu_affinity->cpu[curr_state]) {
				if (cpu == tx_priv->dedicated_napi) {
#ifdef CONFIG_SCSC_WLAN_CPUHP_MONITOR
					cpumask_clear(&except_cpus);
					cpumask_set_cpu(cpu, &except_cpus);
					proper = slsi_find_proper_cpu(bh, curr_state, NP_TX_0,
				...
				slsi_lbm_state_change_for_affinity(bh, curr_state, target);
			}
		}
#else
...
#endif
		atomic_set(&tx_priv->pkt_remained, 1);
