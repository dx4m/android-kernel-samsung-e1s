...
static void slsi_update_ml_ap_detail(struct slsi_dev *sdev, struct net_device *dev, const u8 *bssid)
{
	struct slsi_ssid_info *ssid_info;
	struct slsi_bssid_info *bssid_info, *tmp_info;
	struct slsi_mlo_ap_details *mlo_ap_details;
	u8 num_links = 0, bands = 0;

	if (!bssid) {
		SLSI_ERR(sdev, "Cannot update AP detail when bssid is NULL\n");
		return;
	}
	...
	bssid_info = slsi_get_bssid_info_from_bssid(sdev, dev, &ssid_info->bssid_list, bssid);
	if (!bssid_info) {
		SLSI_DBG1(sdev, SLSI_MLME, "Failed to get bssid_info from list\n");
		return;
	}
	/* MLO AP anyway will be handled in slsi_fill_current_ap_detail() if NULL */
	if (!bssid_info->mlo_ap)
		return;

	mlo_ap_details = bssid_info->mlo_ap;
	list_for_each_entry(tmp_info, &ssid_info->bssid_list, list) {
		if (!SLSI_ETHER_EQUAL(tmp_info->mld_addr, bssid_info->mld_addr))
			continue;
		num_links++;
		bands |= slsi_freq_to_band(tmp_info->freq / 2);
		tmp_info->mlo_ap = mlo_ap_details;
	}
	mlo_ap_details->num_links = num_links;
	mlo_ap_details->bands = bands;
...
continue_next_tbtt:
	...
	if (n_channels) {
		...
		if (!ndev_vif->sta.drv_connect_req_ongoing)
			slsi_populate_ssid_info(sdev, dev, SLSI_SCAN_HW_ID);
		...
		if (!ndev_vif->sta.drv_connect_req_ongoing)
			slsi_update_ml_ap_detail(sdev, dev, sme->bssid ? sme->bssid : sme->bssid_hint);
	}
}
#endif
...
bool slsi_is_akm_downgrade_allowed(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	for (i = 0; i < sme->crypto.n_akm_suites; i++) {
		if (sme->crypto.akm_suites[i] == WLAN_AKM_SUITE_SAE)
			...
			SLSI_NET_DBG1(dev, SLSI_CFG80211, "Found SAE and PSK in akm_suites. Downgrade AKM is allowed\n");
			return true;
		}
	}
...
static u32 slsi_ml_conn_usr_link_selection(struct net_device *dev, u8 band, u32 links, struct list_head *bssid_list)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	u8 num_links = ndev_vif->sta.max_ml_link;
	struct slsi_bssid_info *bssid_info;
	u8 *usr_bssid;
	u32 i = 0;
	u32 link_band = 0;

	if (is_zero_ether_addr(ndev_vif->sta.sec_ap_link_addr[0]))
		return links;

	link_band |= band;
	for (i = 0; i < MAX_SUPP_MLO_LINKS; i++) {
		usr_bssid = ndev_vif->sta.sec_ap_link_addr[i];
		if (is_zero_ether_addr(ndev_vif->sta.sec_ap_link_addr[i]))
			break;

		list_for_each_entry(bssid_info, bssid_list, list) {
			if (!SLSI_ETHER_EQUAL(bssid_info->bssid, usr_bssid) || !bssid_info->mlo_ap)
				continue;
			link_band |= slsi_freq_to_band(bssid_info->freq / 2);
		}
	}
	...
	switch(link_band) {
		...
		case SLSI_FREQ_BAND_2_4GHZ_5GHZ_6GHZ:
			/* As SLSI_FREQ_BAND_2_4GHZ_5GHZ_6GHZ is 3, it signifies 2GHz plus 5GHz */
		...
		case 7:
			/* All three links are set */
			if (num_links < 3) {
				SLSI_ERR_NODEV("device donot support 3 links %d\n", num_links);
				return links;
			} else {
		 ...
		 struct cfg80211_connect_params *sme)
{
	...
	if (ndev_vif->sta.btm_disabled) {
		slsi_mlme_set_roaming_parameters(sdev, dev, SLSI_PSID_UNIFI_ROAM_BTM_ACTIVATED, 0, 1);
		ndev_vif->sta.btm_disabled = 0;
	}

#ifdef CONFIG_SCSC_WLAN_EHT
	slsi_dump_ml_bssid_details(sdev, dev);
	if (ndev_vif->sta.sta_bss &&
	    cfg80211_find_ext_elem(WLAN_EID_EXT_EHT_MULTI_LINK, ndev_vif->sta.sta_bss->ies->data,
			...
			if (bssid_info) {
				if (!bssid_info->mlo_ap) {
					...
					if (r < 0 || !bssid_info->mlo_ap) {
						SLSI_DBG1(sdev, SLSI_CFG80211, "Failed to get MLO AP details\n");
						goto exit_with_vif;
					}
				}
				links = slsi_ml_connection_link_selection(dev, slsi_freq_to_band(center_freq),
									  bssid_info, bssid_info->mlo_ap);
				links = slsi_ml_conn_usr_link_selection(dev, slsi_freq_to_band(center_freq), links,
				...
				SLSI_WARN(sdev, "Failed to find bssid_info\n");
			}
		} else {
			SLSI_WARN(sdev, "Failed to find matching ssid_info\n");
		}
...
int slsi_set_monitor_channel(struct wiphy *wiphy, struct cfg80211_chan_def *chandef)
{
	...
	if ((monitor_vif_set != 1 && monitor_vif_set != 2) ||
	    (sdev->monitor_vif_num != 1 && sdev->monitor_vif_num != 2) ||
	    monitor_vif_set > sdev->monitor_vif_num) {
		SLSI_ERR(sdev, "Invalid monitor_vif_set : %d, monitor_vif_num : %d\n",
			 monitor_vif_set, sdev->monitor_vif_num);
		return -EINVAL;
	}
		   ...
		   struct cfg80211_pmksa *pmksa)
{
	...
	ndev_vif->sta.pmkid = kmalloc(PMKID_LEN, GFP_KERNEL);
	if (!ndev_vif->sta.pmkid) {
		SLSI_NET_ERR(dev, "No mem for set_pmksa\n");
		ret = -ENOMEM;
		goto exit;
	}
	memcpy(ndev_vif->sta.pmkid, pmksa->pmkid, PMKID_LEN);
	peer = ndev_vif->peer_sta_record[SLSI_STA_PEER_QUEUESET]; /* connected AP */
	if (peer && peer->valid && SLSI_ETHER_EQUAL(pmksa->bssid, peer->address) && peer->assoc_ie)
		ret = slsi_configure_pmksa(sdev, dev, peer);
		  ...
		  struct cfg80211_ap_settings *settings)
{
	...
	SLSI_NET_INFO(dev, "cfg80211_ap_settings: band %d, center_freq %d, width %d, link_id %d\n",
		      settings->chandef.chan->band, settings->chandef.chan->center_freq,
		      settings->chandef.width, link_id);
	...
	cfg80211_ch_switch_notify(dev, &settings->chandef, (unsigned int)link_id);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 94))
	cfg80211_ch_switch_notify(dev, &settings->chandef, (unsigned int)link_id, 0);
	...
	SLSI_NET_INFO(dev, "Soft Ap started on frequency: %d MHz\n", settings->chandef.chan->center_freq);
	if (ndev_vif->iftype == NL80211_IFTYPE_P2P_GO)
...
#if (defined(CONFIG_SLSI_WLAN_CP_COEX) && defined(SCSC_SEP_VERSION))
			       ...
			       struct cfg80211_external_auth_params *params)
{
	...
	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
#if !(defined(SCSC_SEP_VERSION) && SCSC_SEP_VERSION < 11)
	if (ndev_vif->sta.wpa3_sae_reconnection && ndev_vif->sta.wpa3_external_auth_abort) {
		SLSI_NET_ERR(dev, "Dropping synchronised_resp for bssid:" MACSTR " with status code: %d\n",
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0))
static const u32 slsi_akm_suites[]= {
	...
	SLSI_RSN_AUTH_KEY_MGMT_PASN,
	SLSI_RSN_AUTH_KEY_MGMT_FT_802_1X_SHA384_UNRESTRICTED,
	SLSI_RSN_AUTH_KEY_MGMT_802_1X_SHA384,
	SLSI_WLAN_AKM_SUITE_SAE_EXT_KEY,
	SLSI_WLAN_AKM_SUITE_FT_SAE_EXT_KEY

};

static const struct wiphy_iftype_akm_suites iftype_akm_suites[] = {
	{
		.iftypes_mask = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_AP),
		.akm_suites = slsi_akm_suites,
		.n_akm_suites = ARRAY_SIZE(slsi_akm_suites),
	},
};
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0))
	wiphy->n_akm_suites = ARRAY_SIZE(slsi_akm_suites);
	wiphy->akm_suites = slsi_akm_suites;
	wiphy->iftype_akm_suites = iftype_akm_suites;
	wiphy->num_iftype_akm_suites = ARRAY_SIZE(iftype_akm_suites);
#endif
...
static bool slsi_update_wiphy_6ghz(struct slsi_dev *sdev)
{
	...
	if (sdev->device_config.supported_band == SLSI_FREQ_BAND_2GHZ)
		sdev->device_config.supported_band = SLSI_FREQ_BAND_2_4GHZ_6GHZ;
	else
		sdev->device_config.supported_band = SLSI_FREQ_BAND_2_4GHZ_5GHZ_6GHZ;
