...
static u32 slsi_mlme_nan_append_pairing_config(struct sk_buff *req, u32 pairing_setup, u32 pairing_cache,
					       u32 pairing_verification, u16 supported_bootstrapping_methods,
					       struct slsi_nan_security_info *sec_info)
{
	u8 *p;

	p = fapi_append_data_u16(req, SLSI_NAN_TLV_NAN_PAIRING_CONFIG);
	p = fapi_append_data_u16(req, 0x0009);
	p = fapi_append_data_bool(req, pairing_setup);
	p = fapi_append_data_bool(req, pairing_cache);
	p = fapi_append_data_bool(req, pairing_verification);
	p = fapi_append_data_u16(req, supported_bootstrapping_methods);
	p = fapi_append_data_u8(req, sec_info->cipher_type);
	if (p)
		return 0;
	return 1;
...
static u32 slsi_mlme_nan_append_identity_key(struct sk_buff *req, u8 *nan_identity_key)
{
	u8 *p;

	p = fapi_append_data_u16(req, SLSI_NAN_TLV_NAN_IDENTITY_KEY);
	p = fapi_append_data_u16(req, 0x0012);
	p = fapi_append_data_bool(req, true);
	p = fapi_append_data(req, nan_identity_key, SLSI_HAL_NAN_IDENTITY_KEY_LEN);
	if (p)
		return 0;
	return 1;
}

static u32 slsi_mlme_nan_append_bootstrapping_comeback_info(struct sk_buff *req,
							    u32 comeback_delay,
							    u8 *data, u16 cookie_length)
{
	u8 *p;

	p = fapi_append_data_u16(req, SLSI_NAN_TLV_NAN_BOOTSTRAPPING_COMEBACK_INFO);
	p = fapi_append_data_u16(req, sizeof(comeback_delay) + sizeof(cookie_length) + cookie_length);
	p = fapi_append_data_u32(req, comeback_delay);
	p = fapi_append_data_u16(req, cookie_length);
	p = fapi_append_data(req, data, cookie_length);
	if (p)
		return 0;
	return 1;
}
...
int slsi_mlme_nan_enable(struct slsi_dev *sdev, struct net_device *dev, struct slsi_hal_nan_enable_req *hal_req)
{
	...
	req = fapi_alloc(mlme_nan_start_req, MLME_NAN_START_REQ, ndev_vif->vifnum, 64);
	if (!req) {
...
#ifdef CONFIG_SCSC_WIFI_NAN_PAIRING
	if (hal_req->pairing_cfg.enable_pairing_setup || hal_req->pairing_cfg.enable_pairing_verification) {
		ret = slsi_mlme_nan_append_pairing_config(req, hal_req->pairing_cfg.enable_pairing_setup,
							  hal_req->pairing_cfg.enable_pairing_cache,
							  hal_req->pairing_cfg.enable_pairing_verification,
							  hal_req->pairing_cfg.supported_bootstrapping_methods,
							  &hal_req->sec_info);
		if (ret) {
			SLSI_WARN_NODEV("Error append pairing config TLV\n");
			return ret;
		}
	}

	if (hal_req->pairing_cfg.enable_pairing_cache) {
		ret = slsi_mlme_nan_append_identity_key(req, hal_req->nan_identity_key);
		if (ret)
			SLSI_WARN_NODEV("Error append identity key TLV\n");
	}
#endif
	return ret;
}
			  ...
			  u16 publish_id)
{
	...
	if (hal_req) {
#ifdef CONFIG_SCSC_WIFI_NAN_PAIRING
		/* discovery_config_tlv, nan_sec_pmk_tlv, ranging_cfg_tlv, pairing_cfg_tlv, identity_key */
		u16 length = 18 + 70 + 11 + 13 + 22;
#else
		/* discovery_config_tlv, nan_sec_pmk_tlv, ranging_cfg_tlv */
		u16 length = 18 + 70 + 11;
#endif
		...
		req = fapi_alloc(mlme_nan_publish_req, MLME_NAN_PUBLISH_REQ, ndev_vif->vifnum, length);
		if (!req) {
			SLSI_NET_ERR(dev, "fapi alloc failure\n");
			return -ENOMEM;
		}
		...
		req = fapi_alloc(mlme_nan_publish_req, MLME_NAN_PUBLISH_REQ, ndev_vif->vifnum, 0);
		if (!req) {
			SLSI_NET_ERR(dev, "fapi alloc failure\n");
			return -ENOMEM;
		}
			    ...
			    u16 subscribe_id)
{
	...
	if (hal_req) {
		...
		req = fapi_alloc(mlme_nan_subscribe_req, MLME_NAN_SUBSCRIBE_REQ, ndev_vif->vifnum, length);
		if (!req) {
			SLSI_NET_ERR(dev, "fapi alloc failure\n");
			return -ENOMEM;
		}
		...
		req = fapi_alloc(mlme_nan_subscribe_req, MLME_NAN_SUBSCRIBE_REQ, ndev_vif->vifnum, 0);
		if (!req) {
			SLSI_NET_ERR(dev, "fapi alloc failure\n");
			return -ENOMEM;
		}
			      ...
			      struct slsi_hal_nan_transmit_followup_req *hal_req)
{
	...
	req = fapi_alloc(mlme_nan_followup_req, MLME_NAN_FOLLOWUP_REQ, ndev_vif->vifnum, 5 * 255);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure.\n");
		return -ENOMEM;
	}
	...
	fapi_set_high16_u32(req, u.mlme_nan_followup_req.spare_1, hal_req->transaction_id);
	fapi_set_low16_u32(req, u.mlme_nan_followup_req.spare_1, hal_req->shared_key_desc_flag);
...
#ifdef CONFIG_SCSC_WIFI_NAN_PAIRING
static u32 slsi_mlme_nan_bootstrapping_request_fapi_data(struct netdev_vif *ndev_vif, struct sk_buff *req,
							 struct slsi_hal_nan_bootstrapping_req *hal_req)
{
	u32 ret;

	ret = slsi_mlme_nan_append_service_specific_info(req, ndev_vif->nan.service_specific_info_len,
							 ndev_vif->nan.service_specific_info);
	if (ret) {
		SLSI_WARN_NODEV("Error append service specific info TLV\n");
		return ret;
	}
	...
	if (hal_req->comebackinfo.comeback_delay) {
		ret = slsi_mlme_nan_append_bootstrapping_comeback_info(req,
								       hal_req->comebackinfo.comeback_delay,
								       hal_req->comebackinfo.cookie,
								       hal_req->comebackinfo.cookie_length);
		if (ret)
			SLSI_WARN_NODEV("Error append bootstrapping come back info TLV\n");
	}
	return ret;
}

int slsi_mlme_nan_bootstrapping_request(struct slsi_dev *sdev, struct net_device *dev,
					struct slsi_hal_nan_bootstrapping_req *hal_req)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	struct sk_buff    *cfm;
	int               r = 0;
	u16               bootstrapping_instance_id;

	SLSI_NET_DBG3(dev, SLSI_MLME, "\n");

	if (!hal_req) {
		SLSI_NET_ERR(dev, "hal_req is NULL\n");
		return -EINVAL;
	}

	req = fapi_alloc(mlme_nan_bootstrapping_request_req, MLME_NAN_BOOTSTRAPPING_REQUEST_REQ,
			 ndev_vif->vifnum, 64);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_bootstrapping_request_req.peer_nmi_address, hal_req->addr);
	fapi_set_u16(req, u.mlme_nan_bootstrapping_request_req.match_id,
		     hal_req->requestor_instance_id);
	fapi_set_u16(req, u.mlme_nan_bootstrapping_request_req.bootstrapping_instance_id,
		     hal_req->bootstrapping_instance_id);
	fapi_set_u16(req, u.mlme_nan_bootstrapping_request_req.bootstrapping_method,
		     hal_req->bootstrapping_method);

	r = slsi_mlme_nan_bootstrapping_request_fapi_data(ndev_vif, req, hal_req);
	if (r) {
		SLSI_NET_ERR(dev, "Failed to construct mbulkdata\n");
		kfree_skb(req);
		return -EINVAL;
	...
	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_BOOTSTRAPPING_REQUEST_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_nan_bootstrapping_request_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "MLME_NAN_BOOTSTRAPPING_REQUEST_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_bootstrapping_request_cfm.result_code));
		r = -EINVAL;
	}

	bootstrapping_instance_id = fapi_get_u16(cfm, u.mlme_nan_bootstrapping_request_cfm.bootstrapping_instance_id);
	if (bootstrapping_instance_id != hal_req->bootstrapping_instance_id) {
		SLSI_NET_ERR(dev, "MLME_NAN_BOOTSTRAPPING_REQUEST_CFM(id:0x%04x)\n", bootstrapping_instance_id);
		r = -EINVAL;
	}

	if (hal_req && !r)
		set_bit(hal_req->bootstrapping_instance_id - 1, ndev_vif->nan.bootstrapping_instance_id_map);
	else
		clear_bit(hal_req->bootstrapping_instance_id - 1, ndev_vif->nan.bootstrapping_instance_id_map);
	kfree_skb(cfm);
	return r;
}

static u32 slsi_mlme_nan_bootstrapping_response_fapi_data(struct netdev_vif *ndev_vif, struct sk_buff *req,
							  struct slsi_hal_nan_bootstrapping_rsp *hal_req)
{
	...
	if (hal_req->comebackinfo.comeback_delay) {
		ret = slsi_mlme_nan_append_bootstrapping_comeback_info(req, hal_req->comebackinfo.comeback_delay,
								       hal_req->comebackinfo.cookie,
								       hal_req->comebackinfo.cookie_length);
		if (ret)
			SLSI_WARN_NODEV("Error append bootstrapping come back info TLV\n");
...
int slsi_mlme_nan_bootstrapping_response(struct slsi_dev *sdev, struct net_device *dev,
					 struct slsi_hal_nan_bootstrapping_rsp *hal_req)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	...
	req = fapi_alloc(mlme_nan_bootstrapping_response_req, MLME_NAN_BOOTSTRAPPING_RESPONSE_REQ,
			 ndev_vif->vifnum, 5 * 255);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
	...
	r = slsi_mlme_nan_bootstrapping_response_fapi_data(ndev_vif, req, hal_req);
	if (r) {
		SLSI_NET_ERR(dev, "Failed to construct mbulkdata\n");
		kfree_skb(req);
		return -EINVAL;
	...
	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_BOOTSTRAPPING_RESPONSE_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_nan_bootstrapping_response_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "MLME_NAN_BOOTSTRAPPING_RESPONSE_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_bootstrapping_response_cfm.result_code));
		r = -EINVAL;
	}

	bootstrapping_instance_id = fapi_get_u16(cfm, u.mlme_nan_bootstrapping_response_cfm.bootstrapping_instance_id);
	if (bootstrapping_instance_id != hal_req->bootstrapping_instance_id) {
		SLSI_NET_ERR(dev, "MLME_NAN_BOOTSTRAPPING_RESPONSE_CFM(id:%d)\n",
			     fapi_get_u16(cfm, u.mlme_nan_bootstrapping_response_cfm.bootstrapping_instance_id));
		r = -EINVAL;
	}
...
static u32 slsi_mlme_nan_pairing_request_fapi_data(struct sk_buff *req,
						   struct slsi_hal_nan_pairing_req *hal_req)
{
	u32 ret = 0;

	if (hal_req->enable_pairing_cache) {
		ret = slsi_mlme_nan_append_identity_key(req, hal_req->nan_identity_key);
		if (ret)
			SLSI_WARN_NODEV("Error append identity key TLV\n");
	}
...
int slsi_mlme_nan_pairing_request(struct slsi_dev *sdev, struct net_device *dev,
				  struct slsi_hal_nan_pairing_req *hal_req)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	...
	req = fapi_alloc(mlme_nan_pairing_request_req, MLME_NAN_PAIRING_REQUEST_REQ,
			 ndev_vif->vifnum, 32);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_pairing_request_req.peer_nmi_address, hal_req->peer_disc_mac_addr);
	fapi_set_u16(req, u.mlme_nan_pairing_request_req.session_id, hal_req->session_id);
	fapi_set_u8(req, u.mlme_nan_pairing_request_req.request_type,
		    hal_req->nan_pairing_request_type);
	fapi_set_u8(req, u.mlme_nan_pairing_request_req.cipher_type,
		    hal_req->sec_info.cipher_type);
	fapi_set_u16(req, u.mlme_nan_pairing_request_req.enable_caching,
		     hal_req->enable_pairing_cache);

	r = slsi_mlme_nan_pairing_request_fapi_data(req, hal_req);
	if (r) {
		SLSI_NET_ERR(dev, "Failed to construct mbulkdata\n");
		kfree_skb(req);
		return -EINVAL;
	...
	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_PAIRING_REQUEST_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_nan_pairing_request_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "MLME_NAN_PAIRING_REQUEST_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_pairing_request_cfm.result_code));
		r = -EINVAL;
	}

	sdev->pasn_params.publish_id = fapi_get_u16(cfm, u.mlme_nan_pairing_request_cfm.publish_id);
	sdev->pasn_params.dialog_token = fapi_get_u16(cfm, u.mlme_nan_pairing_request_cfm.dialog_token);
	sdev->pasn_params.bootstrapping_method = fapi_get_u16(cfm, u.mlme_nan_pairing_request_cfm.bootstrapping_method);
	...
	if (ptr) {
		...
		while (sig_data_len >= tag_len + 4) {
			...
			case SLSI_NAN_TLV_NAN_IDENTITY_RESOLUTION_ATTR:
				if (tag_len < 0x0010) {
					SLSI_WARN(sdev, "Invalid taglen(%d) for "
						  "SLSI_NAN_TLV_NAN_IDENTITY_RESOLUTION_ATTR\n", tag_len);
					break;
				}
				memcpy(sdev->pasn_params.local_nonce, tag_data_ptr, NAN_IDENTITY_NONCE_LEN);
				tag_data_ptr += NAN_IDENTITY_NONCE_LEN;
				memcpy(sdev->pasn_params.local_tag, tag_data_ptr, NAN_IDENTITY_TAG_LEN);
	...
	if (hal_req && !r)
		set_bit(hal_req->pairing_instance_id - 1, ndev_vif->nan.pairing_instance_id_map);
	else
		clear_bit(hal_req->pairing_instance_id - 1, ndev_vif->nan.pairing_instance_id_map);
	kfree_skb(cfm);
	return r;
}

static u32 slsi_mlme_nan_pairing_response_fapi_data(struct sk_buff *req,
...
int slsi_mlme_nan_pairing_response(struct slsi_dev *sdev, struct net_device *dev,
				   struct slsi_hal_nan_pairing_rsp *hal_req)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	...
	req = fapi_alloc(mlme_nan_pairing_response_req, MLME_NAN_PAIRING_RESPONSE_REQ,
			 ndev_vif->vifnum, 32);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_pairing_response_req.peer_nmi_address, hal_req->peer_disc_mac_addr);
	fapi_set_u16(req, u.mlme_nan_pairing_response_req.session_id, hal_req->session_id);
	fapi_set_u16(req, u.mlme_nan_pairing_response_req.request_type,
		     hal_req->nan_pairing_request_type);
	fapi_set_u8(req, u.mlme_nan_pairing_response_req.response,
		    hal_req->rsp_code);
	fapi_set_u8(req, u.mlme_nan_pairing_response_req.cipher_type,
		    hal_req->sec_info.cipher_type);
	fapi_set_u16(req, u.mlme_nan_pairing_response_req.enable_caching,
		     hal_req->enable_pairing_cache);

	r = slsi_mlme_nan_pairing_response_fapi_data(req, hal_req);
	if (r) {
		SLSI_NET_ERR(dev, "Failed to construct mbulkdata\n");
		kfree_skb(req);
		return -EINVAL;
	...
	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_PAIRING_RESPONSE_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_nan_pairing_response_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "MLME_NAN_PAIRING_RESPONSE_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_pairing_response_cfm.result_code));
		r = -EINVAL;
	}
...
int slsi_mlme_nan_pairing_request_response(struct slsi_dev *sdev, struct net_device *dev, u8 response)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	int               r = 0;
	...
	req = fapi_alloc(mlme_nan_pairing_request_res, MLME_NAN_PAIRING_REQUEST_RES,
			 ndev_vif->vifnum, 32);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_pairing_request_res.peer_nmi_address,
			ndev_vif->nan.pairing_request_ind.peer_nmi_address);
	fapi_set_u16(req, u.mlme_nan_pairing_request_res.session_id, ndev_vif->nan.pairing_request_ind.session_id);
	fapi_set_u8(req, u.mlme_nan_pairing_request_res.response, response);

	slsi_mlme_req_no_cfm(sdev, dev, req);
	return r;
}

int slsi_mlme_nan_pairing_response_response(struct slsi_dev *sdev, struct net_device *dev, u8 response)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff    *req;
	int               r = 0;
	...
	req = fapi_alloc(mlme_nan_pairing_response_res, MLME_NAN_PAIRING_RESPONSE_RES,
			 ndev_vif->vifnum, 32);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_pairing_response_res.peer_nmi_address,
			ndev_vif->nan.pairing_response_ind.peer_nmi_address);
	fapi_set_u16(req, u.mlme_nan_pairing_response_res.session_id, ndev_vif->nan.pairing_response_ind.session_id);
	fapi_set_u8(req, u.mlme_nan_pairing_response_res.response, response);

	slsi_mlme_req_no_cfm(sdev, dev, req);
	return r;
}

int slsi_mlme_nan_pairing_end(struct slsi_dev *sdev, struct net_device *dev,
			      struct slsi_hal_nan_pairing_end *hal_req)
{
	...
	req = fapi_alloc(mlme_nan_pairing_end_req, MLME_NAN_PAIRING_END_REQ,
			 ndev_vif->vifnum, 32);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	...
	fapi_set_memcpy(req, u.mlme_nan_pairing_end_req.peer_nmi_address, hal_req->peer_disc_mac_addr);
	fapi_set_u16(req, u.mlme_nan_pairing_end_req.session_id, hal_req->session_id);

	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_PAIRING_END_CFM);
	if (!cfm)
		return -EIO;

	if (fapi_get_u16(cfm, u.mlme_nan_pairing_end_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "MLME_NAN_PAIRING_END_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_pairing_end_cfm.result_code));
		r = -EINVAL;
	}
...
static bool slsi_nan_send_frame_cfm_validate(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *cfm)
{
	int  result = fapi_get_u16(cfm, u.mlme_send_frame_cfm.result_code);
	bool r = false;
	...
	if (result == FAPI_RESULTCODE_SUCCESS)
		...
		SLSI_NET_ERR(dev, "mlme_nan_send_frame_cfm(result:0x%04x) ERROR\n", result);
...
int slsi_mlme_nan_send_frame_mgmt(struct slsi_dev *sdev, struct net_device *dev, const u8 *frame, int frame_len,
				  u16 data_desc, u16 msg_type, u16 host_tag, u16 freq, u32 dwell_time, u32 period)
{
	...
	rx = slsi_mlme_req_cfm_ind(sdev, dev, req, MLME_SEND_FRAME_CFM, MLME_FRAME_TRANSMISSION_IND,
				   slsi_nan_send_frame_cfm_validate);
	if (!rx)
		return -EIO;

	tx_status = fapi_get_u16(rx, u.mlme_frame_transmission_ind.transmission_status);
	if (tx_status) {
		SLSI_NET_ERR(dev, "failed to transmission_status : %d\n", tx_status);
		kfree_skb(rx);
		return -EIO;
	}
...
int slsi_mlme_nan_set_config(struct slsi_dev *sdev, struct net_device *dev, struct slsi_hal_nan_config_req *hal_req)
{
	...
	req = fapi_alloc(mlme_nan_config_req, MLME_NAN_CONFIG_REQ, ndev_vif->vifnum, 64);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
			  ...
			  struct slsi_hal_nan_data_path_initiator_req *hal_req, u32 ndp_instance_id, u16 ndl_vif_id)
{
	...
	req = fapi_alloc(mlme_ndp_request_req, MLME_NDP_REQUEST_REQ, ndev_vif->vifnum, data_len);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
		slsi_rx_netif_napi_initialize(data_dev);
#endif
			   ...
			   struct slsi_hal_nan_data_path_indication_response *hal_req, u16 local_ndp_instance_id)
{
	...
	req = fapi_alloc(mlme_ndp_response_req, MLME_NDP_RESPONSE_REQ, ndev_vif->vifnum, data_len);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
...
int slsi_mlme_ndp_terminate(struct slsi_dev *sdev, struct net_device *dev, u16 ndp_instance_id, u16 transaction_id)
{
	...
	req = fapi_alloc(mlme_ndp_terminate_req, MLME_NDP_TERMINATE_REQ, ndev_vif->vifnum, 0);
	if (!req) {
		SLSI_NET_ERR(dev, "fapi alloc failure\n");
		return -ENOMEM;
	}
			    ...
			    struct slsi_rtt_config *nl_rtt_params)
{
	...
	/*fill the data */
	fapi_set_u16(req, u.mlme_nan_range_req.vif, ndev_vif->vifnum);
	for (i = 0; i < count; i++) {
...
int slsi_mlme_nan_range_cancel_req(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	/*fill the data */
	fapi_set_u16(req, u.mlme_nan_range_cancel_req.vif, ndev_vif->vifnum);
...
int slsi_mlme_vendor_nan_set_command(struct slsi_dev *sdev, struct net_device *dev,
	...
	req = fapi_alloc(mlme_nan_command_req, MLME_NAN_COMMAND_REQ, ndev_vif->vifnum, hal_req->cmd_req_len);
	if (!req) {
		SLSI_NET_ERR(dev, "[STD+][DRIVER]fapi alloc failure\n");
		return -ENOMEM;
	}

	fapi_set_u16(req, u.mlme_nan_command_req.vif, ndev_vif->vifnum);
	fapi_append_data(req, hal_req->cmd_req, hal_req->cmd_req_len);

	cfm = slsi_mlme_req_cfm(sdev, dev, req, MLME_NAN_COMMAND_CFM);
	if (!cfm)
		return -EIO;

	(*hal_cfm) = kzalloc(sizeof(struct slsi_hal_nan_vendor_cmd_resp_data) + fapi_get_datalen(cfm), GFP_KERNEL);
	if (!(*hal_cfm)) {
		SLSI_ERR(sdev, "[STD+][DRIVER]Failed to alloc hal_cfm structure!!!\n");
		kfree_skb(cfm);
		return -ENOMEM;
	}

	(*hal_cfm)->cmd_resp_len = fapi_get_datalen(cfm);
	memcpy((*hal_cfm)->cmd_resp, fapi_get_data(cfm), (*hal_cfm)->cmd_resp_len);
	if (fapi_get_u16(cfm, u.mlme_nan_command_cfm.result_code) != FAPI_RESULTCODE_SUCCESS) {
		SLSI_NET_ERR(dev, "[STD+][DRIVER]MLME_NAN_COMMAND_CFM(res:0x%04x)\n",
			     fapi_get_u16(cfm, u.mlme_nan_command_cfm.result_code));
		r = -EINVAL;
	}
	kfree_skb(cfm);
	return r;
