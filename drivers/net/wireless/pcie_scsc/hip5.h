...
#define FROMHOST_INTR_NUM              3
#define TOHOST_INTR_NUM		       5
...
struct hip_priv {
...
#endif
#ifdef CONFIG_SCSC_PCIE_CHIP
	DECLARE_BITMAP(pcie_claim_bit, 1);
	DECLARE_BITMAP(fromhost_intr_bit, FROMHOST_INTR_NUM);
	DECLARE_BITMAP(tohost_intr_mask_bit, TOHOST_INTR_NUM);
	DECLARE_BITMAP(tohost_intr_unmask_bit, TOHOST_INTR_NUM);
	struct workqueue_struct		*pcie_wq;
	struct pcie_control		pcie_ctrl;
	/* count for user pcie lock hold */
	atomic_t			pcie_claim_non_irq_cnt;
	atomic_t			pcie_claim_intrbit_set_cnt;
	atomic_t			pcie_claim_intrbit_mask_cnt;
	atomic_t			pcie_claim_intrbit_unmask_cnt;
	rwlock_t			rw_pcie;
	rwlock_t			pcie_ctrl_reason;
#endif
	/* Interrupts cache */
	...
	struct slsi_wake_lock             wake_lock_tx;
	struct slsi_wake_lock             wake_lock_ctrl;
	struct slsi_wake_lock             wake_lock_data;
