...
static bool invalid_amsdu_panic;
module_param(invalid_amsdu_panic, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(invalid_amsdu_panic, "enable triggering panic when invalid A-msdu is handed, Y: enable, N: disable(default)");

static bool invalid_mac_to_netdev_panic = false;
module_param(invalid_mac_to_netdev_panic, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(invalid_mac_to_netdev_panic, "trigger panic on invalid MAC addr to Netdev mapping, 1: enable (default), 0: disable");
...
static int sap_ma_notifier(struct slsi_dev *sdev, unsigned long event)
{
	...
	case SCSC_WIFI_STOP:
		...
		rcu_read_lock();
		for (ifnum = SLSI_NET_INDEX_WLAN;
		     ifnum <= SLSI_NET_INDEX_P2PX_SWLAN; ifnum++) {
			struct net_device *ndev =
				slsi_get_netdev_rcu(sdev, ifnum);
...
{
	return (unsigned int)(skb_tail_pointer(skb) - skb->data);
}

/**
 * skb_put_data_frag - Copy data into msdu subframe from given skb
 * @subframe: MSDU frame to be written
 * @skb_pointer: Address of source skb
 * @write_len: Length of the data to write
 *
 * This function copies a specified fragment from the source sk_buff into MSDU subframe
 * which should passed to Upper layer. It handles both linear and non-linear (fragmented)
 * skb data. When skb's length is not enough to handle write_len, skb is replaced with
 * the next fragment (skb->next). Using sk_buf **skb_pointer, the skb replacement is valid
 * even after the function returned.
 *
 * Return: 0 on success
 *         -EINVAL on failure (when write_len is still left, but there is no more frag skb following)
 */
static int skb_put_data_frag(struct sk_buff *subframe, struct sk_buff **skb_pointer, unsigned int write_len)
{
	struct sk_buff *skb = *skb_pointer;

	if (skb_len(skb) >= write_len) {
		skb_put_data(subframe, skb->data, write_len);
		skb_pull(skb, write_len);
	} else {
		write_len -= skb_len(skb);
		skb_put_data(subframe, skb->data, skb_len(skb));

		if (!skb->next)
			return -EINVAL;
		*skb_pointer = skb->next;
...
/**
 * skb_pull_frag - skb pull function regarding fragmented skb
 * @skb_pointer: Address of source skb
 * @pull_len: Length of the data to pull
 *
 * This function is for calling skb_pull, especially for fragmented skb.It handles both linear
 * and non-linear (fragmented) skb data. When skb's length is not enough to handle pull_len,
 * skb is replaced with the next fragment (skb->next). Using sk_buf **skb_pointer,
 * the skb replacement is valid even after the function returned.
 *
 * Return: skb->data on success
 *         0 on failure (when pull_len is still left, but there is no more frag skb following)
 */
static unsigned char *skb_pull_frag(struct sk_buff **skb_pointer, unsigned int pull_len)
{
	struct sk_buff *skb = *skb_pointer;

	if (skb_len(skb) >= pull_len)
		return skb_pull(skb, pull_len);

	pull_len -= skb_len(skb);
	/* No need skb_pull(skb, skb->len); */
...
/**
 * slsi_rx_amsdu_deaggregate - deaggregate A-MSDU into list of MSDUs
 * @dev: Network device
 * @skb: skb->data has A-MSDU, skb could be a list head of fragmented skbs
 * @msdu_list: list head of which MSDUs should be linked to
 *
 * This function splits A-MSDU into individual MSDUs, and link them to msdu_list.
 *
 * Return: 0 on success
 *         ERROR on failure
 */
int slsi_rx_amsdu_deaggregate(struct net_device *dev, struct sk_buff *skb, struct sk_buff_head *msdu_list)
{
	...
	int skb_len_left = skb->len;
	unsigned char padding = 0;
	struct sk_buff *subframe = NULL;
	struct sk_buff *skb_free_all = skb;
	const unsigned char mac_0[ETH_ALEN] = { 0 };
	unsigned char eth_buf[ETH_ALEN * 2 + 2] = { 0 };
	...
	while (skb_len_left > 0) {
		 ...
		 *             ^msdu_len^
		 */
		if (unlikely((ETH_ALEN * 2 + 2) > skb_len_left)) {
			SLSI_NET_ERR(dev, "invalid SKB length %d < %d (SKB:0x%X)\n", skb_len_left, (ETH_ALEN * 2 + 2),
				     skb_cb->skb_dma_addr);
			__skb_queue_purge(msdu_list);
			if (skb_has_frag_list(skb_free_all))
				skb_free_all->next = NULL;
			kfree_skb(skb_free_all);
			if (invalid_amsdu_panic)
				SLSI_FW_BUG_ON_WQ(sdev, 1);
			return -EINVAL;
		}

		/* Save ethernet address and datalen in a buffer.
		 * +-----+-----+-----+-----------------------------+----------------------+------+-----+
		 * | Dst | Src | Len |             LLC             |         SNAP         | Data | Pad |
		 * | (6) | (6) | (2) | DSAP(1) | SSAP(1) | Ctrl(1) | OUI(3) | ETH_TYPE(2) |      |     |
		 * +-----+-----+-----+-----------------------------+----------------------+------+-----+
		 * <----eth_buf----->
		 */
		if (skb_len(skb) > ETH_ALEN * 2 + 2) {
			memcpy(eth_buf, skb->data, ETH_ALEN * 2 + 2);
			data_len = (skb->data[ETH_ALEN * 2] << 8) | skb->data[(ETH_ALEN * 2) + 1];
			skb_pull_frag(&skb, ETH_ALEN * 2 + 2);
		} else {
			struct sk_buff *skb_prev = skb;

			skb = skb->next;
			if (!skb) {
				SLSI_NET_ERR(dev, "invalid data length %d < %d (SKB:0x%X)\n", data_len,
					     LLC_SNAP_HDR_LEN, skb_cb->skb_dma_addr);
				__skb_queue_purge(msdu_list);
				if (skb_has_frag_list(skb_free_all))
					skb_free_all->next = NULL;
				kfree_skb(skb_free_all);
			...
			memcpy(eth_buf, skb_prev->data, skb_len(skb_prev));
			memcpy(eth_buf + skb_len(skb_prev), skb->data, ETH_ALEN * 2 + 2 - skb_len(skb_prev));
			data_len = (eth_buf[ETH_ALEN * 2] << 8) | eth_buf[(ETH_ALEN * 2) + 1];
			skb_pull_frag(&skb, ETH_ALEN * 2 + 2 - skb_len(skb_prev));
		}
		skb_len_left -= (ETH_ALEN * 2 + 2);
		if (unlikely(data_len <= LLC_SNAP_HDR_LEN)) {
			SLSI_NET_ERR(dev, "invalid data length %d < %d (SKB:0x%X)\n", data_len, LLC_SNAP_HDR_LEN, skb_cb->skb_dma_addr);
			__skb_queue_purge(msdu_list);
			if (skb_has_frag_list(skb_free_all))
				skb_free_all->next = NULL;
			kfree_skb(skb_free_all);
		 ...
		 * +-----+-----+-----+-----------------------------+----------------------+------+-----+
		 */
		if (unlikely(LLC_SNAP_HDR_LEN + data_len > skb_len_left)) {
			SLSI_NET_ERR(dev, "invalid MSDU data length %d, SKB length = %d (SKB:0x%X)\n", data_len,
				     skb_len_left, skb_cb->skb_dma_addr);
			__skb_queue_purge(msdu_list);
			if (skb_has_frag_list(skb_free_all))
				skb_free_all->next = NULL;
			kfree_skb(skb_free_all);
		...
		if (discard) {
			skb_pull_frag(&skb, LLC_SNAP_HDR_LEN + subframe_len - sizeof(struct ethhdr) + padding);
			skb_len_left -= (LLC_SNAP_HDR_LEN + subframe_len + padding - sizeof(struct ethhdr));
			continue;
		}
		 ...
		 */
		skb_put(subframe, ETH_ALEN * 2);
		memcpy(subframe->data, eth_buf, ETH_ALEN * 2);
		 ...
		 *             skb_put(subframe, 2)
		 */
		skb_put_data_frag(subframe, &skb, 2);
		skb_len_left -= 2;
		/* Copy data.
		 * +-----+-----+-----+-----------------------------+----------------------+------+-----+
		 * | Dst | Src | Len |             LLC             |         SNAP         | Data | Pad |
		 ...
		 */
		if (unlikely(skb_put_data_frag(subframe, &skb, data_len))) {
			kfree_skb(subframe);
			SLSI_NET_ERR(dev, "Invalid A-MSDU DATA\n");
			__skb_queue_purge(msdu_list);
			if (skb_has_frag_list(skb_free_all))
				skb_free_all->next = NULL;
			kfree_skb(skb_free_all);
		 ...
		 */
		if (skb_len_left > 0) {
			if (unlikely(!skb_pull_frag(&skb, padding))) {
				kfree_skb(subframe);
		...
		if (unlikely(SLSI_ETHER_EQUAL(mh->h_dest, mac_0))) {
			SLSI_NET_DBG3(dev, SLSI_RX,
				      "msdu subframe filtered out: MAC destination address " MACSTR "\n",
				      MAC2STR(mh->h_dest));
			kfree_skb(subframe);
			continue;
		}
	...
	if (unlikely(skb_peek(msdu_list) && slsi_skb_cb_get(skb)->wakeup))
		skb_peek(msdu_list)->mark = SLSI_WAKEUP_PKT_MARK;

	/* An MSDU subframe is completed. Given skb should be freed.
	 * Freeing first fragment frees all the following fragments in a chain.
	 */
	if (skb_has_frag_list(skb_free_all))
		skb_free_all->next = NULL;
	kfree_skb(skb_free_all);
...
#ifdef CONFIG_SLSI_WLAN_LPC
	u32 lpc_tag = 0;
	if (fapi_get_u16(skb, u.ma_unitdata_ind.vif) == SLSI_NET_INDEX_WLAN)
...
static int slsi_get_msdu_frame(struct net_device *dev, struct sk_buff *skb)
{
	...
	if (qc && (*qc & IEEE80211_QOS_CTL_A_MSDU_PRESENT)) {
		...
		for (link_id = 0; link_id < MAX_NUM_MLD_LINKS; link_id++) {
			...
			if (SLSI_ETHER_EQUAL(ieee80211_get_DA(hdr), ndev_vif->sta.links[link_id].addr))
				break;
		}
		da = (link_id >= MAX_NUM_MLD_LINKS) ? ieee80211_get_DA(hdr) : dev->dev_addr;
#else
		 ...
		 */
		memmove(&skb->data[offset - ETH_ALEN], da, ETH_ALEN);
...
void slsi_rx_data_deliver_skb(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb, bool ctx_napi)
{
	...
	if (is_amsdu) {
		...
		if (unlikely(sizeof(struct ethhdr) >= skb->len)) {
			SLSI_NET_WARN(dev, "Packet dropped (Invalid SKB length)\n");
	...
	while (!skb_queue_empty(&msdu_list)) {
			...
			if (is_multicast_ether_addr(ehdr->h_dest)) {
				...
				if (!slsi_is_isolate_ap_or_guest_vlan(sdev, dev, ehdr->h_source, &vlan_dev)) {
					...
					if (!rebroadcast_skb) {
						...
						SLSI_DBG3(sdev, SLSI_RX,
							  "Intra BSS: multicast " MACSTR ", proto=0x%.4X headroom:%d\n",
							  MAC2STR(ehdr->h_dest), be16_to_cpu(ehdr->h_proto),
							  skb_headroom(rebroadcast_skb));
					...
					SLSI_DBG3(sdev, SLSI_RX, "Intra BSS: Don't forward mcast. " MACSTR ", proto=0x%.4X\n",
						  MAC2STR(ehdr->h_dest), be16_to_cpu(ehdr->h_proto));
				}
			} else {
				...
				if (peer && peer->authorized) {
					if (slsi_is_isolate_ap_or_guest_vlan(sdev, dev, ehdr->h_source, &vlan_dev)) {
						 ...
						 */
						SLSI_DBG3(sdev, SLSI_RX, "Intra BSS: Drop unicast. " MACSTR ", proto=0x%.4X\n",
							  MAC2STR(ehdr->h_dest), be16_to_cpu(ehdr->h_proto));
						consume_skb(rx_skb);
						continue;
					}
					SLSI_DBG3(sdev, SLSI_RX,
						  "Intra BSS: unicast " MACSTR ", proto=0x%.4X, headroom:%d\n",
						  MAC2STR(ehdr->h_dest),
						  be16_to_cpu(ehdr->h_proto),
						  skb_headroom(rx_skb));
		...
		if (ndev_vif->ifnum >= SLSI_NAN_DATA_IFINDEX_START) {
			...
			if (is_multicast_ether_addr(ehdr->h_dest)) {
				 ...
				 */
				SLSI_NET_DBG2(dev, SLSI_RX, "NAN: multicast (source address: " MACSTR ")\n",
				...
				for ((i = ndev_vif->ifnum + 1); i <= CONFIG_SCSC_WLAN_MAX_INTERFACES; i++) {
					...
					for (j = 0; j < SLSI_PEER_INDEX_MAX; j++) {
						if (other_ndev_vif->peer_sta_record[j] &&
							...
							SLSI_NET_DBG2(other_dev, SLSI_RX, "NAN: source address match " MACSTR "\n",
								      MAC2STR(other_ndev_vif->peer_sta_record[j]->address));
							if (!duplicate_skb) {
		...
		if (ndev_vif->vif_type == FAPI_VIFTYPE_AP && sdev->num_ap_vlan) {
			...
			if (!vlan_dev) {
				SLSI_DBG3(sdev, SLSI_RX,
					  "No VLAN devices forund. Drop unicast. " MACSTR ", proto=0x%.4X\n",
					  MAC2STR(ehdr->h_dest), be16_to_cpu(ehdr->h_proto));
				consume_skb(rx_skb);
				continue;
			}
	...
	if (to_ndev_vif->enhanced_arp_detect_enabled && (to_ndev_vif->vif_type == FAPI_VIFTYPE_STATION)) {
		...
		if (to_ndev_vif->gro_enabled == SLSI_GRO_ENABLED) {
			...
			if (ctx_napi) {
				...
				read_lock_bh(&ndev_vif->netif_napi_lock);
				if (ndev_vif->use_netif_napi && slsi_skb_cb_get(rx_skb)->is_netif_napi)
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
				read_unlock_bh(&ndev_vif->netif_napi_lock);
#endif
#endif
			} else {
...
#endif
{
	...
	SLSI_NET_DBG4(dev, SLSI_RX,
		      "ma_unitdata_ind(ifnum:%d, dest:" MACSTR ", src:" MACSTR ", tid:%d, seq:%d, cipher:%d, discard:0x%X, is_amsdu:%d, is_tdls:%d delayed:%d last:%d\n",
		      ndev_vif->ifnum,
		      MAC2STR(eth_hdr->h_dest),
	...
	if (ndev_vif->vif_type == FAPI_VIFTYPE_STATION && peer->aid < SLSI_TDLS_PEER_INDEX_MIN && skb_cb->is_tdls) {
		if (printk_ratelimit())
			SLSI_NET_WARN(dev, "Packet received from TDLS but no TDLS exists (seq: 0x%x) Skip BA\n", skb_cb->seq_num);
	...
	if (ndev_vif->vif_type == FAPI_VIFTYPE_STATION && peer->aid >= SLSI_TDLS_PEER_INDEX_MIN && !skb_cb->is_tdls) {
		SLSI_NET_DBG2(dev, SLSI_TDLS, "Packet received from TDLS peer through the AP(seq: 0x%x)\n", skb_cb->seq_num);
	...
	/* store the SN even when BA agreement is not setup yet */
	slsi_ba_store_sn(dev, peer, skb);
...
#ifdef CONFIG_SCSC_WLAN_RX_NAPI
static inline void slsi_rx_without_netif_napi(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
static int slsi_rx_napi_process(struct slsi_dev *sdev, struct sk_buff *skb)
{
	...
	if (ifnum > CONFIG_SCSC_WLAN_MAX_INTERFACES) {
		SLSI_WARN(sdev, "Invalid Ifnum. vif_index:%d sig_id:0x%4X \n", vif, fapi_get_sigid(skb));
		kfree_skb(skb);
		return 0;
	}
	...
	if (ifnum >= SLSI_NAN_DATA_IFINDEX_START &&
	    (fapi_get_sigid(skb) == MA_UNITDATA_IND || fapi_get_sigid(skb) == MA_BLOCKACKREQ_IND)) {
		dev = slsi_nan_get_netdev_rcu(sdev, skb);
		if (invalid_mac_to_netdev_panic && !dev) {
			SLSI_ERR(sdev, "invalid MAC in SKB (SKB:0x%X)\n", slsi_skb_cb_get(skb)->skb_dma_addr);
			SLSI_FW_BUG_ON_WQ(sdev, 1);
		}
	} else {
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
	/* hip5 NAPI instance should be used after netif napi queue are totally drained
	 * because of BA OoO when threaded NAPI is switched off.
	 */
	if (!ndev_vif->use_netif_napi && skb_queue_len_lockless(&ndev_vif->rx_data_q) == 0) {
		slsi_rx_without_netif_napi(sdev, dev, skb);
	} else {
		slsi_skb_cb_get(skb)->is_netif_napi = true;
...
static int slsi_rx_queue_data(struct slsi_dev *sdev, struct sk_buff *skb)
{
	...
	if (ifnum > CONFIG_SCSC_WLAN_MAX_INTERFACES) {
		SLSI_WARN(sdev, "Invalid Ifnum. vif:%d sigid:%d \n", vif, fapi_get_sigid(skb));
		return 0;
	}

	rcu_read_lock();
#ifdef CONFIG_SCSC_WIFI_NAN_ENABLE
	if (ifnum >= SLSI_NAN_DATA_IFINDEX_START && fapi_get_sigid(skb) == MA_UNITDATA_IND) {
		dev = slsi_nan_get_netdev_rcu(sdev, skb);
	} else {
		dev = slsi_get_netdev_rcu(sdev, ifnum);
	}
...
#ifdef CONFIG_SCSC_WLAN_RX_NAPI
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
		slsi_skb_cb_get(skb)->is_netif_napi = false;
#endif
		spin_lock_bh(&sap_ma_lock);
...
static int sap_ma_txdone(struct slsi_dev *sdev,  u8 vif, u8 peer_index, u8 ac)
{
	...
	if (ifnum > CONFIG_SCSC_WLAN_MAX_INTERFACES) {
		SLSI_WARN(sdev, "Invalid Ifnum vif:%d\n", vif);
		rcu_read_unlock();
		return 0;
	}
	dev = slsi_get_netdev_rcu(sdev, ifnum);
