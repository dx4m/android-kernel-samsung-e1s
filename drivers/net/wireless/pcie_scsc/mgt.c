...
#include <pcie_scsc/kic/slsi_kic_lib.h>
#include <linux/fs.h>
...
#include <pcie_scsc/scsc_release.h>
#include "mgt.h"
#include "sap_ma.h"
#include "ioctl.h"
#include "debug.h"
...
#include "../../../misc/samsung/pcie_scsc/scsc_wlbtd.h"
#endif
...
/* Set RF Test mode for 6GHz
 * 0 - RF Test Mode fixed to 6GHz LPI mode (OTA/Tput_test)
 * 1 - User mode (default)
 * 2 - RF Test Mode (referring to 6GHz Regulatory Information)
 * 3 - RF Test Mode fixed to 6GHz VLP mode
 * 4 - RF Test Mode fixed to 6GHz SP mode
 */
static int EnableRfTestMode = 1;

static int slsi_EnableRfTestMode_set(const char *val, const struct kernel_param *kp)
{
	int enableRfTestMode;
	int ret;

	SLSI_INFO_NODEV("EnableRfTestMode value : %s\n" , val);

	ret = kstrtoint(val, 10, &enableRfTestMode);
	if (ret < 0) {
		SLSI_ERR_NODEV("coverting string to int is failed\n");
		return ret;
	}

	if (enableRfTestMode < 0 || enableRfTestMode > 4) {
		SLSI_ERR_NODEV("invalid value %d: must be between 0 and 4\n", enableRfTestMode);
		return -EINVAL;
	}

	*(int *)kp->arg = enableRfTestMode;

	return 0;
}

static int slsi_EnableRfTestMode_get(char *buffer, const struct kernel_param *kp)
{
...
const struct kernel_param_ops slsi_EnableRfTestMode_ops = {
    .set = slsi_EnableRfTestMode_set,
    .get = slsi_EnableRfTestMode_get,
};
module_param_cb(EnableRfTestMode, &slsi_EnableRfTestMode_ops, &EnableRfTestMode, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(EnableRfTestMode, "Enable RF test mode driver. (0 ~ 4)");
...
static void write_softap_info_file(struct slsi_dev *sdev)
{
	...
	offset = snprintf(buf, sizeof(buf),
			  ...
			  "HalFn_setCountryCodeHal=yes\n" /* always supported */
			  "HalFn_getValidChannels=yes\n", /* always supported */
			  sdev->dualband_concurrency ? "yes" : "no", sdev->band_5g_supported ? "yes" : "no",
			  !sdev->softap_max_client ? SLSI_MIB_MAX_CLIENT : sdev->softap_max_client);

	if (sizeof(buf) - offset > 0)
#ifdef CONFIG_SCSC_WLAN_SAP_POWER_SAVE
		offset += snprintf(buf + offset, sizeof(buf) - offset, "PowerSave=yes\n");
#else
		offset += snprintf(buf + offset, sizeof(buf) - offset, "PowerSave=no\n");
#endif

	if (sizeof(buf) - offset > 0)
#ifdef CONFIG_SCSC_WLAN_HE
		offset += snprintf(buf + offset, sizeof(buf) - offset, "11AX=yes\n");
#else
		offset += snprintf(buf + offset, sizeof(buf) - offset, "11AX=no\n");
#endif
...
static int slsi_update_lls_num_radio(struct slsi_dev *sdev)
{
...
#ifdef CONFIG_SCSC_BB_REDWOOD
	sdev->num_of_radios_supported = ARRAY_SIZE(radio_type);
#else
	sdev->num_of_radios_supported = sdev->lls_num_radio;
#endif
	buf_size = sizeof(struct slsi_lls_radio_stat) +
		   (sizeof(struct slsi_lls_channel_stat) * SLSI_MAX_SUPPORTED_CHANNELS_NUM);
	for (i = 0; i < sdev->num_of_radios_supported; i++) {
		if (sdev->radio_stat[i])
			continue;
		sdev->radio_stat[i] = kzalloc(buf_size, GFP_KERNEL);
		if (!sdev->radio_stat[i]) {
			SLSI_WARN(sdev, "No mem. Size:%d\n", buf_size);
			return -ENOMEM;
		}
		sdev->radio_stat[i]->radio = i + 1;
	}
	return 0;
}
...
int slsi_start(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	for (i = 0; i < SLSI_WLAN_MAX_MIB_FILE && sdev->mib[i].mib_file_name; i++) {
		...
		SLSI_INFO(sdev, "mib[%d].mib_file_name : %s", i, sdev->mib[i].mib_file_name);
	...
	SLSI_EC_GOTO(slsi_update_lls_num_radio(sdev), err, err_hip_started);
	...
	slsi_mlme_sar_set_index(sdev, NULL, sdev->cat2_dsi_id);
#endif
	return err;
...
#elif defined(CONFIG_SCSC_WLAN_ICTAS)
	if (values[++mib_index].type != SLSI_MIB_TYPE_NONE)
		sdev->sar_method = values[mib_index].u.uintValue;
	else
		SLSI_ERR(sdev, "SAR reading SAR Method\n");
#endif
...
u32 slsi_mib_get_tx_ba_allowed(struct slsi_dev *sdev, struct net_device *dev)
{
	struct slsi_mib_data mibrsp = { 0, NULL };
	struct slsi_mib_value *values = NULL;
	struct slsi_mib_get_entry get_values[] = { { SLSI_PSID_UNIFI_BA_TX_ENABLE_TID, { 0, 0 } }, };
	u32 value;

	mibrsp.dataLength = 16;
	mibrsp.data = kmalloc(mibrsp.dataLength, GFP_KERNEL);
	if (!mibrsp.data) {
		SLSI_NET_ERR(dev, "fail to alloc %d bytes\n", mibrsp.dataLength);
		return 0;
	}

	values = slsi_read_mibs(sdev, NULL, get_values, 1, &mibrsp);
	if (!values) {
		SLSI_NET_ERR(dev, "MIB read failed\n");
		kfree(mibrsp.data);
		return 0;
	}

	if (values[0].type != SLSI_MIB_TYPE_UINT) {
		SLSI_NET_ERR(dev, "invalid type (%d) for SLSI_PSID_UNIFI_BA_TX_ENABLE_TID\n", values[0].type);
		kfree(mibrsp.data);
		kfree(values);
		return 0;
	}

	value = (int)values[0].u.uintValue;
...
int slsi_send_hanged_vendor_event(struct slsi_dev *sdev, u16 scsc_panic_code)
{
	...
	slsi_hanged_event_count++;
	SLSI_INFO(sdev, "Sending SLSI_NL80211_VENDOR_HANGED_EVENT , count: %d, reason =0x%02x\n", slsi_hanged_event_count, scsc_panic_code);
...
int slsi_vif_activated(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	if (!SLSI_IS_VIF_INDEX_NAN(ndev_vif)) {
		...
		slsi_rx_netif_napi_initialize(dev);
#endif
#ifdef CONFIG_SCSC_WLAN_LOAD_BALANCE_MANAGER
...
#endif
		if (ndev_vif->sta.pmkid) {
			kfree(ndev_vif->sta.pmkid);
			ndev_vif->sta.pmkid = NULL;
		}
					   ...
					   struct slsi_ap_sta_info *ap_sta_info, struct slsi_peer *peer)
{
	...
	if ((supported_oper_class_ie) &&
		...
		for (i = 0; i < ie_len; i++) {
			...
			SLSI_DBG1(sdev, SLSI_MLME, "supported_operating_classes, %d\n", oper_class);
...
static bool slsi_ml_get_sta_bss(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	sta_bss = __cfg80211_get_bss(sdev->wiphy, NULL, ndev_vif->sta.sme.bssid, ndev_vif->sta.sme.ssid,
				     ...
				     (NL80211_BSS_USE_FOR_NORMAL | NL80211_BSS_USE_FOR_MLD_LINK));
#else
...
void slsi_dump_ml_bssid_details(struct slsi_dev *sdev, struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	...
	SLSI_INFO(sdev, "Selected new ap with mld_addr " MACSTR "\n", MAC2STR(ndev_vif->sta.ap_mld_addr));
	list_for_each_entry(bssid_info, &ssid_info->bssid_list, list) {
		if (SLSI_ETHER_EQUAL(bssid_info->mld_addr, ndev_vif->sta.ap_mld_addr))
			SLSI_INFO(sdev, "bssid: " MACSTR ", freq: %u, rssi: %d, etp: %d, nboost_etp: %d,"
				  "connect_attempted: %d in_blacklist: %d\n", MAC2STR(bssid_info->bssid),
				  bssid_info->freq / 2, bssid_info->rssi, bssid_info->etp, bssid_info->nboost_etp,
				  bssid_info->connect_attempted,
				  slsi_is_bssid_in_blacklist(sdev, dev, bssid_info->bssid));
	}
}
#endif
...
int slsi_retry_connection(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	struct slsi_ssid_info *ssid_info;
	struct slsi_bssid_info *bssid_info;
	u8 old_bssid[ETH_ALEN], old_ap_mld_addr[ETH_ALEN];
#endif

	while (!ap_found) {
#ifdef CONFIG_SCSC_WLAN_EHT
		SLSI_ETHER_COPY(old_bssid, ndev_vif->sta.bssid);
		SLSI_ETHER_COPY(old_ap_mld_addr, ndev_vif->sta.ap_mld_addr);
#endif
...
#ifdef CONFIG_SCSC_WLAN_EHT
		/* Proceed if non-ml connection */
		if (!ndev_vif->sta.ml_connection)
			break;
		/* Update sta_bss if it is an new ml connection. If failed to update sta_bss, go for next valid bss */
		if ((ndev_vif->sta.sta_bss && SLSI_ETHER_EQUAL(old_bssid, ndev_vif->sta.sme.bssid)) ||
		    slsi_ml_get_sta_bss(sdev, dev)) {
		...
		if (!ndev_vif->sta.sta_bss) {
			...
			bssid_info = slsi_get_bssid_info_from_bssid(sdev, dev, &ssid_info->bssid_list,
								    ndev_vif->sta.sme.bssid);
			if (!bssid_info) {
			...
			if (!SLSI_ETHER_EQUAL(old_ap_mld_addr, ndev_vif->sta.ap_mld_addr))
				slsi_dump_ml_bssid_details(sdev, dev);
		}
#else
		break;
#endif
...
#ifdef CONFIG_SCSC_WLAN_EHT
	if (ndev_vif->sta.ml_connection)
		SLSI_INFO(sdev, "MLO band control: 0x%x\n", ndev_vif->sta.mlo_band_control);
#endif
...
#ifdef CONFIG_SCSC_WLAN_EHT
void slsi_clear_mlo_band_connection_params(struct slsi_dev *sdev, struct net_device *dev)
{
	struct slsi_ssid_info *ssid_info;
	struct slsi_bssid_info *bssid_info;
	...
	list_for_each_entry(bssid_info, &ssid_info->bssid_list, list) {
		bssid_info->connect_attempted = 0;
		if (!bssid_info->mlo_ap)
			continue;
		bssid_info->mlo_ap->tried_link_comb = 0;
	}
}
#endif
			  ...
			  )
{
	...
	case FAPI_VIFTYPE_STATION:
	{
...
#ifdef CONFIG_SCSC_WLAN_EHT
		slsi_clear_mlo_band_connection_params(sdev, dev);
#endif
		...
		slsi_mlme_set_host_state(sdev, dev, sdev->device_config.host_state);
		SLSI_MUTEX_UNLOCK(sdev->device_config_mutex);
		slsi_mlme_set_low_latency_mode_req(sdev, dev, 0);
		...
		slsi_roam_channel_cache_prune(dev, SLSI_ROAMING_CHANNEL_CACHE_TIMEOUT,
					      ndev_vif->sta.last_connected_bss.ssid,
					      ndev_vif->sta.last_connected_bss.ssid_len);
		SLSI_MUTEX_UNLOCK(ndev_vif->scan_mutex);
		if (slsi_mlme_del_vif(sdev, dev) != 0)
			SLSI_NET_ERR(dev, "slsi_mlme_del_vif failed\n");
		slsi_vif_deactivated(sdev, dev);
...
#ifdef CONFIG_SLSI_WLAN_PACKET_FILETR_V2
	if (slsi_set_multicast_packet_filters(sdev, dev))
		SLSI_NET_ERR(dev, "Failed to slsi_set_multicast_packet_filters");
	if (slsi_set_common_packet_filters(sdev, dev))
		SLSI_NET_ERR(dev, "Failed to slsi_set_common_packet_filters");
#endif
}
...
static enum slsi_bss_security slsi_rsn_get_bss_sec(const u8 *akm_pos, int akm_suite_count)
{
	int i, pos = 0;
	int sec_type = 0;
	u32 akm_suite;
	...
	for (i = 0; i < akm_suite_count; i++) {
		akm_suite = __cpu_to_be32(*(u32 *)&akm_pos[pos]);
		if (akm_suite == WLAN_AKM_SUITE_SAE || akm_suite == WLAN_AKM_SUITE_FT_OVER_SAE ||
		    akm_suite == SLSI_WLAN_AKM_SUITE_SAE_EXT_KEY || akm_suite == SLSI_WLAN_AKM_SUITE_FT_SAE_EXT_KEY)
			sec_type |= SLSI_BSS_SECURED_SAE;
		else if (akm_suite == WLAN_AKM_SUITE_PSK || akm_suite == WLAN_AKM_SUITE_FT_PSK ||
			 akm_suite == WLAN_AKM_SUITE_PSK_SHA256)
			sec_type |= SLSI_BSS_SECURED_PSK;
		else if (akm_suite == WLAN_AKM_SUITE_8021X || akm_suite == WLAN_AKM_SUITE_FT_8021X)
			sec_type |= SLSI_BSS_SECURED_1x;
		pos += 4;
	}
	return sec_type;
}

static int slsi_rsno_get_bss_sec(const u8 *rsnie, int len)
{
	...
	/* Calculate the position of AKM suite in RSNE Override element
	 * ELEMENT ID (dd)(1 byte) + length(1 byte) + vendor oui(4 bytes)+
	...
	if (rsnie[1] < 12)
		return 0;
	pos = 12 + 2 + ((rsnie[12] | (rsnie[13] << 8)) * 4);
	if ((pos + 2) > (rsnie[1] + 2))
		return 0;
...
u8 slsi_bss_connect_type_get(struct slsi_dev *sdev, const u8 *ie, size_t ie_len, u8 *ie_type)
{
	...
	if (rsn) {
		...
		if (rsn_ie_len < (pos + 1))
			return sec_type;
		sec_type = slsi_rsn_get_bss_sec(&rsn[pos + 1], akm_count);

		/* Check for RSNO elements */
		rsne_o1 = cfg80211_find_vendor_ie(WLAN_OUI_WFA, SLSI_RSNE_OVERRIDE_IE_VENDOR_TYPE, ie, ie_len);
		if (rsne_o1)
			sec_type |= slsi_rsno_get_bss_sec(rsne_o1, ie_len);
		rsne_o2 = cfg80211_find_vendor_ie(WLAN_OUI_WFA, SLSI_RSNE_OVERRIDE_2_IE_VENDOR_TYPE, ie, ie_len);
		if (rsne_o2)
			sec_type |= slsi_rsno_get_bss_sec(rsne_o2, ie_len);
...
void slsi_roam_channel_cache_prune(struct net_device *dev, int seconds, char *ssid, int ssid_len)
{
	...
	if (ssid) {
		list_for_each_entry_safe(network_map, tmp, &ndev_vif->sta.network_map, list) {
			if (network_map->ssid.ssid_len == ssid_len &&
			    (memcmp(network_map->ssid.ssid, ssid, network_map->ssid.ssid_len) == 0)) {
			...
			struct slsi_acs_selected_channels acs_selected_channels)
{
	...
	if (acs_selected_channels.link_id != -1)
		err |= nla_put_s16(skb, SLSI_ACS_ATTR_LINK_ID, acs_selected_channels.link_id);
	SLSI_DBG3(sdev, SLSI_MLME, "pri_channel=%d,sec_channel=%d,vht_seg0_center_ch=%d,"
		  "vht_seg1_center_ch=%d, ch_width=%d, hw_mode=%d, band=%d, link_id=%d\n",
		  ...
		  acs_selected_channels.band, acs_selected_channels.link_id);
	if (err) {
		SLSI_ERR_NODEV("Failed nla_put err=%d\n", err);
		kfree_skb(skb);
		return -EINVAL;
...
int slsi_get_beacon_cu(struct slsi_dev *sdev, struct net_device *dev, int *mib_value, u16 vif_idx)
{
	struct slsi_mib_data mibreq = { 0, NULL };
	struct slsi_mib_data mibrsp = { 0, NULL };
	struct slsi_mib_entry mib_val;
	...
	r = slsi_mlme_get_with_vifidx(sdev, dev, mibreq.data, mibreq.dataLength, mibrsp.data,
				      mibrsp.dataLength, &rx_len, vif_idx);
	kfree(mibreq.data);
...
static void slsi_update_sta_sme(struct slsi_dev *sdev, struct net_device *dev, u8 *ssid, int ssid_len, u8 *bssid,
			   struct ieee80211_channel *chan)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
				...
				struct slsi_bssid_info *bssid_info)
{
	...
	struct slsi_mlo_ap_details *mlo_ap_details, *mlo_ap = NULL;
	u8 num_links = 0, bands = 0;
	...
	if (mlo_ap) {
		list_for_each_entry(tmp_info, bssid_list, list) {
			if (!SLSI_ETHER_EQUAL(tmp_info->mld_addr, bssid_info->mld_addr))
				continue;
			num_links++;
			bands |= slsi_freq_to_band(bssid_info->freq / 2);
			tmp_info->mlo_ap = mlo_ap;
		}
		mlo_ap->num_links = num_links;
		mlo_ap->bands = bands;
		return 0;
	}
	...
	if (!mlo_ap_details->num_links)
		kfree(mlo_ap_details);
	return 0;
}

u32 slsi_ml_connection_link_selection(struct net_device *dev, u8 band, struct slsi_bssid_info *bssid_info,
				      struct slsi_mlo_ap_details *mlo_ap_details)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	u8 num_links = min(ndev_vif->sta.max_ml_link, mlo_ap_details->num_links);
	int single_link = 0;
	bool support_5g_6g = false, support_2g_6g = false, support_2g_5g = false;
	...
	switch(band) {
	case SLSI_FREQ_BAND_6GHZ:
		single_link = SLSI_LINK_6GHZ;
		if (num_links == 1)
			break;
		if (mlo_ap_details->bands & SLSI_FREQ_BAND_5GHZ)
			support_5g_6g = true;
		if (mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ)
			support_2g_6g = true;
		break;
	case SLSI_FREQ_BAND_5GHZ:
		single_link = SLSI_LINK_5GHZ;
		if (num_links == 1)
			break;
		if (mlo_ap_details->bands & SLSI_FREQ_BAND_6GHZ)
			support_5g_6g = true;
		if (mlo_ap_details->bands & SLSI_FREQ_BAND_2GHZ)
			support_2g_5g = true;
		break;
	case SLSI_FREQ_BAND_2GHZ:
		single_link = SLSI_LINK_24GHZ;
		if (num_links == 1)
			break;
		if (mlo_ap_details->bands & SLSI_FREQ_BAND_6GHZ)
			support_2g_6g = true;
	...
	if (support_2g_6g && ((mlo_ap_details->tried_link_comb & SLSI_LINK_24GHZ_6GHZ) == 0)) {
		mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ_6GHZ;
		ndev_vif->sta.max_simul_link = 1;
		return SLSI_LINK_24GHZ_6GHZ;
	}
	if (support_2g_5g && ((mlo_ap_details->tried_link_comb & SLSI_LINK_24GHZ_5GHZ) == 0)) {
		mlo_ap_details->tried_link_comb |= SLSI_LINK_24GHZ_5GHZ;
		ndev_vif->sta.max_simul_link = 1;
		return SLSI_LINK_24GHZ_5GHZ;
	}
	if (support_5g_6g && ((mlo_ap_details->tried_link_comb & SLSI_LINK_5GHZ_6GHZ) == 0)) {
		mlo_ap_details->tried_link_comb |= SLSI_LINK_5GHZ_6GHZ;
		ndev_vif->sta.max_simul_link = 1;
		return SLSI_LINK_5GHZ_6GHZ;
	}

	mlo_ap_details->tried_link_comb |= single_link;
	ndev_vif->sta.max_simul_link = 0;
	bssid_info->connect_attempted = true;
	return single_link;
}
...
#define CHECK_6GHZ_3LINK_COMB(mlo_ap) ((mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ_6GHZ) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_6GHZ_2LINK_COMB(mlo_ap) ((((mlo_ap->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_6G_SINGLE_LINK_LEFT(mlo_ap) (((mlo_ap->tried_link_comb & SLSI_LINK_6GHZ) == 0) && \
					   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 3) ? \
...
#define CHECK_5GHZ_3LINK_COMB(mlo_ap) ((mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ_6GHZ) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_6GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)))
#define CHECK_5GHZ_2LINK_COMB(mlo_ap) ((((mlo_ap->bands & SLSI_FREQ_BAND_6GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_5GHZ_6GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_2GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ)))
#define CHECK_5G_SINGLE_LINK_LEFT(mlo_ap) (((mlo_ap->tried_link_comb & SLSI_LINK_5GHZ) == 0) && \
					   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 3) ? \
					   CHECK_5GHZ_3LINK_COMB(mlo_ap) : \
					   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 2) ? \
					    CHECK_5GHZ_2LINK_COMB(mlo_ap) : ((mlo_ap->num_links & SLSI_LINK_5GHZ) == 0))))
#define CHECK_2GHZ_3LINK_COMB(mlo_ap) ((mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ_6GHZ) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_6GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_2GHZ_2LINK_COMB(mlo_ap) ((((mlo_ap->bands & SLSI_FREQ_BAND_5GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_5GHZ)) && \
				       (((mlo_ap->bands & SLSI_FREQ_BAND_6GHZ) == 0) || \
				       (mlo_ap->tried_link_comb & SLSI_LINK_24GHZ_6GHZ)))
#define CHECK_2G_SINGLE_LINK_LEFT(mlo_ap) (((mlo_ap->tried_link_comb & SLSI_LINK_24GHZ) == 0) && \
					   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 3) ? \
					   CHECK_2GHZ_3LINK_COMB(mlo_ap) : \
					   ((min(ndev_vif->sta.max_ml_link, mlo_ap->num_links) == 2) ? \
					    CHECK_2GHZ_2LINK_COMB(mlo_ap) : ((mlo_ap->num_links & SLSI_LINK_24GHZ) == 0))))
#endif
				    ...
				    struct slsi_ssid_info *ssid_info)
{
	...
	if (is_valid_ether_addr(bssid_info->mld_addr)) {
		...
		ndev_vif->sta.mlo_band_control = ffs(links);
...
#ifdef CONFIG_SCSC_WLAN_EHT
static struct slsi_bssid_info *slsi_choose_next_ap_for_connection(struct slsi_dev *sdev, struct net_device *dev,
								  struct slsi_bssid_info *bssid_info,
								  struct slsi_ssid_info *ssid_info)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_bssid_info *tmp_bssid_info;

	if (!ssid_info || !bssid_info || !bssid_info->mlo_ap)
		return NULL;

	if (bssid_info->connect_attempted)
		return NULL;

	if (((ndev_vif->chan->band == NL80211_BAND_6GHZ) && CHECK_6G_SINGLE_LINK_LEFT(bssid_info->mlo_ap)) ||
	    ((ndev_vif->chan->band == NL80211_BAND_5GHZ) && CHECK_5G_SINGLE_LINK_LEFT(bssid_info->mlo_ap)) ||
	    ((ndev_vif->chan->band == NL80211_BAND_2GHZ) && CHECK_2G_SINGLE_LINK_LEFT(bssid_info->mlo_ap))) {
		/* Choose next best MLD AP based on ETP values of current AP and other MLD APs*/
		list_for_each_entry(tmp_bssid_info, &ssid_info->bssid_list, list) {
			if (slsi_is_bssid_in_blacklist(sdev, dev, tmp_bssid_info->bssid) ||
			    tmp_bssid_info->connect_attempted ||
			    SLSI_ETHER_EQUAL(bssid_info->mld_addr, tmp_bssid_info->mld_addr) ||
			    !is_valid_ether_addr(tmp_bssid_info->mld_addr))
					continue;
			if (bssid_info->nboost_etp && (bssid_info->nboost_etp < tmp_bssid_info->etp))
				return tmp_bssid_info;
			if (!bssid_info->nboost_etp && (bssid_info->etp < tmp_bssid_info->etp))
				return tmp_bssid_info;
		}

		/* Choose next best non-MLD AP based on ETP values of current AP and MLD APs*/
		list_for_each_entry(tmp_bssid_info, &ssid_info->bssid_list, list) {
			if (slsi_is_bssid_in_blacklist(sdev, dev, tmp_bssid_info->bssid) ||
			    tmp_bssid_info->connect_attempted ||
			    is_valid_ether_addr(tmp_bssid_info->mld_addr))
				   ...
				   struct ieee80211_channel **channel, bool retry)
{
	...
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_mlo_ap_details *mlo_ap_details;
	struct slsi_bssid_info *next_ap_bssid;
	u32 links;
#endif
	int r;
	...
	/* If all the link combinations of the associating link are tried and only the single link combination is left,
	 * choose next best MLD AP or next non-MLD AP.
	 */
	next_ap_bssid = slsi_choose_next_ap_for_connection(sdev, dev, bssid_info, ssid_info);
	if (next_ap_bssid) {
		r = slsi_update_retry_params(sdev, dev, next_ap_bssid, ssid_info);
		if (r < 0) {
			SLSI_ERR(sdev, "Failed to update retry params: %d\n", r);
			return false;
		}
...
int slsi_set_latency_crt_data(struct net_device *dev, int latency_mode)
{
	...
	ret = slsi_mlme_set_low_latency_mode_req(sdev, dev, latency_mode);
	if (!ret) {
		SLSI_INFO_NODEV("netdev NAPI / GRO should be disabled in low latency mode\n");
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
...
#endif
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
		push_netif_napi_event(dev, (u32)latency_mode, NETIF_NAPI_REASON_LATENCY);
#endif
	}
...
/**
 * To-Do: Remove this code when mib is updated.
 */
#ifndef SLSI_PSID_UNIFI6_GRF_TEST_MODE
#define SLSI_PSID_UNIFI6_GRF_TEST_MODE 0x0ACC
#endif
int slsi_set_mib_6g_rf_test_mode(struct slsi_dev *sdev)
{
	struct slsi_mib_data mib_data = { 0, NULL };
	int ret;

	ret = slsi_mib_encode_uint(&mib_data, SLSI_PSID_UNIFI6_GRF_TEST_MODE, EnableRfTestMode, 0);
	if (ret != SLSI_MIB_STATUS_SUCCESS) {
		SLSI_ERR(sdev, "Failed to encode the mib_data of RF TestMode. %d\n", ret);
		return -ENOMEM;
	}
	...
	ret = slsi_mlme_set(sdev, NULL, mib_data.data, mib_data.dataLength);
	if (ret)
		SLSI_ERR(sdev, "Failed to set mib RF test mode. %d\n", ret);
...
int slsi_configure_pmksa(struct slsi_dev *sdev, struct net_device *dev, struct slsi_peer *peer)
{
	const u8 *assoc_rsn_ie = NULL;
	struct netdev_vif   *ndev_vif = netdev_priv(dev);
	...
	if (!ndev_vif->sta.pmkid) {
		SLSI_DBG1(sdev, SLSI_CFG80211, "No PMKID to set\n");
		return 0;
	}

	if (!peer->assoc_ie) {
		SLSI_NET_ERR(dev, "Peer assoc IE is not present\n");
		ret = -EINVAL;
		goto exit;
	}

	assoc_rsn_ie = cfg80211_find_ie(WLAN_EID_RSN, peer->assoc_ie->data, peer->assoc_ie->len);
	if (!assoc_rsn_ie) {
		SLSI_NET_ERR(dev, "RSN IE is not present in Association request\n");
		ret = -EINVAL;
		goto exit;
	}
	rsnie_len = assoc_rsn_ie[1];
	...
	while (i < 2) {
		pos += 2;
		left -= 2;
		count = le16_to_cpu(*(u16 *)(&assoc_rsn_ie[pos - 2]));
	...
	left -= 2;
	if (pos <= (rsnie_len + 2)) {
		memcpy(rsnie, assoc_rsn_ie, pos);
	} else {
		SLSI_NET_ERR(dev, "pos is invalid\n");
		ret = -ENOMEM;
		goto exit_rsnie;
	}
	count = 0;
	if (left > 0 && left >= 2) {
		count = le16_to_cpu(*(u16 *)(&assoc_rsn_ie[pos]));  /* PMKID count */
		left -= 2;
	}
	pos += 2; /* PMKID count */
	rsnie[pos - 2] = 1;
	...
	memcpy(&rsnie[pos], ndev_vif->sta.pmkid, PMKID_LEN); /* copy PMKID */
	pos += PMKID_LEN;
	if (count && left > 0) {
		left -= (count * PMKID_LEN);
		memcpy(&rsnie[pos], &assoc_rsn_ie[pos + ((count - 1) * PMKID_LEN)], left);
	} else if (left > 0) {
		memcpy(&rsnie[pos], &assoc_rsn_ie[pos - PMKID_LEN], left);
	}
	pos += left;
	rsnie_len = pos;
	rsnie[1] = rsnie_len - 2;
			      ...
			      size_t count)
{
	...
	if (r == 0 && sysfs_pm >= 0 && sysfs_pm <= 4)
		EnableRfTestMode = sysfs_pm;
	else
...
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
	/* There may be a problem of overlapping napi disable */
	if (test_and_clear_bit(SLSI_NETIF_NAPI_ENABLED, &ndev_vif->is_napi_enabled)) {
#ifdef CONFIG_SCSC_WLAN_RX_NAPI_GRO
		read_lock_bh(&ndev_vif->gro_lock);
		write_lock_bh(&ndev_vif->netif_napi_lock);
		if (ndev_vif->gro_enabled == SLSI_GRO_ENABLED)
			napi_gro_flush(&ndev_vif->rx_data_napi, false);
		write_unlock_bh(&ndev_vif->netif_napi_lock);
		read_unlock_bh(&ndev_vif->gro_lock);
#endif
		napi_disable(&ndev_vif->rx_data_napi);
		netif_napi_del(&ndev_vif->rx_data_napi);
		skb_queue_purge(&ndev_vif->rx_data_q);
	}
#endif
...
u16 slsi_get_link_id_from_vif(struct net_device *dev, u16 vif_idx)
{
	...
	for (i = 0; i < MAX_NUM_MLD_LINKS; i++) {
		if (!(ndev_vif->sta.valid_links & BIT(i)))
			continue;
		if (ndev_vif->sta.links[i].mlo_vif_idx == vif_idx) {
			link_id = i;
			break;
		}
	}
