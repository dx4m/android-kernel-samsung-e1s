...
static int slsi_net_open(struct net_device *dev)
{
	...
	if (slsi_is_rf_test_mode_enabled()) {
		...
		slsi_set_mib_6g_rf_test_mode(sdev);
#endif
	}
...
static int slsi_net_stop(struct net_device *dev)
{
	...
	if (ndev_vif->ifnum == SLSI_NET_INDEX_WLAN) {
		...
		slsi_roam_channel_cache_prune(dev, 0, NULL, 0);
...
static netdev_tx_t slsi_net_hw_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (ndev_vif->iftype == NL80211_IFTYPE_STATION &&
		...
		skb_cpy = skb_copy_expand(skb,
					  SLSI_NETIF_SKB_HEADROOM,
					  sdev->hip.hip_priv->unidat_req_tailroom,
					  GFP_ATOMIC);
		if (!skb_cpy) {
			SLSI_ERR_NODEV("Cannot reserve headroom for some reason\n");
...
#ifdef CONFIG_SCSC_WLAN_HIP5
#define MAX_DAT_MBULK_SIZE HIP5_DAT_MBULK_SIZE
#else
#define MAX_DAT_MBULK_SIZE HIP4_DAT_MBULK_SIZE
#endif
	/* check if the frame meets the Max Data MBULK size */
	if ((skb->len + SLSI_NETIF_SKB_HEADROOM) > MAX_DAT_MBULK_SIZE) {
		SLSI_NET_WARN(dev, "Not supported, too large: len:%d, headroom:%d, Max:%d\n", skb->len, SLSI_NETIF_SKB_HEADROOM, MAX_DAT_MBULK_SIZE);
		r = -EFAULT;
		goto evaluate;
	}
...
static void  slsi_set_multicast_list(struct net_device *dev)
{
	struct netdev_vif     *ndev_vif = netdev_priv(dev);
	u8                    count, i = 0;
#if IS_ENABLED(CONFIG_SLSI_WLAN_PACKET_FILETR_V2)
...
#if IS_ENABLED(CONFIG_SLSI_WLAN_PACKET_FILETR_V2)
#if IS_ENABLED(CONFIG_IPV6)
		if (!memcmp(ha->addr, solicited_node_addr, 3) &&
		    !memcmp(&ha->addr[3], ipv6addr_suffix, 3)) { /* local multicast addr handled separately */
#else
		if (memcmp(ha->addr, mc_addr_prefix, 3)) { /* only consider IPv4 multicast addresses */
#endif
#else
#if IS_ENABLED(CONFIG_IPV6)
...
#ifdef CONFIG_SLSI_WLAN_PACKET_FILETR_V2
static void slsi_update_multicast_addr(struct slsi_dev *sdev, struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
static void slsi_update_pkt_filter_work(struct work_struct *data)
{
	...
	if (sdev->device_config.user_suspend_mode == 1 && ndev_vif->is_opt_out_packet) {
		...
		sdev->device_config.host_state &= ~SLSI_HOSTSTATE_SUSPEND_DISALLOWED;
...
#ifdef CONFIG_SCSC_WLAN_RX_NAPI
#ifdef CONFIG_SCSC_WLAN_NAPI_PER_NETDEV
int slsi_rx_netif_napi_poll(struct napi_struct *napi, int budget)
{
	...
	SLSI_DBG3(sdev, SLSI_RX, "skbs todo in queue:%hhu (vif:%s)\n", skb_queue_len(&ndev_vif->rx_data_q), dev->name);
	...
	if (work_done < budget) {
		SLSI_DBG3(sdev, SLSI_RX, "NAPI in netif complete(vif:%s work_done:%d)\n", dev->name, work_done);
		napi_complete(napi);
	} else {
		SLSI_DBG3(sdev, SLSI_RX, "vif:%s work done:%d\n", dev->name, work_done);
	}
	return work_done;
}

void slsi_rx_netif_napi_initialize(struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);

	if (test_bit(SLSI_NETIF_NAPI_ENABLED, &ndev_vif->is_napi_enabled)) {
		SLSI_DBG1_NODEV(SLSI_NETDEV,
				"netif NAPI for vif(ifnum:%d) has been created already\n", ndev_vif->ifnum);
		return;
	}

	/* Add napi instance for each interface */
...
#endif
	set_bit(SLSI_NETIF_NAPI_ENABLED, &ndev_vif->is_napi_enabled);
	ndev_vif->latency_mode = 0;
	ndev_vif->use_netif_napi = 0;
...
int slsi_netif_add_locked(struct slsi_dev *sdev, const char *name, int ifnum)
{
	...
	char                wlock_name[WLOCK_NAME_BUF_LEN] = { 0 };
	...
	snprintf(wlock_name, WLOCK_NAME_BUF_LEN, "wlan_sae%d", ifnum);
	slsi_wake_lock_init(NULL, &ndev_vif->wlan_wl_sae, wlock_name);
#if IS_ENABLED(CONFIG_IPV6)
	...
	if (ndev_vif->ifnum == SLSI_NET_INDEX_AP || ndev_vif->ifnum == SLSI_NET_INDEX_AP2 ||
	    strcmp(name, CONFIG_SCSC_AP_INTERFACE_NAME) == 0) {
#else
	if (ndev_vif->ifnum == SLSI_NET_INDEX_AP || ndev_vif->ifnum == SLSI_NET_INDEX_AP2) {
#endif
