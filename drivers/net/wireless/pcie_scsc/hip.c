...
static int slsi_hip_service_notifier(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	case SCSC_WIFI_RESUME:
		slsi_wake_lock(&sdev->wlan_wl_resume);
		schedule_work(&sdev->resume_work);
		break;
...
{
	sdev->hip_priv_buffer = kmalloc(sizeof(struct hip_priv), GFP_KERNEL);
	if (!sdev->hip_priv_buffer) {
		SLSI_ERR(sdev, "fail to allocate for hip_priv\n");
		return -ENOMEM;
	}
	return 0;
}
...
void slsi_hip_free_hip_priv(struct slsi_dev *sdev)
{
	...
	if (sdev->hip_priv_buffer) {
		kfree(sdev->hip_priv_buffer);
	}
	sdev->hip_priv_buffer = NULL;
}
...
int slsi_hip_rx(struct slsi_dev *sdev, struct sk_buff *skb)
{
	...
	if (fapi_is_ma(skb))
		SLSI_BIN_TAG_DEBUG(BIN_WIFI_DATA_RX, skb->data, skb->len);
	else
		SLSI_BIN_TAG_DEBUG(BIN_WIFI_CTRL_RX, skb->data, skb->len);
#endif
