...
#define SLSI_WIFI_TAG_VD_ULW_TYPE                         0xF056
#define SLSI_WIFI_TAG_VD_REASON                           0xF00D
#define SLSI_WIFI_TAG_VD_SUBREASON                        0xF057
#define SLSI_WIFI_TAG_VD_ROAM_CANCEL_REASON               0xF058

/*ToDO: remove when fapi update is done*/
#define FAPI_EVENT_WIFI_EVENT_BT_COEX_BT_DEVICE_STATUS                0x005F
#define FAPI_EVENT_WIFI_EVENT_WTC_IE_RECEIVED                         0x0060
#define FAPI_EVENT_WIFI_EVENT_WTC_IE_SENT                             0x0061
#define FAPI_EVENT_WIFI_EVENT_ROAM_SCAN_CANCELED                      0x0107
...
char *slsi_print_event_name(int event_id)
{
	...
	case SLSI_NL80211_NDP_REQ_EVENT:
		return "SLSI_NL80211_NDP_REQ_EVENT";
	case SLSI_NL80211_NDP_CFM_EVENT:
		return "SLSI_NL80211_NDP_CFM_EVENT";
	case SLSI_NL80211_NDP_END_EVENT:
		return "SLSI_NL80211_NDP_END_EVENT";
...
static int slsi_get_mib_rsno_support(struct slsi_dev *sdev)
{
	struct slsi_mib_data mibrsp = { 0, NULL };
	struct slsi_mib_value *values = NULL;
	struct slsi_mib_get_entry get_values[] = {{SLSI_PSID_UNIFI_RSNO_ACTIVATED, { 0, 0 } } };
	int ret = 0;
	...
	if (!values) {
		SLSI_ERR(sdev, "Error in slsi_read_mibs for SLSI_PSID_UNIFI_RSNO_ACTIVATED\n");
		ret = -EAGAIN;
		goto exit;
	}
	if (values[0].type != SLSI_MIB_TYPE_UINT) {
		SLSI_ERR(sdev, "Invalid type (%d) for SLSI_PSID_UNIFI_RSNO_ACTIVATED\n", values[0].type);
		ret = -EINVAL;
		goto exit_with_values;
	}

	SLSI_DBG3(sdev, SLSI_MLME, "unifiRsnoActivated: %d\n", values[0].u.boolValue);
	ret = (int)values[0].u.uintValue;
exit_with_values:
	kfree(values);
exit:
	kfree(mibrsp.data);
...
static int slsi_get_features_supported(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int len)
{
	struct slsi_dev *sdev = SDEV_FROM_WIPHY(wiphy);
	struct sk_buff *reply;
	u32 flags = 0;
	...
	if (slsi_get_mib_rsno_support(sdev) == 1)
		flags |= SLSI_VENDOR_FEATURE_RSN_OVERRIDE_STA;
	ret = nla_put_u32(reply, SLSI_VENDOR_ATTR_FEATURE_FLAGS, flags);
	if (ret) {
		SLSI_ERR(sdev, "Error in nla_put*:0x%x\n", ret);
	...
	ret =  cfg80211_vendor_cmd_reply(reply);
	if (ret)
		SLSI_ERR(sdev, "FAILED to reply SLSI_NL80211_VENDOR_SUBCMD_GET_FEATURES\n");
...
char *slsi_get_roam_cancel_reason_str(u32 reason_code)
{
	switch (reason_code) {
	case 0x0001:
		return "User_Scan_Triggered";
	case 0x0002:
		return "Already_Associated_Bssid";
	case 0x0003:
		return "Hard_Roaming_Scan_Triggered";
	case 0x0004:
...
char *slsi_get_nan_ulw_type(int ulw_type)
{
	switch (ulw_type) {
	case 0:
		return "PRESENCE";
...
void slsi_handle_nan_rx_event_log_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	u32 ulw_reason = 0, ulw_index = 0, ulw_start = 0, ulw_period = 0, ulw_duration = 0, ulw_count = 0, ulw_type = 2;
	...
	while (i + 4 < tlv_buffer__len) {
		...
		case SLSI_WIFI_TAG_VENDOR_SPECIFIC:
			...
			case SLSI_WIFI_TAG_VD_ULW_REASON:
				...
				break;
			case SLSI_WIFI_TAG_VD_ULW_TYPE:
				ulw_type = vtag_value;
				break;
	...
	case FAPI_EVENT_WIFI_EVENT_NAN_ULW_UPDATE:
		SLSI_INFO(sdev, "[0x%lx] NAN_ULW_UPDATE, Master_TSF: 0x%x, ULW_Reason:%s, ULW_Index: %d,"
			  " ULW_start_time:%dus, ULW_Period: %dus, ULW_Duration: %dus, ULW_Count: %d, Freq: %d ULW_TYPE: %s\n",
			  timestamp, pm_set.master_tsf, slsi_get_nan_ulw_reason_str(ulw_reason), ulw_index,
			  ulw_start, ulw_period, ulw_duration, ulw_count, pm_set.channel, slsi_get_nan_ulw_type(ulw_type));
		break;
				   ...
				   u16 event_id, struct slsi_rx_evt_info *evt_info)
{
	...
	evt_info->vd.btm_disassoc_timer = -1;
	...
	while (i + 4 < tlv_buffer__len) {
		...
		case SLSI_WIFI_TAG_RSSI:
			evt_info->rssi_val = (short)tag_value;
			break;
		...
		case SLSI_WIFI_TAG_VENDOR_SPECIFIC:
			...
			case SLSI_WIFI_TAG_VD_PARAMETER_SET:
				...
				if (event_id == FAPI_EVENT_WIFI_EVENT_ROAM_SCAN_RESULT) {
					dump_roam_scan_result(sdev, dev, &evt_info->current_info, evt_info->mac_addr,
							      evt_info->chan_frequency, evt_info->rssi_val,
			...
			case SLSI_WIFI_TAG_VD_LOCAL_DURATION:
				if (evt_info->vd.btm_disassoc_timer >= 0)
			...
			case SLSI_WIFI_TAG_VD_REASON:
				...
				break;
			case SLSI_WIFI_TAG_VD_SUBREASON:
				evt_info->vd.subreason = vtag_value;
				break;
			case SLSI_WIFI_TAG_VD_ULW_DURATION:
				evt_info->vd.ulw_duration = vtag_value;
				break;
			case SLSI_WIFI_TAG_VD_ROAM_CANCEL_REASON:
				evt_info->vd.roam_cancel_reason = vtag_value;
				break;
			}
			break;
				    ...
				    struct slsi_rx_evt_info *evt_info)
{
	...
	case FAPI_EVENT_WIFI_EVENT_ROAM_SEARCH_STARTED:
		...
		slsi_conn_log2us_roam_scan_start(sdev, dev, roam_reason, evt_info->rssi_val,
		...
		SLSI_INFO(sdev, "WIFI_EVENT_ROAM_SEARCH_STARTED, Roaming Type : %s, RSSI:%d, Deauth Reason:0x%04x, "
			  ...
			  (evt_info->vd.roaming_type == 0 ? "Legacy" : "NCHO"), evt_info->rssi_val,
	...
	case FAPI_EVENT_WIFI_EVENT_FW_AUTH_SENT:
		...
		slsi_conn_log2us_auth_req(sdev, dev,
					  ...
					  evt_info->vd.is_roaming, evt_info->rssi_val);
		break;
	...
	case FAPI_EVENT_WIFI_EVENT_FW_ASSOC_STARTED:
		...
		slsi_conn_log2us_assoc_req(sdev, dev, evt_info->mac_addr, evt_info->vd.sn, evt_info->status_code,
					   evt_info->vd.mgmt_frame_subtype, evt_info->vd.mlo_band, evt_info->rssi_val);
		break;
	...
	case FAPI_EVENT_WIFI_EVENT_ROAM_SCAN_RESULT:
		dump_roam_scan_result(sdev, dev, &evt_info->current_info, evt_info->mac_addr,
				      evt_info->chan_frequency, evt_info->rssi_val, evt_info->vd.chan_utilisation,
				      evt_info->vd.score_val, evt_info->vd.tp_score_val, evt_info->vd.eligible_value,
				      evt_info->vd.mld_ap);
		break;
	case FAPI_EVENT_WIFI_EVENT_ROAM_SCAN_CANCELED:
		SLSI_INFO(sdev, "WIFI_EVENT_ROAM_SCAN_CANCELED, Reason:%d\n", evt_info->vd.roam_cancel_reason);
		slsi_conn_log2us_roam_cancelled(sdev, dev, evt_info->vd.roam_cancel_reason,
						slsi_get_roam_cancel_reason_str(evt_info->vd.roam_cancel_reason));
		break;
	...
	case FAPI_EVENT_WIFI_EVENT_FW_BTM_CANDIDATE:
		...
		break;
	case FAPI_EVENT_WIFI_EVENT_WTC_IE_RECEIVED:
		SLSI_INFO(sdev, "WIFI_EVENT_WTC_IE_RECEIVED (BTM Request) reason: %d subreason %d duration %d\n",
			  evt_info->vd.reason, evt_info->vd.subreason, evt_info->vd.ulw_duration);
		slsi_conn_log2us_btm_wtc(sdev, dev, evt_info->vd.reason, evt_info->vd.subreason,
					 evt_info->vd.ulw_duration);
		break;
	case FAPI_EVENT_WIFI_EVENT_WTC_IE_SENT:
		SLSI_INFO(sdev, "WIFI_EVENT_WTC_IE_SENT (BTM Request) reason: %d\n",
			  evt_info->vd.reason);
		slsi_conn_log2us_btm_wtc_sent(sdev, dev, evt_info->vd.reason);
		break;
	case FAPI_EVENT_WIFI_EVENT_FW_MLD_SETUP:
	case FAPI_EVENT_WIFI_EVENT_FW_MLD_RECONFIG:
	case FAPI_EVENT_WIFI_EVENT_FW_MLD_T2LM_STATUS:
...
static int slsi_acs_validate_width_hw_mode(struct slsi_acs_request *request)
{
	...
	if (request->ch_width != 20 && request->ch_width != 40 && request->ch_width != 80 &&
	    request->ch_width != 160 && request->ch_width != 320)
		return -EINVAL;
	return 0;
}
			      ...
			      int len, u32 **freq_list, int *freq_list_len)
{
	...
	nla_for_each_attr(attr, data, len, temp) {
		...
		case SLSI_ACS_ATTR_LINK_ID:
		{
			if (slsi_util_nla_get_s16(attr, &request->link_id))
				return -EINVAL;
			SLSI_INFO(sdev, "ACS MLO link_id: %d\n", request->link_id);
			break;
		}
		default:
...
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
static bool slsi_acs_is_vlp_channel(struct slsi_dev *sdev, u16 freq)
{
	int i;
	int cc_index = sdev->regdb.current_cc_index;
	struct regdb_file_reg_rules_collection *collection = sdev->regdb.country[cc_index].collection;

	for (i = 0; i < collection->reg_rule_num; i++) {
		if (freq >= collection->reg_rule[i]->freq_range->start_freq &&
		    freq <= collection->reg_rule[i]->freq_range->end_freq)
			break;
	}
	if (i == collection->reg_rule_num) {
		SLSI_DBG1_NODEV(SLSI_CFG80211, "No reg rule found for freq: %d, cc_index: %d\n",
				freq, cc_index);
		return false;
	}
	if (collection->reg_rule[i]->flags & SLSI_REGULATORY_6GHZ_VLP)
		return true;
			 ...
			 struct wireless_dev *wdev, const void *data, int len)
{
	...
	for (i = 0; i < freq_list_len; i++) {
		...
		if (!channels[num_channels]) {
			...
			SLSI_INFO(sdev, "Skip channel:%d for Reg_rule flag\n", channels[num_channels]->hw_value);
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
		} else if (channels[num_channels]->band == NL80211_BAND_6GHZ &&
			 slsi_acs_is_vlp_channel(sdev, freq_list[i]) && freq_list[i] <= 6105) {
			SLSI_INFO(sdev, "Skip channel:%d (not allowed under 6105MHz)\n",
				  channels[num_channels]->hw_value);
#endif
		} else {
			...
			if (idx >= 0 && idx < SLSI_MAX_CHAN_VALUE_ACS) {
				...
				SLSI_DBG1(sdev, SLSI_CFG80211, "Valid channel:%d for ACS\n",
					  channels[num_channels]->hw_value);
				num_channels++;
			}
		}
	...
	if (num_channels == 1) {
		...
		acs_selected_channels.link_id = request->link_id;
				 ...
				 u32 cnt, u32 iface_mode, u32 max_cnt)
{
	...
	if (iface_mode == SLSI_UC_ITERFACE_STA ||
	    iface_mode == SLSI_UC_ITERFACE_TDLS)
		chan_flags = IEEE80211_CHAN_DISABLED;
	if (iface_mode == SLSI_UC_ITERFACE_P2P_CLIENT)
		chan_flags = (IEEE80211_CHAN_DISABLED | IEEE80211_CHAN_RADAR);
	if (iface_mode == SLSI_UC_ITERFACE_NAN ||
	    iface_mode == SLSI_UC_ITERFACE_P2P_GO)
		chan_flags = (IEEE80211_CHAN_DISABLED | IEEE80211_CHAN_RADAR | IEEE80211_CHAN_NO_IR);
	...
	if (band == NL80211_BAND_6GHZ) {
		if (iface_mode == SLSI_UC_ITERFACE_NAN || iface_mode == SLSI_UC_ITERFACE_P2P_GO ||
		    iface_mode == SLSI_UC_ITERFACE_SOFTAP)
			reg_6g_supp_flags = SLSI_REGULATORY_6GHZ_VLP;
		if (iface_mode == SLSI_UC_ITERFACE_P2P_CLIENT)
			reg_6g_supp_flags = SLSI_REGULATORY_6GHZ_VLP | SLSI_REGULATORY_6GHZ_LPI;
		if (iface_mode == SLSI_UC_ITERFACE_STA || iface_mode == SLSI_UC_ITERFACE_TDLS)
			reg_6g_supp_flags = SLSI_REGULATORY_6GHZ_VLP | SLSI_REGULATORY_6GHZ_LPI | SLSI_REGULATORY_6GHZ_SP;
	...
	for (i = 0; i < chan_data->n_channels; i++) {
		...
		if (band == NL80211_BAND_6GHZ) {
			...
			if (j == collection->reg_rule_num) {
				SLSI_DBG1_NODEV(SLSI_CFG80211, "No reg rule found for freq: %d, cc_index: %d\n",
						center_freq, cc_index);
				continue;
			}
			if (!(collection->reg_rule[j]->flags & reg_6g_supp_flags))
				continue;
		}
#endif
		...
		if (channel->hw_value >= 169 && channel->hw_value <= 177) {
			if (iface_mode == SLSI_UC_ITERFACE_NAN ||
			    iface_mode == SLSI_UC_ITERFACE_P2P_GO ||
			    iface_mode == SLSI_UC_ITERFACE_P2P_CLIENT) {
				SLSI_DBG1_NODEV(SLSI_CFG80211, "Skip UNII-4 channel %d for P2P and NAN.\n",
						channel->hw_value);
				continue;
			}
		}
				    ...
				    struct wireless_dev *wdev, const void *data, int len)
{
	...
	if (request.band & SLSI_UC_MAC_6_BAND && chan_count < request.max_num) {
		if (request.iface_mode == SLSI_UC_ITERFACE_STA ||
		    request.iface_mode == SLSI_UC_ITERFACE_SOFTAP ||
		    request.iface_mode == SLSI_UC_ITERFACE_TDLS ||
		    (request.iface_mode == SLSI_UC_ITERFACE_NAN && sdev->nan_6g_supported))
			chan_count = slsi_uc_add_channels(wiphy, NL80211_BAND_6GHZ,
							  chan_list, chan_count,
							  request.iface_mode, request.max_num);
	}
#endif
...
static int slsi_set_dtim_config(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int len)
{
	...
	SLSI_INFO(sdev, "multiplier: %d, sta.vif_status: %d\n", multiplier, ndev_vif->sta.vif_status);
...
static const struct  nl80211_vendor_cmd_info slsi_vendor_events[] = {
	...
	{ OUI_GOOGLE,  SLSI_NL80211_NDP_REQ_EVENT},
	{ OUI_GOOGLE,  SLSI_NL80211_NDP_CFM_EVENT},
	{ OUI_GOOGLE,  SLSI_NL80211_NDP_END_EVENT},
...
slsi_wlan_vendor_acs_policy[SLSI_ACS_ATTR_MAX + 1] = {
	...
	[SLSI_ACS_ATTR_EHT_ENABLED] = {.type = NLA_FLAG },
	[SLSI_ACS_ATTR_LINK_ID] = {.type = NLA_S16},
...
static const struct nla_policy
slsi_wlan_vendor_conc_matrix_policy[SLSI_ATTRIBUTE_CONCURRENCY_MATRIX_MAX + 1] = {
	[SLSI_ATTRIBUTR_MAX_SET_SIZE] = {.type = NLA_U32},
};
...
static struct wiphy_vendor_command slsi_vendor_cmd[] = {
...

	{
		...
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = slsi_get_concurrency_matrix
	},
	{
		{
			.vendor_id = OUI_SAMSUNG,
			.subcmd = SLSI_NL80211_VENDOR_SUBCMD_GET_FEATURES
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = slsi_get_features_supported
	},
};
...
static void slsi_nll80211_vendor_init_policy(struct wiphy_vendor_command *slsi_vendor_cmd, int n_vendor_commands)
{
	...
	for (i = 0; i < n_vendor_commands; i++) {
		...
		case SLSI_NL80211_VENDOR_SUBCMD_KEY_MGMT_SET_KEY:
			...
			break;
#endif
		case SLSI_NL80211_VENDOR_SUBCMD_GET_FEATURES:
			vcmd->policy = VENDOR_CMD_RAW_DATA;
			vcmd->maxattr = 0;
			break;
		...
		case SLSI_NL80211_VENDOR_SUBCMD_SET_DTIM_CONFIG:
			...
			break;
		case SLSI_NL80211_VENDOR_SUBCMD_GET_CONCURRENCY_MATRIX:
			vcmd->policy = slsi_wlan_vendor_conc_matrix_policy;
			vcmd->maxattr = SLSI_ATTRIBUTE_CONCURRENCY_MATRIX_MAX;
			break;
#ifdef CONFIG_SCSC_WLAN_SAR_SUPPORTED
