...
#define  SLSI_IS_VIF_INDEX_MHS(sdev, ndev_vif) (((ndev_vif)->iftype == NL80211_IFTYPE_AP) &&\
...
#define SLSI_CONNECT_H2E_NOT_SUPPORTED  0x040b
#endif
...
#define TWT_TEARDOWN_SCAN_TRIGGERED                      8
...
enum slsi_vendor_attr_bw_changed_event {
	SLSI_VENDOR_ATTR_BW_CHANGED = 1
};
...
static inline int slsi_freq_to_band(u32 freq)
{
	if (freq < SLSI_5GHZ_MIN_FREQ)
		return SLSI_FREQ_BAND_2GHZ;

	if (freq <= SLSI_5GHZ_MAX_FREQ)
		return SLSI_FREQ_BAND_5GHZ;

	return SLSI_FREQ_BAND_6GHZ;
}
