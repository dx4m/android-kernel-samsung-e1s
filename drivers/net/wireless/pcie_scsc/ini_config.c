...
#define SLSI_MIB_2INDEX_INDEX1_POS           4
#define SLSI_MIB_2INDEX_INDEX2_POS           5
#define SLSI_MIB_2INDEX_MIBLEN_POS           7
#define SLSI_MIB_2INDEX_MIBDATA_START_POS    8
#define SLSI_MIB_PSID2295_TOTALLEN           12
#define SLSI_MIB_PSID2306_TOTALLEN           10
...
#define INI_CONFIG_FILE_PATH_LEGACY          "../firmware/wlan-connection-roaming.ini"
#define INI_CONFIG_FILE_BACKUP_PATH_LEGACY   "../firmware/wlan-connection-roaming-backup.ini"
...
static void set_mibs(struct slsi_dev *sdev)
{
	...
	for (j = 0; j < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; j++) {
		if ((sdev->ini_conf_struct.doubleindexmib[j].pos)) {
			...
			sdev->ini_conf_struct.doubleindexmib[j].pos = 0;
			kfree_skb(cfm);
		}
	}
	return;
...
static int encode_mib_with_ms_to_TU_convert(struct slsi_dev *sdev, u8 *buf, u16 *pos, u8 *value,
			       struct ini_lookup *lookup_entry)
{
	struct slsi_mib_data mib_data = { 0, NULL };
	int error                     = SLSI_MIB_STATUS_FAILURE;
	...
	res = slsi_str2int(value, (int*)&value_uint);
	if (res) {
		SLSI_ERR_NODEV("key_val is invalid for key %s\n", lookup_entry->key);
		return -EINVAL;
	}
	...
	value_uint *= lookup_entry->unit_con_factor;
	value_uint = (value_uint * 1000) / 1024;
	if (slsi_mib_encode_uint(&mib_data, lookup_entry->psid,
	    value_uint, index) != SLSI_MIB_STATUS_SUCCESS) {
		SLSI_ERR_NODEV("Failed to encode mib with key = %s and psid = %d\n",
			       lookup_entry->key, lookup_entry->psid);
					  ...
					  s8 octet_index, u16 psid, int unit_con_factor)
{
	...
	for (j = 0; j < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; j++) {
		if (psid == sdev->ini_conf_struct.doubleindexmib[j].psid &&
		    sdev->ini_conf_struct.doubleindexmib[j].pos) {
			buf = sdev->ini_conf_struct.doubleindexmib[j].ini_conf_buff;
			pos = &sdev->ini_conf_struct.doubleindexmib[j].pos;
			while (*pos > i + 6) {
				if (buf[i + SLSI_MIB_2INDEX_INDEX1_POS] == index1 &&
				    buf[i + SLSI_MIB_2INDEX_INDEX2_POS] == index2 &&
				    *pos >= i + sdev->ini_conf_struct.doubleindexmib[j].miblen) {
					if (octet_index < 0)
						octet_index =  buf[i + SLSI_MIB_2INDEX_MIBLEN_POS] - 1;
					buf[i + SLSI_MIB_2INDEX_MIBDATA_START_POS + octet_index] = value_int & 0xFF;
					return 0;
				}
				i += sdev->ini_conf_struct.doubleindexmib[j].miblen;
			}
		}
	}
	...
	for (j = 0; j < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; j++) {
		if (psid == sdev->ini_conf_struct.doubleindexmib[j].psid) {
			append_mibdata_to_buffer(sdev, sdev->ini_conf_struct.doubleindexmib[j].ini_conf_buff,
						 &sdev->ini_conf_struct.doubleindexmib[j].pos, mib_val_set.data,
...
struct ini_lookup slsi_ini_config_lookup_table[] = {
	...
	{.key = "RoamCU_24DefaultCU", .func_p = encode_uint_mib, .psid = 10035, .index = 1,
	 .unit_con_factor = 1, .min_value = 0, .max_value = 100},
	{.key = "RoamCU_5DefaultCU", .func_p = encode_uint_mib, .psid = 10035, .index = 2,
	 .unit_con_factor = 1, .min_value = 0, .max_value = 100},
	{.key = "RoamCU_6DefaultCU", .func_p = encode_uint_mib, .psid = 10035, .index = 3,
	 .unit_con_factor = 1, .min_value = 0, .max_value = 100},
	...
	{.key = "ConDTIMSkipping_Number", .func_p = encode_uint_mib, .psid = 2476,
	...
	{.key = "ConBeaconLoss_TimeoutOnSleep", .func_p = NULL, .psid = -100, .index = 0,
	 .unit_con_factor = 1, .min_value = 0, .max_value = 20},
	{.key = "ConDTIMSkipping_MaxTime", .func_p = encode_mib_with_ms_to_TU_convert, .psid = 2530,
	 .index = 0, .unit_con_factor = 1, .min_value = 0, .max_value = 2000},
				     ...
				     bool read_operation)
{
	...
	for (i = 0; i < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; i++) {
		if (!sdev->ini_conf_struct.doubleindexmib[i].ini_conf_buff)
			sdev->ini_conf_struct.doubleindexmib[i].ini_conf_buff = kmalloc(SLSI_INI_CONFIG_BUFF_SIZE, GFP_KERNEL);
		if (!sdev->ini_conf_struct.doubleindexmib[i].ini_conf_buff) {
			SLSI_ERR(sdev, "Couldn't allocate static buffer for ini configuration\n");
			return -ENOMEM;
		}
	...
	sdev->ini_conf_struct.ini_conf_buff_pos = 0;
	for (i = 0; i < SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB; i++)
		sdev->ini_conf_struct.doubleindexmib[i].pos = 0;
...
static void slsi_set_build_type(struct slsi_dev *sdev)
{
	struct slsi_mib_data mib_data = { 0, NULL };
	int                  result = 0;

#ifdef CONFIG_SCSC_USER_IMAGE
	bool obfuscate_user_info = true;
#else
	bool obfuscate_user_info = false;
#endif
	result = slsi_mib_encode_bool(&mib_data, SLSI_PSID_UNIFI_OBFUSCATE_USER_INFO, obfuscate_user_info, 0);
	...
	result = slsi_mlme_set(sdev, NULL, mib_data.data, mib_data.dataLength);
	kfree(mib_data.data);
...
exit:
	SLSI_ERR(sdev, "Error in setting user build type. error = %d\n", result);
}

static void slsi_process_ini_config_file(struct slsi_dev *sdev)
{
	...
	if (r) {
		r = slsi_load_ini_config_file(sdev, INI_CONFIG_FILE_PATH_LEGACY, &e);
	}
	...
	if (r) {
		r = slsi_load_ini_config_file(sdev, INI_CONFIG_FILE_BACKUP_PATH_LEGACY, &e);
	}
...
{
	slsi_set_build_type(sdev);
	slsi_set_uint_mib(sdev, NULL, SLSI_PSID_UNIFI_LAST_STA_CONNECTED_CHANNEL, sdev->sta_last_connected_chan_freq);
