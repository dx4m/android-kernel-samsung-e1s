...
#include "cfg80211_ops.h"
	...
	(defined(CONFIG_SCSC_WLAN_TAS) && defined(CONFIG_SCSC_WLAN_DCTAS)) || \
	(defined(CONFIG_SCSC_WLAN_ICTAS) && defined(CONFIG_SCSC_WLAN_DCTAS))
/**
 * From a long-term perspective, CONFIG_SCSC_WLAN_TAS should eventually be deprecated.
 * Ensure that configurations do not increase further
 */
#error "Only one TAS configuration should be selected"
#endif
...
#define SLSI_HOSTSTATE_SUSPEND_DISALLOWED   0x0001
...
#define SLSI_HOSTSTATE_LCD_ACTIVE           0x1000
...
#define SLSI_NUM_DOUBLEINDEX_MULTIOCTECT_MIB 2
...
#define SLSI_MAX_RADIO_SUPPORTED 4
#else
#define SLSI_MAX_RADIO_SUPPORTED 2
#endif
...
struct slsi_peer {
	...
	u16                       last_sn[NUM_BA_SESSIONS_PER_PEER];
...
struct slsi_vif_sta {
	...
	struct {
		...
		struct slsi_lls_iface_data iface_stat;
	} links[MAX_NUM_MLD_LINKS];
	struct slsi_peer link_peer_sta_record[MAX_SUPP_MLO_LINKS];
	u8 sta_link_addr[MAX_SUPP_MLO_LINKS][ETH_ALEN];
	u8 sec_ap_link_addr[MAX_SUPP_MLO_LINKS][ETH_ALEN];
	bool mlo_forced_bandcontrol;
#else
	struct slsi_dynamic_bw     dynamic_bw;
#endif
	struct slsi_lls_iface_data iface_stat;
...
struct netdev_vif {
	...
	u32			    latency_mode;
	unsigned long		    is_napi_enabled;
	rwlock_t		    netif_napi_lock;
	void			    *netif_napi_client;
	struct napi_ctrl_info       *napi_info;
	u32 			    use_netif_napi;
#endif
...
struct ini_conf {
	...
	struct ini_doubleindex_multioctet_mib doubleindexmib[2];
...
struct slsi_dev {
	...
	struct work_struct resume_work;	/* Work on wlan resume */
	...
	struct slsi_wake_lock			wlan_wl_resume;
#endif
	struct slsi_sig_send       sig_wait;
...
#endif
#if defined(CONFIG_SLSI_WLAN_CP_COEX)
	struct notifier_block      ril_notifier;
	struct slsi_mutex          cp_coex_mutex;
	struct cp_noti_cell_infos  cp_cell_infos;
#endif
 ...
 */
	...
	struct slsi_lls_radio_stat *radio_stat[SLSI_MAX_RADIO_SUPPORTED];
	u32                        tdls_offchannel;
#ifdef CONFIG_SCSC_WLAN_MUTEX_DEBUG
	...
	struct work_struct        nan_data_interface_create_delete_work;
	struct list_head          nan_data_interface_create_delete_data;
...
#if defined(CONFIG_SLSI_WLAN_DCTAS)
	struct slsi_dctas          dctas;
	struct slsi_mutex	   dctas_mutex;
#endif
...
u16 slsi_get_vifidx_frm_linkid(struct net_device *dev, u16 link_id);
