...
static void test_slsi_procfs_tput_read(struct kunit *test)
{
	...
	ndev_vif->tput_tx_bytes_per_sec = 500;
	ndev_vif->tput_rx_bytes_per_sec = 500;
	KUNIT_EXPECT_NE(test, 0, slsi_procfs_tput_read(fp, user_buf, 10, &ppos));

	ndev_vif->tput_tx_bytes_per_sec = 1000;
	ndev_vif->tput_rx_bytes_per_sec = 1000;
	KUNIT_EXPECT_NE(test, 0, slsi_procfs_tput_read(fp, user_buf, 10, &ppos));

	ndev_vif->tput_tx_bytes_per_sec = 1000 * 1000;
	ndev_vif->tput_rx_bytes_per_sec = 1000 * 1000;
	KUNIT_EXPECT_NE(test, 0, slsi_procfs_tput_read(fp, user_buf, 10, &ppos));
}
