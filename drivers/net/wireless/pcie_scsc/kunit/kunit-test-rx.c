...
static void test_slsi_mbssid_to_ssid_list(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_mbssid_to_ssid_list(sdev, dev, scan_ssid, ssid_len,
...
static void test_slsi_extract_mbssids(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_extract_mbssids(sdev, dev, mgmt, skb, 1));
}
...
static void test_slsi_populate_ssid_info(struct kunit *test)
{
	...
	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	ndev_vif->scan[0].scan_results = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);
	ndev_vif->scan[1].scan_results = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);
	ndev_vif->scan[0].scan_results->akm_type = 1;
	ndev_vif->scan[1].scan_results->akm_type = 1;
	ndev_vif->scan[0].scan_results->next = ndev_vif->scan[1].scan_results;

	skb = fapi_alloc(mlme_scan_ind, MLME_SCAN_DONE_IND, ndev_vif->ifnum, 52);
	ndev_vif->scan[0].scan_results->beacon = skb;
	ndev_vif->scan[1].scan_results->probe_resp = skb;
	strcpy((ndev_vif->scan[1].scan_results)->ssid, "kunit_ssid");
	(ndev_vif->scan[0].scan_results)->ssid_length = 0;
	(ndev_vif->scan[1].scan_results)->ssid_length = strlen((ndev_vif->scan[1].scan_results)->ssid);
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_populate_ssid_info(sdev, dev, 0));

	list_for_each(pos, &ndev_vif->sta.ssid_info) {
		new_ssid_info = list_entry(pos, struct slsi_ssid_info, list);
		if (SLSI_ETHER_EQUAL(new_ssid_info->ssid.ssid, ndev_vif->scan[1].scan_results->ssid)) {
			list_del(&new_ssid_info->list);
			kfree(new_ssid_info);
			break;
		}
	}
...
static void test_slsi_set_band_any_auto_channel(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 33, acs_selected_channels.sec_channel);
...
static void test_slsi_rx_connect_ind(struct kunit *test)
{
	...
	memcpy(mgmt->bssid, "\x00\x0c\x0e\x54\x35\x35", 6);
	skb_scan = fapi_alloc(mlme_synchronised_ind, MLME_SYNCHRONISED_IND, 0, 1);
	memcpy(mgmt_sync_ind->bssid, "\x00\x0c\x0e\x54\x35\x35", 6);
	fapi_append_data(skb_scan, (u8 *)mgmt_sync_ind, sizeof(struct ieee80211_mgmt));
	skb_queue_tail(&ndev_vif->sta.scan_inds, skb_scan);
	slsi_rx_connect_ind(sdev, dev, skb);
