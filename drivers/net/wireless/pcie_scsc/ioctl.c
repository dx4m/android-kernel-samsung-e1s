...
#ifndef CONFIG_SLSI_WLAN_PACKET_FILETR_V2
...
#define CMD_SET_MAX_BANDWIDTH "SET_MAX_BANDWIDTH"
#define CMD_GET_MAX_BANDWIDTH "GET_MAX_BANDWIDTH"
#define CMD_ENABLE_24G_HE40 "ENABLE_24G_HE40"
...
#define CMD_GET_ML_LINK_STATE         "GET_ML_LINK_STATE"
#define CMD_ML_LINK_STATE_CONTROL "ML_LINK_STATE_CONTROL"
#define CMD_GET_ML_TID_MAPPING_STATUS "GET_ML_TID_MAPPING_STATUS"
#define CMD_MEASURE_ML_CHANNEL_CONDITION "MEASURE_ML_CHANNEL_CONDITION"
#define CMD_ML_TID_MAPPING_REQUEST "ML_TID_MAPPING_REQUEST"
#endif
...
#define CMD_TEST_STARTCAPTURE   "TEST_STARTCAPTURE"
#define CMD_TEST_STOPCAPTURE    "TEST_STOPCAPTURE"
...
#define SLSI_SCHED_PM_MIN_DESIRED_DURATION 8192
#define SLSI_SCHED_PM_MAX_DESIRED_DURATION 64512

#define SLSI_SCHED_PM_MIN_DESIRED_INTERVAL 16384
#define SLSI_SCHED_PM_MAX_DESIRED_INTERVAL 1024000
...
#define SLSI_TX_POWER_BAND_NOT_SUPPORT   0xff
#define SLSI_TX_POWER_NO_PPDU_TRANS      0x80
...
static bool slsi_is_sta_connected(struct netdev_vif *ndev_vif)
{
...
static int slsi_set_suspend_mode(struct net_device *dev, char *command, int cmd_len)
{
	...
	if (user_suspend_mode != previous_suspend_mode) {
		...
		for (vif = 1; vif <= CONFIG_SCSC_WLAN_MAX_INTERFACES; vif++) {
			...
			if (ndev_vif->activated &&
				...
				if (ret != 0)
					SLSI_NET_ERR(dev, "Error in updating /clearing the packet filters,ret=%d\n", ret);
			}
	...
	if (!sdev->device_config.user_suspend_mode)
		host_state = host_state | SLSI_HOSTSTATE_SUSPEND_DISALLOWED;
	else
		host_state = host_state & ~SLSI_HOSTSTATE_SUSPEND_DISALLOWED;
...
static int slsi_delete_interface(struct net_device *dev, char *buffer, int buf_len)
{
	...
	sdev->netdev_ap = NULL;
	SLSI_DBG1_NODEV(SLSI_MLME, "Successfully deleted AP interface %s\n", intf_name);
...
static int slsi_reassoc_frequency_write_legacy(struct net_device *dev, char *command, int buf_len)
{
	...
	if (SLSI_ETHER_EQUAL(bssid, ndev_vif->sta.bssid)) {
		reason_code = 2;
		reason_desc = "REASSOC_FREQUENCY_LEGACY to the already associated bssid";
		slsi_conn_log2us_roam_cancelled(sdev, dev, reason_code, reason_desc);
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		kfree(ioctl_args);
		return -EPERM;
	}
...
static int slsi_reassoc_frequency_write(struct net_device *dev, char *command, int buf_len)
{
	...
	if (SLSI_ETHER_EQUAL(bssid, ndev_vif->sta.bssid)) {
                reason_code = 2;
                reason_desc = "REASSOC_FREQUENCY to the already associated bssid";
                slsi_conn_log2us_roam_cancelled(sdev, dev, reason_code, reason_desc);
                SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
                kfree(ioctl_args);
                return -EPERM;
...
static int slsi_roam_offload_ap_list(struct net_device *dev, char *command, int buf_len)
{
	...
	for (i = 1; i <= ap_count; i++) {
		...
		SLSI_DBG3_NODEV(SLSI_MLME, "[" MACSTR "]\n", MAC2STR(mac_acl->mac_addrs[i].addr));
	}
	mac_acl->acl_policy = NL80211_ACL_POLICY_DENY_UNLESS_LISTED;
	mac_acl->n_acl_entries = ap_count;

	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	r = slsi_mlme_set_acl(sdev, dev, ndev_vif->vifnum, mac_acl->acl_policy, mac_acl->n_acl_entries, mac_acl->mac_addrs);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	kfree(mac_acl);
	kfree(ioctl_args);
	return r;
...
static int slsi_roam_scan_frequencies_write(struct net_device *dev, char *command, int buf_len)
{
	...
	/* Enable the unifiRoamNchoScanControl MIB for SETROAMSCANFREQUENCIES Command. */
	result = slsi_set_mib_roam(sdev, NULL, SLSI_PSID_UNIFI_ROAM_NCHO_SCAN_CONTROL, 1);
	if (result != SLSI_MIB_STATUS_SUCCESS) {
		result = -EINVAL;
		goto exit;
	}

	result = slsi_mlme_set_cached_channels(sdev, dev, sdev->device_config.wes_roam_scan_list.n,
...
static int slsi_roam_add_scan_frequencies(struct net_device *dev, char *command, int buf_len)
{
	...
	/* Disable the unifiRoamNchoScanControl MIB for ADDROAMSCANFREQUENCIES Command. */
	result = slsi_set_mib_roam(sdev, NULL, SLSI_PSID_UNIFI_ROAM_NCHO_SCAN_CONTROL, 0);
	if (result != SLSI_MIB_STATUS_SUCCESS) {
		result = -EINVAL;
		goto exit;
	}
...
static int slsi_set_tx_ant_config(struct net_device *dev, char *command, int buf_len)
{
	...
	if (cfg < 0 || cfg > 5) {
		SLSI_ERR(sdev, "Invalid Tx antenna config: Must be between 0 and 5, cfg = %d\n", cfg);
		kfree(ioctl_args);
		return -EINVAL;
	}
...
static int slsi_set_ncho_mode(struct net_device *dev, char *command, int buf_len)
{
	...
	SLSI_DBG1_NODEV(SLSI_MLME, "mlme_set_roaming_type_req(vif:%u mode:%u)\n", ndev_vif->vifnum, mode);

	req = fapi_alloc(mlme_set_roaming_type_req, MLME_SET_ROAMING_TYPE_REQ, ndev_vif->vifnum, 0);
	if (!req) {
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		kfree(ioctl_args);
		return -EIO;
	}
	fapi_set_u16(req, u.mlme_set_roaming_type_req.vif, ndev_vif->vifnum);
	...
	/* On NCHO Mode reset, resetting the supported roam band value to its default value. */
	if (!mode) {
...
static int slsi_set_keepalive_parameters(struct net_device *dev, char *command, int buf_len)
{
	struct netdev_vif         *ndev_vif = netdev_priv(dev);
	struct slsi_dev	          *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args    *ioctl_args = NULL;
	...
	if (slsi_str2int(ioctl_args->args[0], &keepalive_time)) {
		SLSI_ERR(sdev, "keepalive_time: failed to read from string: '%s'\n", ioctl_args->args[0]);
		result = -EINVAL;
		goto exit;
	}

	if (keepalive_time > SLSI_MAX_KEEP_ALIVE_INTERVAL || keepalive_time < SLSI_MIN_KEEP_ALIVE_INTERVAL) {
		SLSI_ERR(sdev, "Invalid arguments = %d\n", ioctl_args->args[0]);
		result = -EINVAL;
		goto exit;
	}
	...
	result = slsi_mlme_set_keepalive_parameters(sdev, dev, keepalive_time);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
exit:
	kfree(ioctl_args);
	return result;
...
static int slsi_set_pmk(struct net_device *dev, char *command, int buf_len)
{
	...
	if (!ndev_vif->activated) {
		SLSI_NET_ERR(dev, "Vif is not activated\n");
		kfree(ioctl_args);
		return -EINVAL;
	}
...
int slsi_set_tx_power_calling_cat2(struct net_device *dev, char *command, int buf_len)
{
	...
	if (slsi_str2int(ioctl_args->args[0], &dsi_id)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[0]);
		kfree(ioctl_args);
		return -EINVAL;
	}
...
static inline s8 slsi_get_tx_power_value(u8 qdb_val)
{
	if (qdb_val == SLSI_TX_POWER_BAND_NOT_SUPPORT)
		return -1;
...
int slsi_get_tx_power_calling(struct net_device *dev, char *command, int buf_len)
{
	...
	for (i = 0; i < 2; i++) {
		cur_pos += snprintf(command + cur_pos, buf_len - cur_pos, " %d %d %d",
				    slsi_get_tx_power_value(limits[i].antenna1_2g4),
				    slsi_get_tx_power_value(limits[i].antenna2_2g4),
				    slsi_get_tx_power_value(limits[i].antenna1_2_2g4));
		cur_pos += snprintf(command + cur_pos, buf_len - cur_pos, " %d %d %d",
				    slsi_get_tx_power_value(limits[i].antenna1_5g),
				    slsi_get_tx_power_value(limits[i].antenna2_5g),
				    slsi_get_tx_power_value(limits[i].antenna1_2_5g));
		cur_pos += snprintf(command + cur_pos, buf_len - cur_pos, " %d %d %d",
				    slsi_get_tx_power_value(limits[i].antenna1_6g),
				    slsi_get_tx_power_value(limits[i].antenna2_6g),
				    slsi_get_tx_power_value(limits[i].antenna1_2_6g));
	}
...
int slsi_get_avg_tx_power(struct net_device *dev, char *command, int buf_len)
{
	...
	if (slsi_str2int(ioctl_args->args[1], &report_mode)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[1]);
		kfree(ioctl_args);
		return -EINVAL;
	}
...
#define SLSI_GET_AVG_TX_POWER(x, shift) (((x) >> (shift)) & 0x3)
		...
		cur_pos += snprintf(command + cur_pos, buf_len - cur_pos, "%d", avg_tx_pwr.long_window);
	} else {
...
static inline s8 slsi_get_max_tx_power_value(u8 qdb_val)
{
	if (qdb_val == SLSI_TX_POWER_NO_PPDU_TRANS)
		return 0;

	return (s8)qdb_val;
}

static inline s8 slsi_get_avg_tx_power_value(u8 qdb_val)
{
#if defined(CONFIG_SCSC_WLAN_TAS) || defined(CONFIG_SCSC_WLAN_ICTAS)
	return slsi_get_max_tx_power_value(qdb_val);
#else
	return -1;
#endif
}

int slsi_get_max_tx_power(struct net_device *dev, char *command, int buf_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev *sdev = ndev_vif->sdev;
	struct slsi_sar_max_tx_power max_tx_pwr = {0};
	int ret = 0, cur_pos = 0;

	ret = slsi_mlme_sar_get_max_tx_power(sdev, dev, &max_tx_pwr);
	if (ret)
		return -EIO;

	cur_pos = snprintf(command, buf_len, "%d %d %d %d %d %d %d %d",
			   (s8)max_tx_pwr.antenna1non_dbs_power_limit,
			   (s8)max_tx_pwr.antenna2non_dbs_power_limit,
			   (s8)max_tx_pwr.antenna1dbs_power_limit,
			   (s8)max_tx_pwr.antenna2dbs_power_limit,
			   slsi_get_max_tx_power_value(max_tx_pwr.antenna1last_tx_power),
			   slsi_get_max_tx_power_value(max_tx_pwr.antenna2last_tx_power),
			   slsi_get_avg_tx_power_value(max_tx_pwr.antenna1average_tx_power),
			   slsi_get_avg_tx_power_value(max_tx_pwr.antenna2average_tx_power));
				       ...
				       struct slsi_sub_band *sub_bands, int sub_bands_max)
{
	...
	if (values[1].type != SLSI_MIB_TYPE_OCTET) {
		SLSI_ERR(sdev, "6GHz: Supported_Chan invalid type %d\n", values[1].type);
		goto exit_with_values;
	}
...
int slsi_get_sta_info(struct net_device *dev, char *command, int buf_len)
{
	...
	if (!SLSI_IS_VIF_INDEX_MHS(sdev, ndev_vif)) {
		SLSI_ERR(sdev, "ndev index %d is not for AP mode\n", ndev_vif->ifnum);
		return -EINVAL;
	}
	...
	if (!ndev_vif->activated || ndev_vif->vif_type != FAPI_VIFTYPE_AP) {
		SLSI_ERR(sdev, "AP is not up.Command not allowed vif.activated:%d, vif.type:%d\n",
			 ndev_vif->activated, ndev_vif->vif_type);
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		return -EINVAL;
	}
	...
	if (strcmp("ALL", ioctl_args->args[0]) == 0) {
		...
		for (i = 0; i < SLSI_PEER_INDEX_MAX; i++) {
			if (ndev_vif->peer_sta_record[i] && ndev_vif->peer_sta_record[i]->valid &&
				...
				for (j = 0; j < SLSI_MAX_NUM_SUPPORTED_RATES; j++) {
					sta_info_all.tx_rate_stats[j] += sta_info.tx_rate_stats[j];
					sta_info_all.rx_rate_stats[j] += sta_info.rx_rate_stats[j];
				}
				if (sta_info_all.support_mode < sta_info.support_mode)
					sta_info_all.support_mode = sta_info.support_mode;
				memset(&sta_info, 0, sizeof(sta_info));
			}
		}
...
static int slsi_get_cu(struct net_device *dev, char *command, int buf_len)
{
	...
	if (ndev_vif->sta.valid_links) {
		...
		for (i = 0; i < MAX_NUM_MLD_LINKS; i++) {
			...
			vif_idx = slsi_get_vifidx_frm_linkid(dev, i);

			res = slsi_get_beacon_cu(sdev, dev, &mib_value, vif_idx);
			if (res)
				goto exit;

			pos += snprintf(command + pos, buf_len - pos, "%d:%d ", i, mib_value);
		}

		pos += snprintf(command + pos, buf_len - pos, "\n");
		res = pos;
	...
	vif_idx = slsi_get_vifidx_frm_linkid(dev, link_id);
	res = slsi_get_beacon_cu(sdev, dev, &mib_value, vif_idx);
...
exit:
	SLSI_NET_INFO(dev, "res :%d command %s\n", res, command);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	return res;
}
...
int slsi_set_bandwidth(struct net_device *dev, char *command, int cmd_len)
{
	...
	if (ret != 0)
		SLSI_NET_ERR(dev, "Error in setting the Country, ret=%d\n", ret);
...
int slsi_set_bss_bandwidth(struct net_device *dev, char *command, int cmd_len)
{
	...
	SLSI_NET_DBG1(dev, SLSI_MLME, "mlme_set_bss_max_channel_width_req(vif:%u, bw:%d, protection_scope:0x%x)\n",
		      ndev_vif->vifnum, bandwidth, protection_scope);
	...
	fapi_set_u16(req, u.mlme_set_bss_max_channel_width_req.reserved, protection_scope);
...
static int slsi_twt_get_status(struct net_device *dev, char *command, int command_len)
{
	...
	ret = slsi_verify_ioctl_args(sdev, ioctl_args);
	if (ret) {
		SLSI_ERR(sdev, "Invalid arguments\n");
		return ret;
	}
...
static int slsi_get_sr_statistics(struct net_device *dev, char *command, int buf_len)
{
	...
	for (i = 0; i < ARRAY_SIZE(get_values); i++) {
		if (values[i].type != SLSI_MIB_TYPE_UINT) {
			SLSI_ERR(sdev, "invalid SR Statistics type.(%d)\n", i);
			goto exit_with_values;
		}
	}
...
static int slsi_sched_pm_setup(struct net_device *dev, char *command, int buf_len)
{
	...
	sched_pm_setup.grace_period = ndev_vif->grace_period;
...
static int slsi_set_ap_rps_params(struct net_device *dev, char *command, int buf_len)
{
	...
	if (!sap_ndev_vif->activated || sap_ndev_vif->vif_type != FAPI_VIFTYPE_AP ||
		...
		SLSI_ERR(sdev,
			 "AP is not up.Command not allowed vif.activated:%d, vif.type:%d, nl80211.vif.type:%d\n",
...
static int slsi_set_ap_rps(struct net_device *dev, char *command, int buf_len)
{
	...
	result = slsi_mlme_set_ap_rps(sdev, dev, rps_enable, sap_ndev_vif->vifnum);
...
static int slsi_set_ap_tx_power(struct net_device *dev, char *command, int buf_len)
{
	...
	result = slsi_mlme_set_ap_tx_power(sdev, dev, tx_power, ndev_vif->vifnum);
...
static int slsi_get_ap_tx_power(struct net_device *dev, char *command, int buf_len)
{
	...
	result = slsi_mlme_get_ap_tx_power(sdev, dev, command, buf_len, ndev_vif->vifnum);
...
static int slsi_get_tdls_wider_bandwidth(struct net_device *dev, char *command, int buf_len)
{
	...
	struct slsi_mib_data      mibrsp = { 0, NULL };
	struct slsi_mib_value     *values = NULL;
	unsigned int              tdls_wider_bw_supported;
	struct slsi_mib_get_entry get_values[] = {{SLSI_PSID_UNIFI_TDLS_WIDER_BANDWIDTH_PROHIBITED, { 0, 0 } } };
	int                       res = -EINVAL;

	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	if (!ndev_vif->activated) {
		SLSI_DBG3(sdev, SLSI_MLME, "vif not activated. return CMD_GET_TDLS_WIDER_BW to 1 as default\n");
		res = snprintf(command, buf_len, "%s %d\n", CMD_GET_TDLS_WIDER_BW, 1);
		goto exit_with_error;
	}
	...
	tdls_wider_bw_supported = !(values[0].u.boolValue);
	SLSI_DBG3(sdev, SLSI_MLME, "TDLS WIDER BAND SUPPORTED = %d\n", tdls_wider_bw_supported);
	res = snprintf(command, buf_len, "%s %d\n", CMD_GET_TDLS_WIDER_BW, tdls_wider_bw_supported);
...
int slsi_get_elna_status(struct net_device *dev, char *command, int buf_len)
{
	...
	elna_vif = netdev_priv(elna_net_device);
	SLSI_MUTEX_LOCK(elna_vif->vif_mutex);
	if (!elna_vif->activated || elna_vif->iftype != NL80211_IFTYPE_STATION) {
		elna_status = 2;
		goto exit_print;
	}
...
#ifdef CONFIG_SCSC_WLAN_EHT
static int slsi_set_wifi7_enable(struct net_device *dev, char *command, int cmd_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args *ioctl_args;
	int status, ret = -EINVAL;
	unsigned int wifi7enable = 0;
	...
	if (slsi_str2int(ioctl_args->args[0], &wifi7enable)) {
		SLSI_ERR(sdev, "Invalid Wifi7 Enable Param '%s'\n", ioctl_args->args[0]);
		goto exit;
	}
	if (wifi7enable > 1) {
		SLSI_ERR(sdev, "Invalid Wifi7enable value '%d'\n", wifi7enable);
		goto exit;
	}
	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	ret = slsi_set_boolean_mib(sdev, NULL, SLSI_PSID_UNIFI_EHT_ACTIVATED, wifi7enable, 0);
...
static int slsi_set_num_allowed_mlo_link(struct net_device *dev, char *command, int cmd_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args *ioctl_args;
	int status, ret = -EINVAL;
	unsigned int num_mlo_link = 0;
	...
	if (slsi_str2int(ioctl_args->args[0], &num_mlo_link)) {
		SLSI_ERR(sdev, "Invalid num_mlo_link  Param '%s'\n", ioctl_args->args[0]);
		goto exit;
	}
	if (num_mlo_link > 3) {
		SLSI_ERR(sdev, "Invalid num_mlo_link value '%d'\n", num_mlo_link);
		goto exit;
	}
	/* @param num_mlo_link 0: reset the number of allowed link by default value */
	if (num_mlo_link == 0)
		num_mlo_link = 2;
	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	ret = slsi_set_uint_mib(sdev, NULL, SLSI_PSID_UNIFI_MULTILINK_NUMBER_OF_LINKS, num_mlo_link);
	if (!ret)
		ndev_vif->sta.max_ml_link = num_mlo_link;
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
exit:
	kfree(ioctl_args);
	return ret;
...
static int slsi_get_ml_link_state(struct net_device *dev, char *command, int command_len)
{
	struct netdev_vif      *ndev_vif = netdev_priv(dev);
	struct slsi_dev        *sdev = ndev_vif->sdev;
	int                    ret = SLSI_WIFI_MLO_ERROR_INVALID_ARGUMENT;

	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	if (!sdev->fw_sta_eht_supported) {
		SLSI_ERR(sdev, "fw doesn't support sta eht: %d\n", sdev->fw_sta_eht_supported);
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		ret = SLSI_WIFI_MLO_ERROR_NOT_SUPPORTED;
		return ret;
	}
	if (!ndev_vif->activated || ndev_vif->vif_type != FAPI_VIFTYPE_STATION
	    || ndev_vif->sta.vif_status != SLSI_VIF_STATUS_CONNECTED
	    || (ndev_vif->sta.valid_links == 0)) {
		SLSI_NET_ERR(dev, "Not a STA vif or status is not CONNECTED or no active mlo link id present\n");
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		ret = SLSI_WIFI_MLO_ERROR_NOT_AVAILABLE;
		return ret;
	}
	ret = slsi_mlme_ml_link_state_query(sdev, dev, command, command_len);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	return ret;
}

static int slsi_ml_link_state_control(struct net_device *dev, char *command, int buf_len)
{
	struct netdev_vif      *ndev_vif = netdev_priv(dev);
	struct slsi_dev        *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args *ioctl_args = NULL;
	unsigned int           control_mode;
	unsigned int           num_active_links = 0;
	u16                    active_links_bitmap = 0;
	unsigned int           link_id, link_state;
	int                    r, ret = SLSI_WIFI_MLO_ERROR_INVALID_ARGUMENT;
	int                    index = 1;
	int                    max_arg_count = MAX_NUM_MLD_LINKS * 2 + 1;
	u8                     num_cmd_links = 0;
	u8                     num_valid_links = 0;

	ioctl_args = slsi_get_private_command_args(command, buf_len, max_arg_count);
	r = slsi_verify_ioctl_args(sdev, ioctl_args);
	if (r)
		return ret;
	...
	if (slsi_str2int(ioctl_args->args[0], &control_mode)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[0]);
		goto exit;
	}
	if (control_mode > 2) {
		SLSI_ERR(sdev, "Invalid control mode: '%d'\n", control_mode);
		goto exit;
	}

	num_valid_links = hweight16(ndev_vif->sta.valid_links);

	if (control_mode == 1) {
		if (ioctl_args->arg_count % 2 == 0) {
			SLSI_ERR(sdev, "Invalid arguments\n");
			goto exit;
		}

		while (ioctl_args->arg_count >= (index + 2)) {
			if (slsi_str2int(ioctl_args->args[index++], &link_id)) {
				SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[index - 1]);
				goto exit;
			}
			if (slsi_str2int(ioctl_args->args[index++], &link_state)) {
				SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[index - 1]);
				goto exit;
			}
			if (link_id >= MAX_NUM_MLD_LINKS || link_state > 1) {
				SLSI_ERR(sdev, "Invalid arguments\n");
				goto exit;
			}
			if (!(ndev_vif->sta.valid_links & BIT(link_id))) {
				SLSI_ERR(sdev, "No active connection present with Link Id: '%s'\n", link_id);
				goto exit;
			}
			if (link_state == 1)
				active_links_bitmap |= (1 << link_id);

			num_cmd_links++;
		}

		if (num_cmd_links != num_valid_links) {
			SLSI_ERR(sdev, "Unexpected CMD. No. of links provided %d doesn't match connection links %d\n",
				 num_cmd_links, num_valid_links);
			goto exit;
		}

		if (!active_links_bitmap) {
			SLSI_ERR(sdev, "Cannot make all the links inactive\n");
			goto exit;
		}
		...
		if (slsi_str2int(ioctl_args->args[1], &num_active_links)) {
			SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[1]);
			goto exit;
		}
		if (num_active_links < 1 || num_active_links > 2) {
			SLSI_ERR(sdev, "Invalid number of active links: '%d'\n", num_active_links);
			goto exit;
		}
		if (num_valid_links < num_active_links) {
			SLSI_ERR(sdev, "Invalid active links: '%d' for mlo connection, valid links %d\n",
				 num_active_links, hweight16(ndev_vif->sta.valid_links));
			goto exit;
		}
	}
	ret = slsi_mlme_ml_link_state_control(sdev, dev, control_mode, num_active_links,
					      active_links_bitmap);
exit:
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	kfree(ioctl_args);
...
static int slsi_get_ml_tid_mapping_status(struct net_device *dev, char *command, int command_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev   *sdev = ndev_vif->sdev;
	int               ret = SLSI_WIFI_MLO_ERROR_INVALID_ARGUMENT;
	...
	ret = slsi_mlme_get_ml_tid_mapping_status(sdev, dev, command, command_len);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	return ret;
}

static int slsi_measure_ml_channel_condition(struct net_device *dev, char *command, int command_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct slsi_dev   *sdev = ndev_vif->sdev;
	int               ret = SLSI_WIFI_MLO_ERROR_INVALID_ARGUMENT;
	...
	ret = slsi_mlme_get_measure_ml_channel_condition(sdev, dev);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	return ret;
}

static int slsi_ml_tid_mapping_request(struct net_device *dev, char *command, int buf_len)
{
	struct netdev_vif      *ndev_vif = netdev_priv(dev);
	struct slsi_dev        *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args *ioctl_args = NULL;
	int                    r, ret = SLSI_WIFI_MLO_ERROR_INVALID_ARGUMENT;
	unsigned int           default_mapping, link_id;
	int                    max_arg_count = MAX_NUM_MLD_LINKS * 3 + 1;
	struct ttlm_element    ttlm_list[MAX_NUM_MLD_LINKS];
	int                    index = 0, tid, num_links = 0;
	...
	if (slsi_str2int(ioctl_args->args[index++], &default_mapping)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[index - 1]);
		goto exit;
	}
	if (default_mapping > 1 || (ioctl_args->arg_count % 3 != 1)) {
		SLSI_ERR(sdev, "Invalid argument\n");
		goto exit;
	}
	if (default_mapping == 0) {
		while (index + 2 < ioctl_args->arg_count) {
			if (slsi_str2int(ioctl_args->args[index++], &link_id)) {
				SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[index - 1]);
				goto exit;
			}
			ttlm_list[num_links].link_id = (u16)link_id;
			if (!(ndev_vif->sta.valid_links & BIT(ttlm_list[num_links].link_id)) ||
			    ttlm_list[num_links].link_id > 14) {
				SLSI_ERR(sdev, "Invalid or No active connection present with Link Id: %d\n",
					 ttlm_list[num_links].link_id);
				goto exit;
			}
			tid = slsi_binary_string_to_integer(ioctl_args->args[index++]);
			if (tid < 0) {
				SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[index - 1]);
				goto exit;
			}
			ttlm_list[num_links].downlink_tid = tid;
			...
			ttlm_list[num_links].uplink_tid = tid;
			num_links++;
		}
	}
	ret = slsi_mlme_ml_tid_mapping_request(sdev, dev, default_mapping, ttlm_list, num_links);
exit:
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	kfree(ioctl_args);
	return ret;
...
static int slsi_get_wifi6e_channels(struct net_device *dev, char *buf, int buf_len)
{
	...
	SLSI_INFO(sdev, "get_wifi6e_channels for power scheme: %d\n", pwr_mode);
...
static int slsi_set_wifi_safe_mode(struct net_device *dev, char *command, int cmd_len)
{
	...
	/* Priority order is RF test mode > Airplane mode > Wi-Fi Safe Mode */
	SLSI_MUTEX_LOCK(sdev->device_config_mutex);
	if (slsi_is_rf_test_mode_enabled() || !(sdev->device_config.host_state & SLSI_HOSTSTATE_CELLULAR_ACTIVE)) {
		SLSI_INFO(sdev, "Ignore SETWSECINFO configuration in RF test mode / Airplane mode\n");
		SLSI_MUTEX_UNLOCK(sdev->device_config_mutex);
		ret = 0;
		goto err;
	}
...
#elif defined(CONFIG_SCSC_WLAN_ICTAS)
	tas_mode = sdev->sar_method;
	/* For customer, ICTAS is same with C-TAS */
	if (tas_mode > 3)
		tas_mode = 3;
#endif

	len = snprintf(command, buf_len, "%d", tas_mode);
	return len;
}
...
int slsi_set_max_bw(struct net_device *dev, char *command, int cmd_len)
{
	...
	struct slsi_dev        *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args *ioctl_args = NULL;
	int                    link_id = 0, bw_index = 0, leaky_ap_detect = 0;
	u16                    bandwidth = 0;
	int                    ret = 0;

	ioctl_args = slsi_get_private_command_args(command, cmd_len, 3);
	ret = slsi_verify_ioctl_args(sdev, ioctl_args);
	if (ret)
		return -1;
	if (ioctl_args->arg_count != 3) {
	...
	if (slsi_str2int(ioctl_args->args[0], &link_id)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[0]);
		ret = -1;
		goto exit_with_args;
	}
	...
	if (slsi_str2int(ioctl_args->args[2], &leaky_ap_detect)) {
		SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[2]);
		ret = -1;
		goto exit_with_args;
	}
	...
	if (!slsi_is_sta_connected(ndev_vif)) {
		SLSI_ERR(sdev, "Must be STA interface and connected, iftype = %d\n", ndev_vif->iftype);
		ret = -1;
		goto exit_with_mutex_args;
	}
#ifdef CONFIG_SCSC_WLAN_EHT
	if (ndev_vif->sta.valid_links) {
		if (link_id < 0 || link_id >= MAX_NUM_MLD_LINKS || !(ndev_vif->sta.valid_links & BIT(link_id))) {
			SLSI_ERR(sdev, "Invalid or No active connection, link_id: '%d'\n", link_id);
			ret = -1;
			goto exit_with_mutex_args;
		}
	} else {
...
#endif
	if (bw_index < 0 || bw_index > 5) {
		SLSI_ERR(sdev, "Invalid bw_index: '%d'\n", bw_index);
		ret = -1;
		goto exit_with_mutex_args;
	}
	if (leaky_ap_detect != 0 && leaky_ap_detect != 1) {
		SLSI_ERR(sdev, "Invalid liakeyAP_detect: '%d'\n", leaky_ap_detect);
		ret = -1;
		goto exit_with_mutex_args;
	}
	bandwidth = (bw_index == 5) ? 176 : 20 << bw_index;
	...
	ret = slsi_mlme_set_max_bw(sdev, dev, (u8)link_id, leaky_ap_detect, bandwidth);
	if (ret < 0) {
		ret = -1;
	} else {
#ifdef CONFIG_SCSC_WLAN_EHT
		ndev_vif->sta.links[link_id].dynamic_bw.bw_index = bw_index;
		ndev_vif->sta.links[link_id].dynamic_bw.leaky_ap_detect = leaky_ap_detect;
#else
		ndev_vif->sta.dynamic_bw.bw_index = bw_index;
		ndev_vif->sta.dynamic_bw.leaky_ap_detect = leaky_ap_detect;
#endif
		ret = 0;
	}

exit_with_mutex_args:
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
exit_with_args:
	kfree(ioctl_args);
...
#ifdef CONFIG_SCSC_WLAN_EHT
	for_each_valid_link(&ndev_vif->sta, link_id)
		len += snprintf(command + len, cmd_len - len, "%d %d %d ", link_id,
				ndev_vif->sta.links[link_id].dynamic_bw.bw_index,
				ndev_vif->sta.links[link_id].dynamic_bw.unknown ?
				-1 : ndev_vif->sta.links[link_id].dynamic_bw.leaky_ap_detect);
#else
	len = snprintf(command, cmd_len, "0 %d %d", ndev_vif->sta.dynamic_bw.bw_index,
		       ndev_vif->sta.dynamic_bw.unknown ? -1 : ndev_vif->sta.dynamic_bw.leaky_ap_detect);
#endif
...
int slsi_enable_24g_he40(struct net_device *dev, char *command, int cmd_len)
{
	struct netdev_vif	*ndev_vif = netdev_priv(dev);
	struct slsi_dev		*sdev = ndev_vif->sdev;
	struct slsi_ioctl_args  *ioctl_args = NULL;
	...
	if (enable != 0 && enable != 1) {
		SLSI_ERR(sdev, "Invalid enable: '%d'\n", enable);
		ret = -EINVAL;
		goto exit_with_args;
	}

	ret = slsi_set_boolean_mib(sdev, NULL, SLSI_PSID_UNIFI24_G40_MHZ_CHANNELS, enable, 0);
...
static int slsi_local_packet_capture_start(struct net_device *dev, char *command, int buf_len)
{
	...
	ioctl_args = slsi_get_private_command_args(command, buf_len, 20);
	ret = slsi_verify_ioctl_args(sdev, ioctl_args);
	if (ret) {
		ret = LPC_UNSUPPORTED_COMMAND;
		len = snprintf(command, buf_len, "%d", ret);
		return len;
	}
	...
	ret = slsi_lpc_start(sdev, lpc_enable_flag, ioctl_args->args[1]);
...
static int slsi_local_packet_capture_stop(struct net_device *dev, char *command, int buf_len)
{
	...
	ret = slsi_lpc_stop(sdev, ioctl_args->args[0], false);
...
static int slsi_local_packet_capture_start_for_test(struct net_device *dev, char *command, int buf_len)
{
	return slsi_local_packet_capture_start(dev, command, buf_len);
}

static int slsi_local_packet_capture_stop_for_test(struct net_device *dev, char *command, int buf_len)
{
	return slsi_local_packet_capture_stop(dev, command, buf_len);
}
...
static int slsi_local_packet_capture_is_running(struct net_device *dev, char *command, int buf_len)
{
	...
	struct slsi_dev           *sdev = ndev_vif->sdev;
	struct slsi_ioctl_args    *ioctl_args = NULL;
	int ret = LPC_UNKNOWN_ERROR; /* Failed due to unknown errors */
	int len = 0;
	...
	ret = slsi_lpc_is_lpc_enabled_by_name(ioctl_args->args[0]);
...
static int slsi_disable_btm_legacy(struct net_device *dev, char *buf, int buf_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	int               ret = 0;

	SLSI_NET_INFO(dev, "Requested BTM disable\n");
	ndev_vif->sta.btm_disabled = 1;
...
static const struct slsi_ioctl_fn slsi_ioctl_fn_table[] = {
	...
	{ CMD_GET_MAX_TX_POWER,             slsi_get_max_tx_power },
	...
	{ CMD_SET_KEEP_ALIVE_INTERVAL_LEGACY,     slsi_set_keepalive_parameters },
...
#endif
#ifdef CONFIG_SCSC_WLAN_EHT
	{CMD_SET_WIFI7_ENABLE,              slsi_set_wifi7_enable},
	{CMD_SET_NUM_ALLOWED_MLO_LINK,      slsi_set_num_allowed_mlo_link},
	{CMD_GET_ML_LINK_STATE,             slsi_get_ml_link_state},
	{CMD_ML_LINK_STATE_CONTROL,         slsi_ml_link_state_control},
        {CMD_GET_ML_TID_MAPPING_STATUS,     slsi_get_ml_tid_mapping_status},
	{CMD_MEASURE_ML_CHANNEL_CONDITION,  slsi_measure_ml_channel_condition},
	{CMD_ML_TID_MAPPING_REQUEST,        slsi_ml_tid_mapping_request},
#endif
	...
	{CMD_SET_MAX_BANDWIDTH,             slsi_set_max_bw},
	{CMD_GET_MAX_BANDWIDTH,             slsi_get_max_bw},
	{CMD_ENABLE_24G_HE40,               slsi_enable_24g_he40},
	{CMD_DISABLE_BTM_LEGACY,            slsi_disable_btm_legacy},
#ifdef CONFIG_SLSI_WLAN_LPC
	{CMD_TEST_STARTCAPTURE,             slsi_local_packet_capture_start_for_test},
	{CMD_TEST_STOPCAPTURE,              slsi_local_packet_capture_stop_for_test},
