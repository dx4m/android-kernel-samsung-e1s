...
#if IS_ENABLED(CONFIG_INTERCONNECT) && !IS_ENABLED(CONFIG_CNSS_EXYNOS)
/**
...
#define CPU_VOTE_CLOCK_11BE 2208000
#define INT_VOTE_CLOCK_11BE 533000
#define MIF_VOTE_CLOCK_11BE 3172000
				    ...
				    u32 bw, bool save)
{
	switch (bw) {
	    ...
	    case CNSS_BUS_WIDTH_MAX:
		/*11BE: CPU(9) prime core 2200000 + SNOC 533000 + DDR 3172000 per test */
...
void exynos_pm_qos_init(void)
{
	...
	if(cpucl1_policy && cpucl2_policy) {
	    freq_qos_add_request(&cpucl1_policy->constraints, &qos_req_cl1, FREQ_QOS_MIN, CPU_VOTE_CLOCK_DEF);
	    freq_qos_add_request(&cpucl2_policy->constraints, &qos_req_cl2, FREQ_QOS_MIN, CPU_VOTE_CLOCK_DEF);
	} else {
...
static int cnss_pci_suspend(struct device *dev)
{
	...
	if (!pci_priv->disable_pc) {
		...
		cnss_pr_err("cnss_pci_suspend : PC is disabled : %d!!\n",
			pci_priv->disable_pc);
		ret = -EAGAIN;
		goto resume_driver;
	}
