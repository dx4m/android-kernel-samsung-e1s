...
int bts_update_type(unsigned int index, unsigned int info_val)
{
	...
	layer = BTS_BIT_GET(info_val, BTS_ONE_BIT_MASK, BTS_HINT_LAYER_SHIFT);
	...
	if (layer)
		btsdev->bwops_val |= BTS_BIT_SET(layer, BTS_ONE_BIT_MASK, BTS_HINT_LAYER_SHIFT);
	else
		btsdev->bwops_val &= BTS_BIT_CLEAR(BTS_ONE_BIT_MASK, BTS_HINT_LAYER_SHIFT);
