...
extern bool scsc_pcie_shutdown(void);
extern int exynos_pcie_rc_chk_link_status(int ch_num);
extern struct device *scsc_pcie_get_dev(void);
extern void scsc_pcie_register_print(void (*print_bitmap)(void));
extern void scsc_pcie_register_shutdown_cb(void (*shutdown_cb)(void));
extern void scsc_pcie_unregister_shutdown_cb(void);
...
static const char *users[21] = {"DEFAULT", "WLAN_HOST", "GDB", "RAMRP", "LOGGER_GERERATE",
"LOGGER_COLLECT", "LOGGER_UNREGISTER","FAILURE_WORK_WLAN", "FAILURE_WORK_WPAN", "FAILURE_WORK",
"FREEZE", "FORCE_PANIC", "RECOVER", "SET_RECOVERY", "LERNA", "SERVICE_START",
"SERVICE_STOP", "SERVICE_CLOSE", "SERVICE_OPEN", "RUNTIME_PM", "LAST"};
#endif
...
enum scsc_subsystem scsc_service_id_subsystem_mapping(enum scsc_service_id id)
{
	...
	case SCSC_SERVICE_ID_LR_WPAN:   return SCSC_SUBSYSTEM_WPAN;
...
int scsc_service_register_shutdown_cb(void(*shutdown_cb)(void))
{
	SCSC_TAG_INFO(MXMAN, "register shutdown_cb\n");

	scsc_pcie_register_shutdown_cb(shutdown_cb);

	return 0;
}
EXPORT_SYMBOL(scsc_service_register_shutdown_cb);

int scsc_service_unregister_shutdown_cb(void)
{
	SCSC_TAG_INFO(MXMAN, "unregister shutdown_cb\n");

	scsc_pcie_unregister_shutdown_cb();

	return 0;
}
EXPORT_SYMBOL(scsc_service_unregister_shutdown_cb);
#endif
...
#endif
#if defined(CONFIG_SCSC_PCIE_CHIP)
	scsc_pcie_register_print(pcie_bitmap_print);
#endif
...
bool srvman_start_stop_not_allowed(struct srvman *srvman)
{
	...
	if (srvman->error == NOT_ALLOWED_START_STOP || srvman->error == NOT_ALLOWED_STOP)
		is_not_allowed = true;
	else
...
void pcie_bitmap_print(void)
{
	SCSC_TAG_INFO(MXMAN, "claim bitmap 0x%x\n", claim_bitmap);
}
EXPORT_SYMBOL(pcie_bitmap_print);
...
int scsc_mx_service_stop(struct scsc_service *service)
{
	...
	if (srvman_start_stop_not_allowed(srvman)) {
		 ...
		 */
		if (mxman_recovery_disabled()) {
			if (mxman_if_get_syserr_recovery_in_progress(mxman)) {
				SCSC_TAG_WARNING(MXMAN, "recovery disabled, but return -EILSEQ(%d) due to log collection by syserr\n", -EILSEQ);
				return -EILSEQ;
			} else {
...
int scsc_mx_service_close(struct scsc_service *service)
{
	...
	} else if (srvman_start_stop_not_allowed(srvman)) {
#else
	if (srvman_start_stop_not_allowed(srvman)) {
#endif
#if defined(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
		...
		if (mxman_recovery_disabled()) {
			if (mxman_if_get_syserr_recovery_in_progress(mxman)) {
				SCSC_TAG_WARNING(MXMAN, "recovery disabled, but return -EIO(%d) due to log collection by syserr\n", -EIO);
				return -EIO;
			} else {
				SCSC_TAG_ERR(MXMAN, "recovery disabled, return -EPERM (%d)\n", -EPERM);
...
static struct scsc_service *__scsc_mx_service_open(struct scsc_mx *mx, enum scsc_service_id id, struct scsc_service_client *client, int *status, void *data, size_t data_sz)
{
	...
	if (mxman_if_check_sysfs_for_hdm(sub)) {
		SCSC_TAG_ERR(MXMAN, "HDM locked %s service. Reject the service open.\n", sub ? "BT":"WiFi");
		return NULL;
	}
#endif
	mutex_lock(&srvman->api_access_mutex);
...
uintptr_t scsc_mx_service_mif_get_mem_start(struct scsc_service *service)
{
        struct scsc_mx      *mx = service->mx;
        ...
        return (uintptr_t)mif_abs->get_mem_start(mif_abs);
}
EXPORT_SYMBOL(scsc_mx_service_mif_get_mem_start);
...
#if defined(CONFIG_SCSC_PCIE_CHIP)
void scsc_service_irqdump(struct scsc_service *service)
{
	struct scsc_mif_abs *mif_abs;
	mif_abs = scsc_mx_get_mif_abs(service->mx);
	mif_abs->wlbt_irqdump(mif_abs);
}
EXPORT_SYMBOL(scsc_service_irqdump);
#endif