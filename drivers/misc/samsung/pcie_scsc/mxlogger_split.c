...
static void mxlogger_to_shared_dram(struct mxlogger *mxlogger, u8 channel)
{
	...
	if (bt_fw_realtime_log_disabled && channel == MXLOGGER_CHANNEL_WPAN) {
		SCSC_TAG_INFO(MXMAN, "Because of BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
	} else {
...
static int mxlogger_collect_init(struct scsc_log_collector_client *collect_client)
{
	...
	if (!mxlogger->initialized || !chan->enabled || chan->crashed)
		return 0;
...
int mxlogger_start_channel(struct mxlogger *mxlogger, enum scsc_mif_abs_target target)
{
	...
	mxlogger->chan[channel].crashed = false;
	...
	if (!mxlogger->observers) {
		...
		if (channel == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
			SCSC_TAG_INFO(MXMAN, "WPAN channel is starting, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
			mxlogger_to_shared_dram(mxlogger, channel);
		} else {
#if IS_ENABLED(CONFIG_BT_FWSNOOP_LOGGING)
...
int mxlogger_stop_channel(struct mxlogger *mxlogger, enum scsc_mif_abs_target target)
{
	...
	if (!mxlogger->observers) {
		...
		if (channel == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
			SCSC_TAG_INFO(MXMAN, "WPAN channel is stopping, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
			mxlogger_to_shared_dram(mxlogger, channel);
		} else {
#if IS_ENABLED(CONFIG_BT_FWSNOOP_LOGGING)
			mxlogger_to_host_class(mxlogger, channel, MXLOGGER_CLASS_REGISTERED);
...
void mxlogger_mark_channel_as_crashed(struct mxlogger *mxlogger, enum scsc_mif_abs_target target)
{
	...
	if (!mxlogger->configured || !chan->enabled) {
		SCSC_TAG_INFO(MXMAN, "Mxlogger not %s\n", (!mxlogger->configured) ? "configured" : "channel enabled");
		return;
	}

	SCSC_TAG_INFO(MXMAN, "Mark %s channel as crashed\n", (target == SCSC_MIF_ABS_TARGET_WPAN) ? "WPAN" : "WLAN");
	mutex_lock(&mxlogger->lock);
	/* Mark channel */
	mxlogger->chan[channel].crashed = true;
	mutex_unlock(&mxlogger->lock);
}
...
void mxlogger_deinit(struct scsc_mx *mx, struct mxlogger *mxlogger)
{
	...
	if (!mxlogger->observers) {
		...
		for (i = 0; i < MXLOGGER_CHANNELS; i++) {
			if (i == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
				SCSC_TAG_INFO(MXMAN, "mxlogger deinit, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
				mxlogger_to_shared_dram(mxlogger, i);
			} else {
#if IS_ENABLED(CONFIG_BT_FWSNOOP_LOGGING)
...
int mxlogger_register_observer(struct mxlogger *mxlogger, char *name)
{
	...
	for (i = 0; i < MXLOGGER_CHANNELS; i++) {
		if (i == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
			SCSC_TAG_INFO(MXMAN, "Register observer, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
			mxlogger_to_shared_dram(mxlogger, i);
		} else {
#if IS_ENABLED(CONFIG_BT_FWSNOOP_LOGGING)
			mxlogger_to_host_class(mxlogger, i, MXLOGGER_CLASS_REGISTERED);
...
int mxlogger_unregister_observer(struct mxlogger *mxlogger, char *name)
{
	...
	if (mxlogger->observers == 0) {
		...
		for (i = 0; i < MXLOGGER_CHANNELS; i++) {
			if (i == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
				SCSC_TAG_INFO(MXMAN, "UN-register observer, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
				mxlogger_to_shared_dram(mxlogger, i);
			} else {
#if IS_ENABLED(CONFIG_BT_FWSNOOP_LOGGING)
				mxlogger_to_host_class(mxlogger, i, MXLOGGER_CLASS_REGISTERED);
...
int mxlogger_register_observer_class(struct mxlogger *mxlogger, char *name, uint8_t class)
{
	...
	for (i = 0; i < MXLOGGER_CHANNELS; i++) {
		if (i == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
			SCSC_TAG_INFO(MXMAN, "Register observer Class, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
			mxlogger_to_shared_dram(mxlogger, i);
		} else {
...
int mxlogger_unregister_observer_class(struct mxlogger *mxlogger, char *name, uint8_t class)
{
	...
	if (mxlogger->observers == 0) {
		...
		for (i = 0; i < MXLOGGER_CHANNELS; i++) {
			if (i == MXLOGGER_CHANNEL_WPAN && bt_fw_realtime_log_disabled) {
				SCSC_TAG_INFO(MXMAN, "UN-register observer Class, BT fw realtime log disabled, inform BT firmware to send logs to DRAM\n");
				mxlogger_to_shared_dram(mxlogger, i);
			} else {
				mxlogger_to_host_class(mxlogger, i, MXLOGGER_CLASS_REGISTERED);
			}
