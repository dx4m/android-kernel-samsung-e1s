...
static int __init scsc_mx_module_init(void)
{
	...
	SCSC_TAG_INFO(MXMAN,"No wlbt device\n");
	return 0;
#endif
...
void scsc_mx_module_client_remove(enum scsc_module_client_reason reason)
{
...
void scsc_mx_module_client_probe(enum scsc_module_client_reason reason)
{
	struct clients_node *clients_node;
	struct mx_node      *mx_node, *next_mx;
...
EXPORT_SYMBOL(scsc_mx_module_client_remove);
EXPORT_SYMBOL(scsc_mx_module_client_probe);
#else
