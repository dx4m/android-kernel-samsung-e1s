...
extern void pcie_mif_set_iommu_dom(struct pcie_mif *pcie, struct iommu_domain *iommu_dom);
extern struct iommu_domain *pcie_mif_get_iommu_dom(struct pcie_mif *pcie);
#endif
...
#define HWID_UNUSED -1
static int hwid = HWID_UNUSED;
module_param(hwid, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(hwid, "hwid for hcf load");
...
static uint on_pinctrl_delay = 60;
module_param(on_pinctrl_delay, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(on_pinctrl_delay, "wlbt_pinctrl on delay(ms)");
...
static bool enable_link_fail_panic = false;
...
#if defined(CONFIG_SCSC_BB_PAEAN) || defined(CONFIG_SCSC_BB_REDWOOD)
...
enum pcie_event_type {
	...
	PCIE_EVT_PMU_ON_REQ,
	PCIE_EVT_LINK_CHANGE,
};

static const char *events[7] = {"RST ON", "RST OFF", "CLAIM", "RELEASE", "PMU OFF", "PMU ON", "LINK CHANGE"};

#ifdef CONFIG_SCSC_QOS
#if defined(CONFIG_SOC_S5E9945)
/* cpucl0: 400Mhz / 864Mhz / 1248Mhz for cpucl0 */
static uint qos_cpucl0_lv[] = {0, 4, 8};
/* cpucl1: 672Mhz / 1440Mhz / 2592Mhz for cpucl1 */
static uint qos_cpucl1_lv[] = {0, 8, 20};
/* cpucl2: 672Mhz / 1536Mhz / 2900Mhz for cpucl2 */
static uint qos_cpucl2_lv[] = {0, 9, 24};
/* cpucl3: 672Mhz / 1536Mhz / 3207Mhz for cpucl3 */
static uint qos_cpucl3_lv[] = {0, 9, 26};
...
struct platform_mif {
	...
	void (*pmu_pcie_handler)(int irq, void *data);
	...
	/* spinlock for synchorinzing gen/lane change with irq set */
	spinlock_t link_spinlock;

	void (*pcie_recovery_notifier)(void);

	/* The following variables are required to break AP SUSPEND and make PCIe link turned on.*/
	bool kthread_is_parked;
	struct completion kthread_parked;
	struct wake_lock claim_wakelock;
...
void (*scsc_pcie_print_bitmap) (void);
...
const char *clk_req_print_type[3] = {
	"before linkup",
	"after linkup",
	"last",
};
 ...
 * bit 6 (0x40)
*/
#define CLK_REQ_PEND_BASE 0x12850A0C
void __iomem *clk_req_pending;

static void print_clk_req_pend(struct platform_mif * platform, int num)
{
#if !(defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E9955)) || defined(SCSC_SEP_VERSION)
	u32 clk_req = 0;
	u32 pending = 0;
	clk_req_pending = ioremap(CLK_REQ_PEND_BASE, SZ_4);
	clk_req = gpio_get_value(platform->clk_req_gpio);
	pending = readl(clk_req_pending);
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "%s clk_req= 0x%x, pending bit= 0x%x\n",clk_req_print_type[num], clk_req, pending);
	iounmap(clk_req_pending);
#endif
}

/* These variables are used to determine when the FSM thread operates in conjunction with context acquisition. */
static ktime_t last_fsm_start_time;
static ktime_t last_fsm_end_time;
static enum pcie_event_type last_event_type;
static ktime_t last_event_time;

static DEFINE_RWLOCK(is_parked_lock);
...
static int platform_mif_set_fw_runtime_pm(struct scsc_mif_abs *interface, int runtime_pm)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	return pcie_mif_set_fw_runtime_pm(platform->pcie, runtime_pm);
}
#endif

static void platform_mif_link_change(void)
{
	int need_delay;
	unsigned long flags;

	spin_lock_irqsave(&g_platform->link_spinlock, flags);
	need_delay = pcie_mif_link_change(g_platform->pcie);
	set_pcie_change(g_platform->pcie, false);
	if(need_delay){
		spin_unlock_irqrestore(&g_platform->link_spinlock, flags);
		msleep(5);
		pcie_mif_l1ss_ctrl(g_platform->pcie, 1);
	} else {
		pcie_mif_l1ss_ctrl(g_platform->pcie, 1);
		spin_unlock_irqrestore(&g_platform->link_spinlock, flags);
	}
...
static int platform_mif_pcie_control_fsm(void *data)
{
	...
	while (true) {
		...
		last_fsm_start_time = ktime_get();
		last_event_time = ev_data->event_time;
		last_event_type = ev_data->event;
		...
		if (!ret) {
			...
			continue;
		} else if (pcie_get_shutdown_flag(platform->pcie)) {
			SCSC_TAG_ERR(PLAT_MIF, "Not allow to handle event(%d) due to device shutdown\n", ev_data->event);
			if (ev_data->complete)
		...
		case PCIE_STATE_OFF:
			if (ev_data->event == PCIE_EVT_RST_ON) {
				print_clk_req_pend(platform, BEFORE_LINKUP);
				ret = pcie_mif_poweron(platform->pcie);
				print_clk_req_pend(platform, AFTER_LINKUP);
				if (ret)
		...
		case PCIE_STATE_ON:
			if (ev_data->event == PCIE_EVT_RST_OFF) {
				...
				if (current_users == 0) {
					...
					pcie_set_mbox_pmu_pcie(platform->pcie, PMU_AP_MSG_WLBT_PCIE_OFF_ACCEPT);
					pcie_mif_poweroff(platform->pcie);
					state = PCIE_STATE_SLEEP;
				} else {
					SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "PCIe users. Send PMU_AP_MSG_WLBT_PCIE_OFF_REJECT\n");
					if(scsc_pcie_print_bitmap != NULL)
						scsc_pcie_print_bitmap();
					/* Since off is rejected, we have to
					 * increase the number of fw_users */
					platform->fw_users += 1;
					pcie_set_mbox_pmu_pcie(platform->pcie, PMU_AP_MSG_WLBT_PCIE_OFF_REJECT);
		...
		case PCIE_STATE_HOLD_HOST:
			...
			if (ev_data->event == PCIE_EVT_RST_OFF) {
			 	...
			 	* to PMU */
				pcie_mif_l1ss_ctrl(platform->pcie, 0);
				SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Host PCIe release. Send PMU_AP_MSG_WLBT_PCIE_OFF_REJECT_CANCEL\n");
				pcie_set_mbox_pmu_pcie(platform->pcie, PMU_AP_MSG_WLBT_PCIE_OFF_REJECT_CANCEL);
		...
		if(ev_data->event == PCIE_EVT_LINK_CHANGE) {
			platform_mif_link_change();
		}
...
#if !(defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E9955)) || defined(SCSC_SEP_VERSION)
static void platform_mif_set_wlbt_clk(struct platform_mif *platform)
{
	u32 eint_num;
...
static int platform_mif_register_clk(struct platform_mif *platform)
{
	int ret = 0;
...
#if !(defined(SCSC_SEP_VERSION) && defined(CONFIG_SOC_S5E9945))
...
static void platform_recovery_disabled_reg(struct scsc_mif_abs *interface, bool (*handler)(void))
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif recovery %pS\n", handler);
...
static void platform_mif_pmu_pcie_isr(int irq, void *data)
{
	struct platform_mif *platform = (struct platform_mif *)data;
	u32 cmd = pcie_get_mbox_pmu_pcie(platform->pcie);
	unsigned long flags;

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "PCIE IRQ received\n");
	...
	case PMU_AP_MSG_LINK_CHANGE:
		SCSC_TAG_INFO(PLAT_MIF, "Received PMU IRQ cmd [PCIE LINK CHANGE REQ]\n");
		spin_lock_irqsave(&platform->cb_sync, flags);
		set_pcie_change(platform->pcie, true);
		platform_mif_send_event_to_fsm(platform, PCIE_EVT_LINK_CHANGE);
		spin_unlock_irqrestore(&platform->cb_sync, flags);
		break;
	default:
		break;
	...
	if (platform->pmu_pcie_handler != platform_mif_irq_default_handler){
		platform->pmu_pcie_handler(irq, platform->irq_dev_pmu);
	}
	else
		SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "MIF PMU pcie Interrupt Handler not registered\n");
}
...
static void platform_mif_default_notifier(void)
{
	/* recovery notifier not registered */
	SCSC_TAG_INFO_DEV(PLAT_MIF, NULL, "Recovery notifier not registered\n");
}

static void platform_mif_pcie_recovery_notifier(void)
{
	struct platform_mif *platform = g_platform;

	if (platform->pcie_recovery_notifier != platform_mif_default_notifier)
		platform->pcie_recovery_notifier();
	else
		SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "MIF recovery notifier not registered\n");
}
...
int scsc_pcie_complete(void)
{
	...
	*/
	if(!wait_for_completion_timeout(&g_platform->pcie_on, 5*HZ)) {
		...
		SCSC_TAG_ERR_DEV(PLAT_MIF, g_platform->dev,
						"curtime = %llu, last fsm start %llu, last fsm end %llu, last event %s was sent %llu\n",
						curtime, last_fsm_start_time, last_fsm_end_time, events[last_event_type], last_event_time);
	...
	if (enable_platform_mif_arm_reset) {
		...
		if (!reset) { /* Release from reset */
			...
			if (!platform_mif_get_scan2mem_mode(interface)) {
				if (keep_powered && !pcie_get_link_down_cb_flag(platform->pcie)) {
					/* If WLBT was kept powered after reset, to allow the PMU to
					 * sleep HW, power cycle it now to reset the HW.
					 */
...
#ifdef CONFIG_SCSC_BB_REDWOOD
			SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Delay between pmic_on and reset_release is set to %u ms\n", on_pinctrl_delay);
			mdelay(on_pinctrl_delay);
			gpio_set_value(platform->reset_gpio, 1);
#endif
#if !(defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E9955)) || defined(SCSC_SEP_VERSION)
			platform_mif_set_wlbt_clk(platform);
#endif
			...
			pcie_set_link_down_cb_flag(platform->pcie, false);
			...
			if (!init) {
				...
				if (get_pcie_link_state(platform->pcie) == PCIE_LINK_STATE_ON) {
...
#ifdef CONFIG_SCSC_BB_REDWOOD
			/* Note Scan2mem requires a reset */
			if (!keep_powered || platform_mif_get_scan2mem_mode(interface) || pcie_get_link_down_cb_flag(platform->pcie))
				gpio_set_value(platform->reset_gpio, 0);
#endif
			if (!platform_mif_get_scan2mem_mode(interface)) {
				if (!keep_powered || pcie_get_link_down_cb_flag(platform->pcie)) {
...
static void *platform_mif_map(struct scsc_mif_abs *interface, size_t *allocated)
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev,
			"EXYNOS SOC_VERSION: product_id 0x%x, rev 0x%x\n",
	...
	ret = pcie_map(platform->pcie, allocated);
	if(!ret) {
					 ...
					 void *dev)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif int handler %pS in %p %p\n", handler, platform,
			  interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static void platform_mif_irq_unreg_handler(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Unregistering mif int handler %pS\n", interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static int platform_mif_get_pcie_link_state(struct scsc_mif_abs *interface)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	return get_pcie_link_state(platform->pcie);
}
					      ...
					      void *dev)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif int handler for WPAN %pS in %p %p\n", handler,
			  platform, interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static void platform_mif_irq_unreg_handler_wpan(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Unregistering mif int handler for WPAN %pS\n", interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
						       ...
						       void (*handler)(int irq, void *data), void *dev)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif reset_request int handler %pS in %p %p\n", handler,
			  platform, interface);
...
static void platform_mif_irq_unreg_reset_request_handler(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "UnRegistering mif reset_request int handler %pS\n", interface);
					     ...
					     void (*resume)(struct scsc_mif_abs *abs, void *data), void *data)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif suspend/resume handlers in %p %p\n", platform,
			  interface);
...
static void platform_mif_suspend_unreg_handler(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Unregistering mif suspend/resume handlers in %p %p\n", platform,
			  interface);
...
static int platform_mif_get_mbox_pmu_pcie(struct scsc_mif_abs *interface)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);
	u32 val = 0;

	val = pcie_get_mbox_pmu_pcie(platform->pcie);

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Read PMU_PCIE_MAILBOX: %u\n", val);
	return val;
}

static int platform_mif_set_mbox_pmu_pcie(struct scsc_mif_abs *interface, u32 val)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Write PMU_PCIE_MAILBOX: %u\n", val);

	pcie_set_mbox_pmu_pcie(platform->pcie, val);
...
static void platform_mif_set_l1ss_ctrl(struct scsc_mif_abs *interface, u32 val)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "%s L1ss ctrl\n", val ? "Enable":"Disable");

	pcie_mif_l1ss_ctrl(platform->pcie, val);
}
#endif
					     ...
					     void *dev)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif pmu int handler %pS in %p %p\n", handler, platform,
			  interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
					     ...
					     void *dev)
{
	...
	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif pmu pcie_error int handler %pS in %p %p\n", handler, platform,
			  interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static void platform_mif_irq_reg_pmu_pcie_handler(struct scsc_mif_abs *interface, void (*handler)(int irq, void *data),
					     void *dev)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif pmu pcie int handler %pS in %p %p\n", handler, platform,
			  interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
	platform->pmu_pcie_handler = handler;
	platform->irq_dev_pmu = dev;
	spin_unlock_irqrestore(&platform->mif_spinlock, flags);
}

static void platform_mif_reg_pcie_recovery_notifier(struct scsc_mif_abs *interface, void (*notifier)(void))
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Registering mif recovery notifier %pS in %p\n", notifier, interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
	platform->pcie_recovery_notifier = notifier;
	spin_unlock_irqrestore(&platform->mif_spinlock, flags);
}

static void platform_mif_unreg_pcie_recovery_notifier(struct scsc_mif_abs *interface)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, platform->dev, "Unregistering mif recovery notifier in %p\n", interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
	platform->pcie_recovery_notifier = platform_mif_default_notifier;
	spin_unlock_irqrestore(&platform->mif_spinlock, flags);
}

static void platform_mif_set_pcie_cb_panic(struct scsc_mif_abs *interface, bool val)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Set the value as %s\n", (val) ? "true":"false");

	pcie_set_pcie_cb_panic(val);
}

static void platform_mif_set_link_fail_panic(struct scsc_mif_abs *interface, bool val)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Set the value as %s\n", (val) ? "true":"false");

	enable_link_fail_panic = val;
}
...
struct scsc_mif_abs *platform_mif_create(struct platform_device *pdev)
{
	...
	struct platform_device *pdev_pcie;
	struct iommu_domain *iommu_domain_pcie = NULL;
	int vid;
#endif
	int err = 0;
	...
	platform_if->get_pcie_link_state = platform_mif_get_pcie_link_state;
	...
	platform_if->get_mbox_pmu_pcie = platform_mif_get_mbox_pmu_pcie;
	platform_if->set_mbox_pmu_pcie = platform_mif_set_mbox_pmu_pcie;
	platform_if->get_mbox_pmu_error = platform_mif_get_mbox_pmu_error;
#if defined(CONFIG_SCSC_BB_REDWOOD)
	platform_if->set_l1ss_ctrl = platform_mif_set_l1ss_ctrl;
#endif
	...
	platform_if->irq_reg_pmu_pcie_handler = platform_mif_irq_reg_pmu_pcie_handler;
	platform->pmu_handler = platform_mif_irq_default_handler;
	platform->pmu_pcie_handler = platform_mif_irq_default_handler;
...
#if defined(CONFIG_SCSC_RUNTIMEPM)
	platform_if->set_fw_runtime_pm = platform_mif_set_fw_runtime_pm;
#endif
	...
	platform_if->reg_pcie_recovery_notifier = platform_mif_reg_pcie_recovery_notifier;
	platform_if->unreg_pcie_recovery_notifier = platform_mif_unreg_pcie_recovery_notifier;
	platform_if->set_pcie_cb_panic = platform_mif_set_pcie_cb_panic;
	platform_if->set_link_fail_panic = platform_mif_set_link_fail_panic;
	...
	platform->pmic_gpio = of_get_named_gpio(platform->dev->of_node, "wlbt,pmic_gpio", 0);
#else
...
#if !(defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E9955)) || defined(SCSC_SEP_VERSION)
	platform->clk_req_gpio = of_get_named_gpio(platform->dev->of_node,
				"wlbt,clk_gpio", 0);

	err = platform_mif_register_clk(platform);
	if (err) {
...
#if defined(SCSC_SEP_VERSION)
	platform->pmic_regulator = devm_regulator_get(platform->dev, "vdd-wlbt");
	if(IS_ERR(platform->pmic_regulator)) {
		SCSC_TAG_ERR_DEV(PLAT_MIF, platform->dev, "cannot get pmic regulator\n");
		goto error_exit;
	}
	err = regulator_enable(platform->pmic_regulator);
	if (err) {
		SCSC_TAG_ERR_DEV(PLAT_MIF, platform->dev, "cannot sub_pmic enable");
		goto error_exit;
	}
#endif
	...
	pcie_irq_reg_pmu_pcie_handler(pcie, platform_mif_pmu_pcie_isr, (void *)platform);
	...
	pcie_reg_pcie_recovery_notifier(pcie, platform_mif_pcie_recovery_notifier);
	platform_mif_set_scan2mem_mode(platform_if, false);

	if(of_property_read_u32(platform->dev->of_node, "hwid",  &hwid)) {
		SCSC_TAG_INFO_DEV(PLAT_MIF, &pdev->dev, "hwid is not set, skip dual hcf setting\n");
	}
#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
	if(of_property_read_u32(platform->dev->of_node, "vid",  &vid)) {
		SCSC_TAG_INFO_DEV(PLAT_MIF, &pdev->dev, "vid is not set, skip iommu setting\n");
	}else {
		np_pcie = of_parse_phandle(platform->dev->of_node, "sysmmu_pcie", 0);
		if (!np_pcie) {
			dev_err(&pdev->dev, "Failed to get pcie sysmmu device node\n");
			goto error_iommu;
		}

		pdev_pcie = of_find_device_by_node(np_pcie);
		if (!pdev_pcie) {
			dev_err(&pdev->dev, "Failed to get pcie sysmmu platform device\n");
			goto error_iommu;
		}

		iommu_domain_pcie = hsi_sysmmu_get_domain_for_dev(&pdev_pcie->dev, vid);
		if (!iommu_domain_pcie) {
			dev_err(&pdev->dev, "Failed to get iommu domain\n");
			goto error_iommu;
		}
error_iommu:
		if(!iommu_domain_pcie)
			iommu_domain_pcie = hsi_sysmmu_get_domain_for_port("PCIe", vid);
		pcie_mif_set_iommu_dom(platform->pcie, iommu_domain_pcie);
	}
#endif

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	wake_lock_init(NULL, &(platform->wakehash_wakelock.ws), "wakehash_wakelock");
	wake_lock_init(NULL, &(platform->claim_wakelock.ws), "claim_wakelock");
#else
	wake_lock_init(&platform->wakehash_wakelock, WAKE_LOCK_SUSPEND, "wakehash_wakelock");
	wake_lock_init(&platform->claim_wakelock, WAKE_LOCK_SUSPEND, "claim_wakelock");
#endif
	spin_lock_init(&platform->link_spinlock);
...
void platform_mif_destroy_platform(struct platform_device *pdev, struct scsc_mif_abs *interface)
{
	...
	wake_lock_destroy(&platform->claim_wakelock);
...
static void platform_mif_set_is_parked(bool val)
{
	unsigned long flags;

	write_lock_irqsave(&is_parked_lock, flags);
	g_platform->kthread_is_parked = val;
	write_unlock_irqrestore(&is_parked_lock, flags);
}

static bool platform_mif_get_is_parked(void)
{
	bool ret;
	unsigned long flags;

	read_lock_irqsave(&is_parked_lock, flags);
	ret = g_platform->kthread_is_parked;
	read_unlock_irqrestore(&is_parked_lock, flags);
	return ret;
}
...
int wlbt_rc_pm_notifier(struct notifier_block *nb, unsigned long mode, void *_unused)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		platform_mif_set_is_parked(true);
		kthread_park(platform->t);
		break;
	case PM_POST_SUSPEND:
		kthread_unpark(platform->t);
		platform_mif_set_is_parked(false);
		complete(&g_platform->kthread_parked);
...
void scsc_pcie_register_print(void (*print_bitmap)(void))
{
	scsc_pcie_print_bitmap = print_bitmap;
}
EXPORT_SYMBOL(scsc_pcie_register_print);
...
bool scsc_pcie_link_state_error(void)
{
	struct pcie_mif *pcie = pcie_get_instance();
	return (get_pcie_link_state(pcie) == PCIE_LINK_STATE_ERROR) ? true : false;
}
EXPORT_SYMBOL(scsc_pcie_link_state_error);

bool scsc_pcie_link_state_on(void)
{
		struct pcie_mif *pcie = pcie_get_instance();
			return (get_pcie_link_state(pcie) == PCIE_LINK_STATE_ON) ? true : false;
}
EXPORT_SYMBOL(scsc_pcie_link_state_on);

bool scsc_pcie_shutdown(void)
{
		struct pcie_mif *pcie = pcie_get_instance();
		return (pcie_get_shutdown_flag(pcie)) ? true : false;
}
EXPORT_SYMBOL(scsc_pcie_shutdown);

struct device *scsc_pcie_get_dev(void)
{
	return pcie_mif_get_pcie_dev(g_platform->pcie);
}
EXPORT_SYMBOL(scsc_pcie_get_dev);

/*
	FUNCTION NAME : scsc_pcie_is_parked

	DESCRIPTION
	This function is used to check if AP is suspended and kthread for PCIe link is parked
	under the premise that the state of PCIe link is not 'ON'.
	If so, the host should try to change AP's status from SUSPEND to RESUME once.
	If this attempt fails, the host notifies the user of this function
	by returning 'false' that AP is suspended and kthread for PCIe link is parked.
*/
bool scsc_pcie_is_parked(void)
{
	bool ret = false;
	if (platform_mif_get_is_parked() && !scsc_pcie_link_state_on()) {
		wake_lock(&g_platform->claim_wakelock);
		reinit_completion(&g_platform->kthread_parked);
		SCSC_TAG_ERR_DEV(PLAT_MIF, g_platform->dev, "Wake lock for claiming PCIe link\n");
		if(!wait_for_completion_timeout(&g_platform->kthread_parked, 3*HZ)) {
			SCSC_TAG_ERR_DEV(PLAT_MIF, g_platform->dev, "wait for complete timeout\n");
			ret = true;
		}
		wake_unlock(&g_platform->claim_wakelock);
	}
	return ret;
}
EXPORT_SYMBOL(scsc_pcie_is_parked);

#if defined(CONFIG_WLBT_DUAL_HCF)
int slsi_wlbt_hwid(void)
{
	return hwid;
}
EXPORT_SYMBOL(slsi_wlbt_hwid);
#endif

#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
struct iommu_domain *scsc_pcie_get_iommu_dom(void)
{
	return pcie_mif_get_iommu_dom(g_platform->pcie);
}
EXPORT_SYMBOL(scsc_pcie_get_iommu_dom);
#endif

void scsc_pcie_register_shutdown_cb(void (*shutdown_cb)(void))
{
	pcie_mif_register_shutdown_cb(g_platform->pcie, shutdown_cb);
}
EXPORT_SYMBOL(scsc_pcie_register_shutdown_cb);

void scsc_pcie_unregister_shutdown_cb(void)
{
	pcie_mif_unregister_shutdown_cb(g_platform->pcie);
}
EXPORT_SYMBOL(scsc_pcie_unregister_shutdown_cb);