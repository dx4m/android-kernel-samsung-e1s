...
static const char *mmtrans_chan_str[MMTRANS_NUM_CHANNELS] = {
																"MAXWELL MANAGEMENT",
																"SERVICE MANAGEMENT",
																"MAXWELL LOGGING"
																};

static const char *mmtrans_sm_msg_str[] = {
											"SM_MSG_START_REQ", "SM_MSG_START_CFM",
											"SM_MSG_STOP_REQ", "SM_MSG_STOP_CFM"
											};

static const char *mmtrans_mm_msg_str[] = {
											"MM_START_IND", "MM_HALT_REQ", "MM_FORCE_PANIC",
											"MM_HOST_SUSPEND", "MM_HOST_RESUME", "MM_FW_CONFIG",
											"MM_HALT_RSP", "MM_FM_RADIO_CONFIG", "MM_LERNA_CONFIG",
											"MM_SYSERR_IND", "MM_SYSERR_CMD"
											};

static const char *logging_msg_str[] = {
											"MM_MXLOGGER_LOGGER_CMD", "MM_MXLOGGER_DIRECTION_CMD",
											"MM_MXLOGGER_CONFIG_CMD", "MM_MXLOGGER_INITIALIZED_EVT",
											"MM_MXLOGGER_SYNC_RECORD", "MM_MXLOGGER_STARTED_EVT",
											"MM_MXLOGGER_STOPPED_EVT", "MM_MXLOGGER_COLLECTION_FW_REQ_EVT",
											"MM_MXLOGGER_DUMP_BUFFER_RT_CMD",
											"MM_MXLOGGER_DUMP_BUFFER_RT_FLUSHED_EVT"
											};

static const char *logging_arg_str[] = {
											"LOGGER_ENABLE", "LOGGER_DISABLE",
											"DIRECTION_DRAM", "DIRECTION_HOST"
											};
#if defined(CONFIG_CHIPLOGGER_V_2_0)
static const char *mxlogger_buff_str[] = {
											"MXLOGGER_SYNC", "MXLOGGER_IMP", "MXLOGGER_RESERVED_COMMON",
											"MXLOGGER_RESERVED_BT", "MXLOGGER_RESERVED_WLAN",
											"MXLOGGER_RESERVED_RADIO", "MXLOGGER_MXLOG",
											"MXLOGGER_UDI", "MXLOGGER_IMPD12",
											"MXLOGGER_LINK"
											};
#endif
...
void mxmgmt_print_sent_data_dump(void)
{
	...
	uint8_t msg_arg;
	uint8_t *data_offset;
	uint8_t *dump_addr = mif_stream_get_dump_for_write_gather(&irq_bit, &target);
	...
	if (chan_id > MMTRANS_CHAN_ID_MAXWELL_LOGGING) {
		SCSC_TAG_ERR(MXMGT_TRANS, "Wrong Channel ID(%d)!!\n", chan_id);
		return;
	}

	SCSC_TAG_INFO(MXMGT_TRANS, "Target subsystem: %s, IRQ Bit number: %d\n", (target == SCSC_MIF_ABS_TARGET_WPAN) ? "WPAN" : "WLAN", irq_bit);
	switch (chan_id) {
	case MMTRANS_CHAN_ID_MAXWELL_MANAGEMENT:
		msg_id = *(dump_addr + sizeof(uint8_t));
		data_offset = dump_addr + 2 * sizeof(uint8_t);
		SCSC_TAG_INFO(MXMGT_TRANS, "channel: %s, msg: %s\n", mmtrans_chan_str[chan_id], mmtrans_mm_msg_str[msg_id]);
		SCSC_TAG_INFO(MXMGT_TRANS, "opt data: 0x%08x\n", (uint32_t)(*data_offset));
		break;
	case MMTRANS_CHAN_ID_SERVICE_MANAGEMENT:
		serv_id = *(dump_addr + sizeof(uint8_t));
		msg_id = *(dump_addr + 2 * sizeof(uint8_t));
		data_offset = dump_addr + 3 * sizeof(uint8_t);
		SCSC_TAG_INFO(MXMGT_TRANS, "channel: %s, msg: %s\n", mmtrans_chan_str[chan_id], mmtrans_sm_msg_str[msg_id]);
		SCSC_TAG_INFO(MXMGT_TRANS, "serv ID: %d, opt data: 0x%08x\n", serv_id, (uint32_t)(*data_offset));
		/* In case of SM_MSG_START_REQ, the optional data means the byte offset value of the write starting point in kernel memory. */
		break;
	case MMTRANS_CHAN_ID_MAXWELL_LOGGING:
		msg_id = *(dump_addr + sizeof(uint8_t));
		msg_arg = *(dump_addr + 2 * sizeof(uint8_t));
		data_offset = dump_addr + 3 * sizeof(uint8_t);
		SCSC_TAG_INFO(MXMGT_TRANS, "channel: %s, msg: %s\n", mmtrans_chan_str[chan_id], logging_msg_str[msg_id]);
		if (msg_id == MM_MXLOGGER_LOGGER_CMD) {
			SCSC_TAG_INFO(MXMGT_TRANS, "arg: %s, reason: %s\n",
						logging_arg_str[msg_arg], (*data_offset) ? "DISABLE_REASON_COLLECTION":"DISABLE_REASON_STOP");
		} else if (msg_id == MM_MXLOGGER_DIRECTION_CMD) {
			SCSC_TAG_INFO(MXMGT_TRANS, "arg: %s, class: %d\n",
						logging_arg_str[msg_arg + 2 * msg_id], (u8)(*data_offset));
		} else if (msg_id == MM_MXLOGGER_CONFIG_CMD) {
			SCSC_TAG_INFO(MXMGT_TRANS, "arg: CONFIG_BASE_ADDR, offset: 0x%08x\n", (s32)(*data_offset));
		} else if (msg_id == MM_MXLOGGER_SYNC_RECORD) {
			SCSC_TAG_INFO(MXMGT_TRANS, "arg: SYNC_INDEX, buffer idx: %d\n", (u8)(*data_offset));
		}
#if defined(CONFIG_CHIPLOGGER_V_2_0)
		else if (msg_id == MM_MXLOGGER_DUMP_BUFFER_RT_CMD) {
			SCSC_TAG_INFO(MXMGT_TRANS, "arg: %s\n", mxlogger_buff_str[(u8)(*data_offset)]);
		}
#endif
		break;
	}
