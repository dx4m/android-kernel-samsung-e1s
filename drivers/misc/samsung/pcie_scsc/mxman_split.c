...
#endif
#endif
#if defined(CONFIG_HDM_WLBT_SUPPORT)
...
#include <asm/page.h>
#include <pcie_scsc/api/bt_audio.h>
...
#endif
#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU) && IS_ENABLED(CONFIG_SCSC_BB_REDWOOD)
extern struct iommu_domain *scsc_pcie_get_iommu_dom(void);
#endif
...
#if IS_ENABLED(CONFIG_SCSC_BB_REDWOOD) 
#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
extern void register_mxman_force_panic(struct iommu_domain *dom, int (*fp)(void));
extern void unregister_mxman_force_panic(struct iommu_domain *dom);
#else
extern void register_mxman_force_panic(int (*fp)(void));
#endif
#endif

#if defined(CONFIG_HDM_WLBT_SUPPORT)
#define SLSI_MX_HDM_CONTROL_DISABLE		(-1)
#define SLSI_MX_HDM_CONTROL_ENABLE		(0)
#define SLSI_MX_HDM_CONTROL_TERMINATE	(1)

static int hdm_wlan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
...
static int hdm_wpan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
static ssize_t sysfs_show_hdm_wpan_loader(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static ssize_t sysfs_store_hdm_wpan_loader(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static struct kobj_attribute hdm_wpan_loader_attr = __ATTR(hdm_wpan_loader, 0660, sysfs_show_hdm_wpan_loader, sysfs_store_hdm_wpan_loader);

#if defined(SCSC_SEP_VERSION) && (SCSC_SEP_VERSION >= 14)
/*
*	The function 'hdm_is_applied' is provided by MX HDM.
* 	As MX HDM is included in MX image only, the below function is valid for MX build system.
*/
extern bool hdm_is_applied(uint32_t bit);
#else
/* For HDM Testing */
#define HDM_WIFI	(0)
#define HDM_BT		(1)

static bool hdm_wifi_support;
module_param(hdm_wifi_support, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(hdm_wifi_support, "hdm_wifi_support");

static bool hdm_bt_support;
module_param(hdm_bt_support, bool, S_IRUGO | S_IWUSR);
...
bool hdm_is_applied(uint32_t bit)
{
	...
	switch (bit) {
	case HDM_WIFI:
		ret = hdm_wifi_support;
		SCSC_TAG_INFO(MXMAN, "For HDM_WIFI, it's for testing on ERD.(ret: %d)\n", ret);
		break;
	case HDM_BT:
		ret = hdm_bt_support;
		SCSC_TAG_INFO(MXMAN, "For HDM_BT, it's for testing on ERD.(ret: %d)\n", ret);
		break;
	default:
		SCSC_TAG_WARNING(MXMAN, "wrong value of HDM service: %d\n", bit);
		break;
	}
	return ret;
}
...
static bool exclude_loading_wlan_fw;

/* Check if a specific sysfs file for HDM exists */
bool mxman_check_sysfs_for_hdm(enum scsc_subsystem sub)
{
	int loader_status = 0;
	bool wpan = false;

	if (sub == SCSC_SUBSYSTEM_WLAN)
		loader_status = hdm_wlan_loader;
	else if (sub == SCSC_SUBSYSTEM_WPAN) {
		loader_status = hdm_wpan_loader;
		wpan = true;
	} else if (sub == SCSC_SUBSYSTEM_WLAN_WPAN || sub == SCSC_SUBSYSTEM_PMU) {
		if (hdm_wlan_loader > SLSI_MX_HDM_CONTROL_DISABLE) {
			loader_status = hdm_wlan_loader;
		} else if (hdm_wpan_loader > SLSI_MX_HDM_CONTROL_DISABLE) {
			loader_status = hdm_wpan_loader;
			wpan = true;
		}
	} else {
		SCSC_TAG_ERR(MXMAN, "wrong target subsystem: %d\n", sub);
		return false;
	}

	if (loader_status > SLSI_MX_HDM_CONTROL_DISABLE) {
		SCSC_TAG_INFO(MXMAN, "For %s (sub: %d), loader_status is set as %d.\n",
			wpan ? "hdm_wpan_loader":"hdm_wlan_loader", sub, loader_status);
		return true;
	}
	return false;
}

bool mxman_get_hdm_wifi_support(void)
{
	return hdm_is_applied(HDM_WIFI);
}

int mxman_get_hdm_wlan_loader(void)
{
	return hdm_wlan_loader;
}
...
static ssize_t sysfs_store_hdm_wlan_loader(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int r;
	int loader = 0;
	u16 panic_code = (SCSC_PANIC_CODE_HOST << 15) | (SCSC_SERVICE_ID_WLAN << SCSC_SYSERR_HOST_SERVICE_SHIFT);

	if (active_mxman == NULL) {
		SCSC_TAG_ERR(MXMAN, "No active MXMAN\n");
		return 0;
	}

	r = kstrtoint(buf, 10, &loader);
	if (r < 0)
		return 0;

	switch (loader) {
	case SLSI_MX_HDM_CONTROL_TERMINATE:
		SCSC_TAG_INFO(MXMAN, "Unlock the HW region restriction on WiFi service.\n");
		hdm_wlan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
		break;
	case SLSI_MX_HDM_CONTROL_ENABLE:
		SCSC_TAG_INFO(MXMAN, "Lock WiFi service by setting hdm_wlan_loader as '0'\n");
		if (hdm_wlan_loader == SLSI_MX_HDM_CONTROL_DISABLE) {
			hdm_wlan_loader = SLSI_MX_HDM_CONTROL_ENABLE;
			mxman_if_fail(active_mxman, panic_code, __func__, SCSC_SUBSYSTEM_WLAN);
		}
		break;
	default:
		SCSC_TAG_INFO(MXMAN, "The value(%d) of hdm_wlan_loader is not meaningful.\n", loader);
		break;
	}
...
void mxman_create_sysfs_hdm_wlan_loader(void)
{
	...
	if (r == -EEXIST) {
		...
		SCSC_TAG_INFO(MXMAN, "/sys/kernel/hdm_wlan_loader already exists\n");
	} else if (r) {
		/* Failed, so clean up dir */
		SCSC_TAG_ERR(MXMAN, "Can't create /sys/kernel/hdm_wlan_loader(reason: %d)\n", r);
	} else {
		SCSC_TAG_INFO(MXMAN, "/sys/kernel/hdm_wlan_loader created successfully\n");
		hdm_wlan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
	}
}
...
bool mxman_get_hdm_bt_support(void)
{
	return hdm_is_applied(HDM_BT);
}

int mxman_get_hdm_wpan_loader(void)
{
	return hdm_wpan_loader;
}

/* Retrieve hdm_wpan_loader in sysfs global */
static ssize_t sysfs_show_hdm_wpan_loader(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", hdm_wpan_loader);
}

/* Update hdm_wpan_loader in sysfs global */
static ssize_t sysfs_store_hdm_wpan_loader(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int r;
	int loader = 0;
	u16 panic_code = (SCSC_PANIC_CODE_HOST << 15) | (SCSC_SERVICE_ID_BT << SCSC_SYSERR_HOST_SERVICE_SHIFT);
	...
	switch (loader) {
	case SLSI_MX_HDM_CONTROL_TERMINATE:
		SCSC_TAG_INFO(MXMAN, "Unlock the HW region restriction on BT service.\n");
		hdm_wpan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
		break;
	case SLSI_MX_HDM_CONTROL_ENABLE:
		SCSC_TAG_INFO(MXMAN, "Lock BT service by setting hdm_wpan_loader as '0'\n");
		if (hdm_wpan_loader == SLSI_MX_HDM_CONTROL_DISABLE) {
			hdm_wpan_loader = SLSI_MX_HDM_CONTROL_ENABLE;
			mxman_if_fail(active_mxman, panic_code, __func__, SCSC_SUBSYSTEM_WPAN);
		}
		break;
	default:
		SCSC_TAG_INFO(MXMAN, "The value(%d) of hdm_wpan_loader is not meaningful.\n", loader);
		break;
	}
...
/* Register hdm_wpan_loader override */
void mxman_create_sysfs_hdm_wpan_loader(void)
{
	int r;

	/* Create sysfs file /sys/kernel/hdm_wpan_loader */
	r = sysfs_create_file(kernel_kobj, &hdm_wpan_loader_attr.attr);
	if (r == -EEXIST) {
		SCSC_TAG_INFO(MXMAN, "/sys/kernel/hdm_wpan_loader already exists\n");
	} else if (r) {
		/* Failed, so clean up dir */
		SCSC_TAG_ERR(MXMAN, "Can't create /sys/kernel/hdm_wpan_loader (reason: %d)\n", r);
	} else {
		SCSC_TAG_INFO(MXMAN, "/sys/kernel/hdm_wpan_loader created successfully\n");
		hdm_wpan_loader = SLSI_MX_HDM_CONTROL_DISABLE;
	}
}

/* Unregister hdm_wpan_loader override */
void mxman_destroy_sysfs_hdm_wpan_loader(void)
{
	/* Destroy /sys/kernel/hdm_wpan_loader file */
	sysfs_remove_file(kernel_kobj, &hdm_wpan_loader_attr.attr);
}
#endif
...
static ssize_t sysfs_store_memdump(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	case 2:
		...
		mif_abs->set_pcie_cb_panic(mif_abs, false);
		mif_abs->set_link_fail_panic(mif_abs, false);
#endif
		break;
	case 3:
		disable_recovery_from_memdump_file = true;
#if defined(CONFIG_SCSC_PCIE_CHIP) && !defined(CONFIG_SEC_FACTORY_INTERPOSER)
		mif_abs = scsc_mx_get_mif_abs(active_mxman->mx);
		mif_abs->set_pcie_cb_panic(mif_abs, true);
		mif_abs->set_link_fail_panic(mif_abs, true);
#endif
		break;
	default:
		disable_recovery_from_memdump_file = true;
...
void mxman_enable_split_recovery(struct mxman *mxman)
{
...
#if IS_ENABLED(CONFIG_SCSC_BB_REDWOOD) && IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
	unregister_mxman_force_panic(scsc_pcie_get_iommu_dom());
#endif
}
...
static int mxman_start_boot(struct mxman *mxman, enum scsc_subsystem sub)
{
...
#if IS_ENABLED(CONFIG_SCSC_BB_REDWOOD)
#if IS_ENABLED(CONFIG_EXYNOS_HSI_IOMMU)
	register_mxman_force_panic(scsc_pcie_get_iommu_dom(), &mxman_if_force_panic_active);
#else
	register_mxman_force_panic(&mxman_if_force_panic_active);
#endif
#endif
	...
	ret = mxman_res_fw_init(mxman, &mxman->fw_wlan, &mxman->fw_wpan, mxman->start_dram, mxman->size_dram,
			mxman_check_sysfs_for_hdm(SCSC_SUBSYSTEM_WLAN));
	if (sub == SCSC_SUBSYSTEM_WPAN && mxman_check_sysfs_for_hdm(SCSC_SUBSYSTEM_WLAN)) {
		SCSC_TAG_INFO(MXMAN, "Copy only wpan fw image to memory. Exclude loading wlan fw.\n");
		exclude_loading_wlan_fw = true;
	}
#else
	ret = mxman_res_fw_init(mxman, &mxman->fw_wlan, &mxman->fw_wpan, mxman->start_dram, mxman->size_dram);
#endif
	   ...
	   (mxman->scsc_panic_sub == SCSC_SUBSYSTEM_WLAN || mxman->scsc_panic_sub == SCSC_SUBSYSTEM_WLAN_WPAN)) {
		...
		case SCSC_R4_V2_MINOR_53:
			...
			if (ucpu_panic_record_ok) {
				...
				BUG_ON(sizeof(mxman->last_panic_rec_r) < ucpu_panic_record_length * sizeof(u32));
...
#endif
#if defined(CONFIG_HDM_WLBT_SUPPORT)
		if (mxman_get_hdm_wlan_loader() == SLSI_MX_HDM_CONTROL_ENABLE) {
			SCSC_TAG_INFO(MXMAN, "Disable Log Collection. This panic is caused by WiFi HDM Control.\n");
		} else if (mxman_recovery_disabled()) {
#else
		if (mxman_recovery_disabled()) {
#endif
...
skip_dump:
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (!mxman_recovery_disabled() || mxman_get_hdm_wlan_loader() == SLSI_MX_HDM_CONTROL_ENABLE) {
#else
	if (!mxman_recovery_disabled()) {
#endif
	 ...
	 */
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_recovery_disabled() && mxman_get_hdm_wlan_loader() == SLSI_MX_HDM_CONTROL_DISABLE)
#else
	if (mxman_recovery_disabled())
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_get_hdm_wlan_loader() == SLSI_MX_HDM_CONTROL_ENABLE) {
		mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_CLOSE_ONLY);
	} else if (!mxman_recovery_disabled()) {
#else
	if (!mxman_recovery_disabled()) {
#endif
...
#endif
#if defined(CONFIG_HDM_WLBT_SUPPORT)
		if (mxman_get_hdm_wpan_loader() == SLSI_MX_HDM_CONTROL_ENABLE) {
			SCSC_TAG_INFO(MXMAN, "Disable Log Collection. This panic is caused by BT HDM Control.\n");
		} else if (mxman_recovery_disabled()) {
#else
		if (mxman_recovery_disabled()) {
#endif
...
skip_dump:
...
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (!mxman_recovery_disabled() || mxman_get_hdm_wpan_loader() == SLSI_MX_HDM_CONTROL_ENABLE) {
#else
	if (!mxman_recovery_disabled()) {
#endif
		SCSC_TAG_INFO(MXMAN, "Calling srvman_unfreeze_services\n");
	 ...
	 */
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_recovery_disabled() && mxman_get_hdm_wpan_loader() == SLSI_MX_HDM_CONTROL_DISABLE)
#else
	if (mxman_recovery_disabled())
#endif
		complete(&mxman->recovery_completion);
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	bool hdm_panic = ((mxman_get_hdm_wlan_loader() == SLSI_MX_HDM_CONTROL_ENABLE && subsys == SYSERR_SUBSYS_WLAN)
		|| (mxman_get_hdm_wpan_loader() == SLSI_MX_HDM_CONTROL_ENABLE && subsys == SYSERR_SUBSYS_BT))
		? true : false;
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
		if (hdm_panic)
			SCSC_TAG_INFO(MXMAN, "Disable Log Collection. This panic is caused by HDM Control.\n");
		else if (disable_recovery_handling) {
#else
...
#endif
#if !defined(CONFIG_WLBT_SPLIT_RECOVERY)
		if (scsc_wlbtd_get_wlbtd_termination_flag() && memdump == 3) {
			SCSC_TAG_INFO(MXMAN,"Skip log collection and wait for shutdown\n");
			goto skip_dump;
		}
#endif
		SCSC_TAG_INFO(MXMAN, "Trying to schedule coredump\n");
...
#if defined(CONFIG_HDM_WLBT_SUPPORT) || !defined(CONFIG_WLBT_SPLIT_RECOVERY)
skip_dump:
#endif
	SCSC_TAG_INFO(MXMAN, "Auto-recovery: %s\n", mxman_recovery_disabled() ? "off" : "on");
	...
	mutex_unlock(&mxman->mxman_mutex);
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (!mxman_recovery_disabled() || hdm_panic) {
#else
	if (!mxman_recovery_disabled()) {
#endif
		SCSC_TAG_INFO(MXMAN, "Calling srvman_unfreeze_services\n");
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (hdm_panic)
		mxman_send_rcvry_evt_to_fsm(mxman, RCVRY_EVT_RESET_DONE);
	else if (!mxman_recovery_disabled()) {
#else
	if (!mxman_recovery_disabled()) {
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_recovery_disabled() && !mxman_check_sysfs_for_hdm(th->target_sub)) {
#else
	if (mxman_recovery_disabled()) {
#endif
		goto exit;
...
static int mxman_set_pmu_to_monitor_mode(struct mxman *mxman, enum scsc_subsystem sub, bool bhost_fail)
{
	int ret = 0;
...
#endif
	if ((enable_scan2mem_dump && !bhost_fail) || fail_after_recovery_enable)
	...
	while (true) {
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
				if (enable_scan2mem_dump && !mxman_check_sysfs_for_hdm(mxman->scsc_panic_sub))
#else
				if (enable_scan2mem_dump)
#endif
					mxman_set_recovery_mode(mxman, false);

				ret = mxman_set_pmu_to_monitor_mode(mxman,
													th->last_panic_sub,
													(th->last_panic_code & (SCSC_PANIC_CODE_HOST << 15)) ? true : false);
				if (ret) {
		...
		case RCVRY_STATE_SUBSYSTEM_RESET:
			if (evt_data->event == RCVRY_EVT_RESET_DONE) {
				...
				th->indep_substate = (th->indep_substate == WLAN_INDEP_NORMAL) ? WLAN_INDEP_ERR : WPAN_INDEP_ERR;
				srvman_set_error_complete(scsc_mx_get_srvman(mxman->mx), NOT_ALLOWED_STOP);
				recovery_state = RCVRY_STATE_ERROR_IN_PROGRESS;
			}
#if defined(CONFIG_HDM_WLBT_SUPPORT)
			else if (evt_data->event == RCVRY_EVT_CLOSE_ONLY) {
				recovery_state = RCVRY_STATE_NONE;
			}
#endif
			break;
		case RCVRY_STATE_WAITING_START:
			if (evt_data->event == RCVRY_EVT_START_SUCCESS) {
				mxman_enable_split_recovery(mxman);
				recovery_state = RCVRY_STATE_NONE;
...
void mxman_park_recovery_thread(void)
{
	struct rcvry_fsm_thread *th;
	if (!active_mxman) {
		SCSC_TAG_ERR(MXMAN, "No active MXMAN\n");
		return;
	} else if (memdump != 3) {
	...
	th = &active_mxman->rcvry_thread;
	if (th->kthread_is_parked)
		return;

	kthread_park(th->task);
	th->kthread_is_parked = true;

	SCSC_TAG_WARNING(MXMAN, "Due to WLBTD termination, the recovery thread is parked!\n");
}
EXPORT_SYMBOL(mxman_park_recovery_thread);

void mxman_unpark_recovery_thread(void)
{
	struct rcvry_fsm_thread *th;
	if (!active_mxman) {
		SCSC_TAG_ERR(MXMAN, "No active MXMAN\n");
	...
	kthread_unpark(th->task);
	th->kthread_is_parked = false;

	SCSC_TAG_WARNING(MXMAN, "Due to WLBTD initialization, the recovery thread is unparked!\n");
}
EXPORT_SYMBOL(mxman_unpark_recovery_thread);
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	} else if (mxman->users == 0 && mxman->users_wpan > 0 &&
			sub == SCSC_SUBSYSTEM_WLAN && exclude_loading_wlan_fw) {
		ret = mxman_res_delayed_wlan_fw_init(mxman, mxman->start_dram);
		if (ret) {
			SCSC_TAG_ERR(MXMAN, "Error loading wlan fw only!\n");
			goto error;
		}
		exclude_loading_wlan_fw = false;
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
void mxman_hdm_handling(struct mxman *mxman, u16 failure_source, const char *reason, enum scsc_subsystem sub, bool bFull)
#else
void mxman_hdm_handling(struct mxman *mxman, u16 failure_source, const char *reason, enum scsc_subsystem sub)
#endif
{
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
	struct rcvry_fsm_thread		*th = &mxman->rcvry_thread;
...
#endif
	SCSC_TAG_WARNING(MXMAN, "[HDM Handling] target subsystem: %s\n", sub ? "WPAN":"WLAN");
	...
	th->last_panic_level = MX_SYSERR_LEVEL_7;
	strscpy(th->last_failure_reason, reason, sizeof(th->last_failure_reason));

	if (failure_source & (SCSC_PANIC_CODE_HOST << 15))
		evt = RCVRY_EVT_RECOVER_HOST;
	else {
		switch(sub) {
		case SCSC_SUBSYSTEM_WLAN:
			evt = (bFull) ? RCVRY_EVT_ERROR : RCVRY_EVT_RECOVER_WLAN;
			break;
		case SCSC_SUBSYSTEM_WPAN:
			evt = (bFull) ? RCVRY_EVT_ERROR : RCVRY_EVT_RECOVER_WPAN;
			break;
		default:
			evt = RCVRY_EVT_ERROR;
			break;
	...
	case SCSC_SUBSYSTEM_WLAN_WPAN:
		mxman->last_syserr.subsys = (mxman_get_hdm_wlan_loader() > SLSI_MX_HDM_CONTROL_DISABLE)
									? SYSERR_SUBSYS_WLAN:SYSERR_SUBSYS_BT;
	...
	strscpy(mxman->failure_reason, reason, sizeof(mxman->failure_reason));
...
void mxman_set_failure_params(struct mxman *mxman, bool full_recovery)
{
	 ...
	 */
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_recovery_disabled() && !mxman_check_sysfs_for_hdm(th->last_panic_sub))
#else
	if (mxman_recovery_disabled())
#endif
...
#if defined(CONFIG_HDM_WLBT_SUPPORT)
	if (mxman_check_sysfs_for_hdm(sub)) {
		mxman_hdm_handling(mxman, failure_source, reason, sub, false);
		return;
	}
#endif
...
void mxman_fail(struct mxman *mxman, u16 failure_source, const char *reason, enum scsc_subsystem sub)
{
	...
	if (mxman_check_sysfs_for_hdm(sub)) {
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		mxman_hdm_handling(mxman, failure_source, reason, sub, true);
#else
		mxman_hdm_handling(mxman, failure_source, reason, sub);
#endif
		return;
	}
#endif
...
void mxman_init(struct mxman *mxman, struct scsc_mx *mx)
{
	...
	mxman_create_sysfs_hdm_wpan_loader();

	if (mxman_get_hdm_wifi_support()) {
		hdm_wlan_loader = SLSI_MX_HDM_CONTROL_ENABLE;
		SCSC_TAG_INFO(MXMAN, "WiFi Service has been disabled by HDM before device booting.\n");
	}
	if (mxman_get_hdm_bt_support()) {
		hdm_wpan_loader = SLSI_MX_HDM_CONTROL_ENABLE;
		SCSC_TAG_INFO(MXMAN, "BT Service has been disabled by HDM before device booting.\n");
	}
#endif
	scsc_lerna_init();
#if IS_ENABLED(CONFIG_SCSC_MXLOGGER)
...
void mxman_deinit(struct mxman *mxman)
{
	...
	mxman_destroy_sysfs_hdm_wpan_loader();
#endif
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
...
#if IS_ENABLED(CONFIG_SCSC_BB_REDWOOD)
int mxman_force_panic_active(void)
{
	scsc_wlbtd_set_sysmmu_fault_panic_flag(1);
	disable_recovery_handling = 1;
	return mxman_force_panic(active_mxman, SCSC_SUBSYSTEM_WLAN);
}
#endif
